...
function ccPatchSessionKey(s){if(!s)return null;try{return(s.sessionId&&s.sessionId.value)||s.internalId||null}catch(e){return null}}
...
function ccPatchQueueNotify(){try{localStorage.setItem(`ccPatchQueue:`+(globalThis.ccPatchWsKey||``),JSON.stringify((globalThis.ccPatchQueue||[]).filter(function(it){return it.skey}).map(function(it){return{id:it.id,text:it.text,sel:it.sel,skey:it.skey}})))}catch(e){}globalThis.ccPatchQueueListeners.forEach(function(f){try{f()}catch(err){}})}
function ccPatchSetSendMode(m){globalThis.ccPatchSendMode=m;try{localStorage.setItem(`ccPatchSendMode`,m)}catch(e){}}
function ccPatchQueueTick(){var s=ccPatchActiveSession();if(s!==globalThis.ccPatchQueueSessRef){globalThis.ccPatchQueueSessRef=s;globalThis.ccPatchQueueWasBusy=!1}var busy=!1;try{busy=!!(s&&s.busy&&s.busy.value)}catch(e){}if(busy){globalThis.ccPatchQueueWasBusy=!0;return}if(!globalThis.ccPatchQueueWasBusy)return;globalThis.ccPatchQueueWasBusy=!1;if(!s)return;var key=ccPatchSessionKey(s),q=globalThis.ccPatchQueue||[],idx=-1;for(var i=0;i<q.length;i++){if(q[i].sess===s||(key&&q[i].skey===key)){idx=i;break}}if(idx<0)return;var item=q[idx];globalThis.ccPatchQueue=q.slice(0,idx).concat(q.slice(idx+1));ccPatchQueueNotify();ccPatchSendText(item.text,item.files,item.sel)}
