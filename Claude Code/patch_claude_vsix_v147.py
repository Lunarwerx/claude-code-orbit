...
function ccPatchSyncWsTheme(sess,ctx){var cwd=``;try{cwd=(sess&&sess.defaultCwd&&sess.defaultCwd.value)||(ctx&&ctx.defaultCwd&&ctx.defaultCwd.value)||``}catch(e){}if(cwd===globalThis.ccPatchWsKey)return;globalThis.ccPatchWsKey=cwd;ccPatchApplyTheme(ccPatchWsTheme())}
