...
function ccPatchShowDefaultsMenu(){if(document.querySelector(`.ccPatchDefaultsOverlay`)){ccPatchCloseDefaultsMenu();return}try{ccPatchCloseSettingsMenu()}catch(e){}var ctx=globalThis.ccPatchCtx;var ov=document.createElement(`div`);ov.className=`ccPatchDefaultsOverlay`;var card=document.createElement(`div`);card.className=`ccPatchDefaultsCard`;ov.appendChild(card);var head=document.createElement(`div`);head.className=`ccPatchDefHead`;head.textContent=`Default for new chats`;card.appendChild(head);var sub=document.createElement(`div`);sub.className=`ccPatchDefSub`;sub.textContent=`What every new chat opens on. Chats already going keep their own.`;card.appendChild(sub);var saved={model:ccPatchDefaultModelGet(),mode:ccPatchDefaultModeGet(),effort:ccPatchDefaultEffortRaw()};var staged={model:saved.model,mode:saved.mode,effort:saved.effort};var saveBtn;function dirty(){return staged.model!==saved.model||staged.mode!==saved.mode||staged.effort!==saved.effort}function refreshSave(){if(saveBtn){var d=dirty();saveBtn.disabled=!d;saveBtn.classList.toggle(`ccPatchSaveOn`,d)}}var models=ccPatchAvailableModels();var modelOpts=[[``,`Account default`]];for(var i=0;i<models.length;i++){if(models[i]&&models[i].value)modelOpts.push([models[i].value,models[i].displayName||models[i].value])}card.appendChild(ccPatchDefRow(`Model`,ccPatchBuildSelect(staged.model,modelOpts,function(v){staged.model=v;refreshSave()})));var modeSel=ccPatchBuildSelect(staged.mode,ccPatchModeStops(),function(v){if(v===`bypassPermissions`){ccPatchYoloConfirm(function(){staged.mode=v;refreshSave()},function(){modeSel.ccSet(staged.mode)})}else{staged.mode=v;refreshSave()}});card.appendChild(ccPatchDefRow(`Mode`,modeSel));(function(){var w=document.createElement(`div`);w.className=`ccPatchDefEffWrap`;var vl=document.createElement(`span`);vl.className=`ccPatchDefEffVal`;w.appendChild(vl);w.appendChild(ccPatchBuildEffortSlider(vl,function(id){staged.effort=id;refreshSave()},staged.effort));card.appendChild(ccPatchDefRow(`Effort`,w))})();(function(){var row=document.createElement(`div`);row.className=`ccPatchDefSaveRow`;saveBtn=document.createElement(`button`);saveBtn.type=`button`;saveBtn.className=`ccPatchSaveBtn`;saveBtn.textContent=`Save`;saveBtn.disabled=true;saveBtn.onmousedown=function(e){e.preventDefault();e.stopPropagation();if(!dirty())return;ccPatchDefaultModelSet(staged.model);ccPatchDefaultModeSet(staged.mode);ccPatchSetDefaultEffort(staged.effort);saved={model:staged.model,mode:staged.mode,effort:staged.effort};refreshSave();saveBtn.textContent=`Saved`;setTimeout(function(){if(saveBtn)saveBtn.textContent=`Save`},900)};row.appendChild(saveBtn);card.appendChild(row)})();var divEl=document.createElement(`div`);divEl.className=`ccPatchDefDivider`;card.appendChild(divEl);(function(){var d=document.createElement(`div`);d.className=`ccPatchDefActionRow`;var l=document.createElement(`span`);l.textContent=`Auto-resume`;d.appendChild(l);var ex=document.createElement(`span`);ex.className=`ccPatchDefActionHint`;ex.textContent=ccPatchContAnyOn()?`On`:`Off`;d.appendChild(ex);d.onmousedown=function(e){e.preventDefault();e.stopPropagation();ccPatchCloseDefaultsMenu();setTimeout(ccPatchShowContinuationMenu,0)};card.appendChild(d)})();(function(){var d=document.createElement(`div`);d.className=`ccPatchDefActionRow`;var l=document.createElement(`span`);l.textContent=`Copy diagnostics`;d.appendChild(l);d.onmousedown=function(e){e.preventDefault();e.stopPropagation();try{ccPatchCopyDiagnostics(ctx||globalThis.ccPatchCtx)}catch(er){}ccPatchCloseDefaultsMenu()};card.appendChild(d)})();document.body.appendChild(ov);var k=function(g){if(g.key===`Escape`&&!document.querySelector(`.ccPatchConfirmOverlay`))ccPatchCloseDefaultsMenu()};ov._ccPatchKey=k;document.addEventListener(`keydown`,k);ov.onmousedown=function(g){if(g.target===ov)ccPatchCloseDefaultsMenu()};card.onmousedown=function(g){g.stopPropagation()}}
