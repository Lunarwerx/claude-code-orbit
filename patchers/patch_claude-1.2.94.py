...
// signal (rendered as the errorBanner). The exact wording/prefix is SERVER-supplied
// ("API Error" is not even a literal in the bundle), so ccPatchContErrorStop reads
// s.error.value directly -- any non-empty value is, by construction, a real turn
// error (it's only ever set in a catch). ccPatchContErrSig below is the SECONDARY
// text-signature used only when scraping ambiguous message/DOM text, where false
// positives matter; it's distinctive enough not to fire on normal assistant output.
function ccPatchContErrSig(t){t=String(t||``);if(/status\.claude\.com|server-?side issue|temporarily unavailable/i.test(t))return true;return /API Error\b/i.test(t)&&/Internal server error|try again|overloaded|rate limit|too many requests|\b(5\d\d|429)\b/i.test(t)}
...
def verify_extension_dir(extension_dir: Path) -> None:
    ...
    checks = {
        ...
        # while a REAL maxed usage window (>=95%) still waits for its reset.
