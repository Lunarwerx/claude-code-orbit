...
// Centered modal (Jacob: the corner popup looked bad). Backdrop + card; the model
// and mode pickers are custom ccPatchBuildSelect dropdowns; effort reuses the slider.
function ccPatchShowDefaultsMenu(){if(document.querySelector(`.ccPatchDefaultsOverlay`)){ccPatchCloseDefaultsMenu();return}try{ccPatchCloseSettingsMenu()}catch(e){}var ov=document.createElement(`div`);ov.className=`ccPatchDefaultsOverlay`;var card=document.createElement(`div`);card.className=`ccPatchDefaultsCard`;ov.appendChild(card);var head=document.createElement(`div`);head.className=`ccPatchDefHead`;head.textContent=`Default for new chats`;card.appendChild(head);var sub=document.createElement(`div`);sub.className=`ccPatchDefSub`;sub.textContent=`What every new chat opens on. Chats already going keep their own.`;card.appendChild(sub);var models=ccPatchAvailableModels();var modelOpts=[[``,`Account default`]];for(var i=0;i<models.length;i++){if(models[i]&&models[i].value)modelOpts.push([models[i].value,models[i].displayName||models[i].value])}card.appendChild(ccPatchDefRow(`Model`,ccPatchBuildSelect(ccPatchDefaultModelGet(),modelOpts,function(v){ccPatchDefaultModelSet(v)})));card.appendChild(ccPatchDefRow(`Mode`,ccPatchBuildSelect(ccPatchDefaultModeGet(),ccPatchModeStops(),function(v){ccPatchDefaultModeSet(v)})));(function(){var w=document.createElement(`div`);w.className=`ccPatchDefEffWrap`;var vl=document.createElement(`span`);vl.className=`ccPatchDefEffVal`;vl.textContent=ccPatchDefaultEffortLabel()||`Auto`;w.appendChild(vl);w.appendChild(ccPatchBuildEffortSlider(vl));card.appendChild(ccPatchDefRow(`Effort`,w))})();document.body.appendChild(ov);var k=function(g){if(g.key===`Escape`)ccPatchCloseDefaultsMenu()};ov._ccPatchKey=k;document.addEventListener(`keydown`,k);ov.onmousedown=function(g){if(g.target===ov)ccPatchCloseDefaultsMenu()};card.onmousedown=function(g){g.stopPropagation()}}
...
def patch_webview_js(webview_js: Path) -> bool:
    ...
    # props so the in-popup effort row disappears (it is standalone now).
...
def patch_webview_css(webview_css: Path) -> bool:
    ...
    new = old + modal_overlay_rule + (
        ...
        ".ccPatchDefaultsOverlay{position:fixed;inset:0;z-index:2147483646;display:flex;"
