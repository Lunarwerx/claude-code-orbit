...
# the account's organizationUuid. Switching just copies a saved .cred.json over
# ~/.claude/.credentials.json; getOAuthTokens() reads that file fresh per request,
# so the next message/new chat is the new account with no reload. ccEnsure()
# auto-saves the CURRENT login before any switch so it's never lost, and mirrors
# any refreshed token back into its slot. Placeholders __MSG__/__FS__/__PATH__ are
# replaced with the captured host request/fs/path identifiers.
