...
// Orbit "YOLO mode" removed: it relabeled + force-injected the native
// `bypassPermissions` mode, but that mode is broken upstream in the VS Code
// extension (the host `requestToolPermission` ignores the permission mode and
// prompts anyway — Anthropic issues #43953/#49173), so the button never actually
// skipped prompts. Permission modes are now left 100% native. (.ccPatchYoloToggle
// / .ccPatchYoloSlider CSS classes are kept — they're the generic toggle styling
// reused by the Chat-preview, Goal-enforcement, and Auto-resume switches.)
