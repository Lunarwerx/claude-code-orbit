...
try{if(localStorage.getItem(`ccPatchYoloMode`)===`1`)document.documentElement.classList.add(`ccPatchYoloMode`)}catch(e){}
...
function ccPatchYoloSet(on){document.documentElement.classList.toggle(`ccPatchYoloMode`,!!on);try{localStorage.setItem(`ccPatchYoloMode`,on?`1`:`0`)}catch(e){}}
function ccPatchYoloToggle($){var on=!ccPatchYoloOn();ccPatchYoloSet(on);if($){try{ccPatchYoloApplyArr($.sessions&&$.sessions.value,on)}catch(e){}try{ccPatchYoloApplyArr($.remoteSessions&&$.remoteSessions.value,on)}catch(e){}}}
...
def patch_webview_js(webview_js: Path) -> bool:
    ...
    # ──────────────────────────────────────────────────────────────────────
    if not feature_on("yolo-mode"):
        log("Skipping YOLO perm patch (feature disabled)")
    else:
        m_perm = re.search(
    ...
    #      Settings ▸ "Default" slider. Mirrors the YOLO seam: the session-class
    #      field initializer `effortLevel=at(void 0)` becomes
    ...
    mode_new = (
        ...
        f'onSelect:(ccPE)=>{{try{{ccPatchYoloSet(ccPE==="bypassPermissions")}}catch(ccE){{}}{XE}.setPermissionMode(ccPE,!0)}},anchorRight:!0}}))'
    )
