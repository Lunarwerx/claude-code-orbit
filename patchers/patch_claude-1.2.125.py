...
try{if(ccPatchAutoApproveOn())document.documentElement.classList.add(`ccPatchAutoApproveOn`)}catch(e){}
try{ccPatchAutoApproveTick()}catch(e){}
try{if(!globalThis.ccPatchAutoApproveTimer)globalThis.ccPatchAutoApproveTimer=setInterval(ccPatchAutoApproveTick,7000)}catch(e){}
...
def patch_webview_js(webview_js: Path) -> bool:
    ...
    # Orbit's REAL host-side auto-approve (the requestToolPermission short-circuit set
    # via the orbit_set_yolo RPC), NOT the broken native bypassPermissions enforcement
    # — see the mode-selector onSelect in block 18b below. The label anchor is
    # required (the whole entry hinges on it); the description is best-effort.
    ...
    mode_new = (
        ...
        f'{RE}.default.createElement({GJE},{{mode:(ccPatchAutoApproveOn()?"bypassPermissions":{XT}),'
        f'availableModes:(function(ccA){{return ccA&&ccA.indexOf("bypassPermissions")<0?ccA.concat(["bypassPermissions"]):ccA}})({XI}),'
        f'onSelect:(ccPE)=>{{if(ccPE==="bypassPermissions"){{var ccGo=function(){{ccPatchAutoApproveSet(!0);ccPatchAutoApprovePush(!0);{XE}.setPermissionMode("bypassPermissions",!0)}};if(ccPatchYoloConfirmedGet())ccGo();else ccPatchYoloConfirm(ccGo)}}else{{ccPatchAutoApproveSet(!1);ccPatchAutoApprovePush(!1);{XE}.setPermissionMode(ccPE,!0)}}}},anchorRight:!0}}))'
    )
...
def patch_webview_css(webview_css: Path) -> bool:
    ...
    new = old + modal_overlay_rule + (
        ...
        # Real YOLO ("Auto-approve everything") indicator: a fixed red badge while
        # on, so a session that auto-approves everything can never be silently left
        # running. Driven by the html.ccPatchAutoApproveOn class (ccPatchAutoApproveSet).
        "html.ccPatchAutoApproveOn body::after{content:\"YOLO MODE - auto-approving everything\";"
        "position:fixed;bottom:8px;right:10px;z-index:2147483646;background:#b91c1c;color:#fff;"
        "font-size:10.5px;font-weight:600;letter-spacing:.2px;padding:3px 9px;border-radius:6px;"
        "pointer-events:none;opacity:.93;font-family:var(--app-font-family,sans-serif)}"
...
def patch_extension_host_js(path: Path) -> bool:
    ...
    if "ccOrbitApiLog" not in text:
        ...
        changed = True
    # ── Real YOLO ("Auto-approve everything") ──────────────────────────────
    # The unbreakable seam: requestToolPermission is the SINGLE chokepoint every
    # tool / file-edit / bash approval funnels through (it sends the prompt to the
    # webview and returns its answer). When the webview "YOLO mode" toggle is on it
    # sets the host global ccOrbitYolo (via the orbit_set_yolo RPC below); we short-
    # circuit to "allow" right here, so the prompt never even renders — independent
    # of the native permission mode (which the IDE's requestToolPermission ignores,
    # the bug that made the old bypassPermissions "YOLO" never work). The native
    # chrome-MCP early-return right after uses this exact shape. OFF (the global is
    # undefined) until the webview explicitly turns it on.
    if "globalThis.ccOrbitYolo" not in text:
        m_rtp = re.search(
            rf"async requestToolPermission\((?P<params>{JS_ID}(?:,{JS_ID})*)\)\{{",
            text,
        )
        if not m_rtp:
            raise RuntimeError("Lost requestToolPermission anchor (real YOLO auto-approve)")
        _yolo_inputs = m_rtp.group("params").split(",")[2]
        _yolo_ins = f'if(globalThis.ccOrbitYolo)return{{behavior:"allow",updatedInput:{_yolo_inputs}}};'
        text = text[: m_rtp.end()] + _yolo_ins + text[m_rtp.end():]
        changed = True
    ...
    insert = (
        ...
        # orbit_set_yolo: the webview's real "YOLO mode" toggle sets the host global
        # that the requestToolPermission short-circuit (above) reads. Carries the
        # boolean only; OFF (undefined) by default. Re-asserted by a webview tick so
        # the host re-learns the state after a reload/restart.
        f'case"orbit_set_yolo":{{try{{globalThis.ccOrbitYolo=!!{msg}.request.on}}catch(ccE){{}}'
        f'return{{type:"orbit_set_yolo_response",ok:!0,on:!!globalThis.ccOrbitYolo}}}}'
    )
...
def verify_extension_dir(extension_dir: Path) -> None:
    ...
    checks = {
        ...
        # Real YOLO ("Auto-approve everything"): the unbreakable host short-circuit
        # of requestToolPermission gated on globalThis.ccOrbitYolo, set by the
        # orbit_set_yolo RPC; the webview toggle + state push + the red indicator.
        "yolo auto-approve host": ("async requestToolPermission" in host
                                   and 'if(globalThis.ccOrbitYolo)return{behavior:"allow"' in host
                                   and 'case"orbit_set_yolo"' in host),
        "yolo mode menu": ('label:"YOLO mode"' in js and 'label:"Bypass permissions"' not in js
                           and "availableModes:(function(ccA)" in js
                           and "ccPatchAutoApproveOn" in js and "ccPatchAutoApprovePush" in js
                           and "ccPatchYoloConfirm" in js and "orbit_set_yolo" in js
                           and ".ccPatchAutoApproveOn body::after" in css),
