...
function ccPatchContTransientErr(s){function sig(t){return /not your usage limit|temporarily limiting/i.test(String(t||``))}try{var ev=s&&s.error&&s.error.value;if(typeof ev===`string`&&sig(ev))return true}catch(e){}try{if(s===ccPatchActiveSession()){var eb=document.querySelectorAll(`[class*="errorBanner_"],[class*="errorMessage_"]`);for(var b=0;b<eb.length;b++){if(eb[b]&&sig(eb[b].innerText||eb[b].textContent||``))return true}}}catch(e){}return false}
