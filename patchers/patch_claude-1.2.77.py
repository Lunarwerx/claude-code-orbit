...
def patch_webview_js(webview_js: Path) -> bool:
    ...
    # ──────────────────────────────────────────────────────────────────────
    # 17b. Fork-from-here "branch now, visit later" rewrite — the native
    #     fork handler (`ee()`) calls the CONTEXT service's forkConversation,
    #     which (a) forks off the on-disk transcript and (b) immediately tries
    #     to NAVIGATE to the new session via `viewSession`. Both halves race a
    #     live, mid-stream turn: while busy, the session list / disk snapshot
    #     lags the live conversation, so `viewSession` -> activateSessionFrom
    #     Server's `listSessions().find(newId)` misses and returns a bare
    #     `false`, and the service's un-awaited call + `return""` swallows it.
    #     Net: click Fork mid-stream -> the branch is often created but nothing
    #     visibly happens (no toast, no navigation). That is the "fork button
    #     never works" report — it's a SILENT navigation no-op, not a throw.
    #
    #     The community-loved model (GitHub Copilot's fork) is: click Fork ->
    #     it snapshots everything ABOVE this message into a separate chat
    #     INSTANTLY, tells you it exists, and lets you open it at your leisure
    #     — it does NOT yank you away. So we stop depending on the fragile
    #     auto-navigation entirely:
    #       - call the raw connection RPC (`comms.connection.value.fork
    #         Conversation(sessionId, resumeAt)`) to CREATE the branch and get
    #         its new id, with NO navigation;
    #       - always confirm with a toast ("Forked from here ✓ — open your new
    #         branch anytime", with an [Open] button) so it is never silent;
    #       - only navigate if the user clicks [Open], mirroring the service's
    #         own openNewInTab vs viewSession choice (so it lands the same way
    #         the stock flow would, but on-demand and after the race is gone).
    #       - any genuine failure now surfaces a real error toast instead of a
    #         silent no-op.
    #     The first-message branch (`if(A){r(o)}` -> onCreateNewSession) and the
    #     "Unknown session id" guard are preserved byte-for-byte; a defensive
    #     fallback to the stock service call is kept in case the `comms`/
    #     `connection` internals move in a future Claude build. Identifiers are
    #     captured (not hard-coded) so minifier renames across versions are
    #     handled. Anchored on the unique "Unknown session id" fork string;
    #     the rewind-modal's `g(b)` twin shares that string but does NOT match
    #     this shape (its guards are `!b.resumeAtMessageId` / `n(b.promptText)`).
    # ──────────────────────────────────────────────────────────────────────
    ...
    if not feature_on("fork-row"):
        ...
        if m_ee is None:
            ...
            new_ee = (
                ...
                f'{TT}.showNotification("Forked from here \\u2713 \\u2014 open your new branch anytime.","info",["Open"]).then(function(ccForkBtn){{'
                f'if(ccForkBtn!=="Open")return;var ccForkCfg=ccForkConn.config&&ccForkConn.config.value;'
                f'if(ccForkCfg&&ccForkCfg.openNewInTab&&typeof {E}.startNewConversationTab==="function"){E}.startNewConversationTab({O},ccForkNewId);'
                f'else if(typeof {E}.viewSession==="function"){E}.viewSession(ccForkNewId,{O})}})'
            ...
            log("Fork-from-here rewired: branch-now/visit-later (no silent mid-stream no-op)")
...
def verify_extension_dir(extension_dir: Path) -> None:
    ...
    checks = {
        ...
        # Fork-from-here is rewired (block 17b): the stock service wrapper both
        # forks AND auto-navigates, and the navigation silently no-ops mid-stream
        # (un-awaited viewSession + listSessions().find miss + return""). We now
        # call the raw connection RPC to create the branch, confirm with a toast,
        # and only navigate on the [Open] button — see "fork mid-stream fix".
    }
