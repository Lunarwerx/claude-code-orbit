...
// YOLO is scoped to ONE chat — keyed by the session's claudeChannelId, exactly
// like the native ask/plan/edit modes (NOT global). The actual auto-approve runs
// in the webview's handleToolPermissionRequest (see patcher patch_webview_js): it
// calls ccPatchAutoApproveOn(channelId) and returns "allow" before the prompt can
// render. State is a JSON map { channelId: 1 } in localStorage — no global flag,
// no host involvement, so YOLO can never leak across chats. (A legacy global "1"/
// "0" value from earlier builds parses to a non-object and is harmlessly ignored,
// resetting everyone to per-chat OFF.) ccPatchConfirmDialog is in-scope here; the
// mode selector (bundle scope) calls the EXPORTED ccPatchYoloConfirm wrapper.
...
// session.setModel, YOLO via ccPatchAutoApproveSet once the chat has a
// claudeChannelId (void 0 until first send). Model list = the account's
...
ORBIT_WHATSNEW: str = "**What's new since the last stable update**\n\n**A real YOLO mode.** Open the composer's Modes menu (\u26a1) and pick **YOLO mode** \u2014 right under Auto mode \u2014 and that chat auto-approves every prompt: file edits, terminal commands, MCP tools. Claude never stops to ask. It's **per-chat**, so turning it on in one chat leaves your others alone. (The old \"YOLO\" was just a relabel of a mode the editor quietly ignores; this one actually works, because Orbit answers the approval itself.)\n\n**A new \"Default\" panel.** The gear menu's \"Switch model\" is now **Default** \u2014 one panel where you set what every *new* chat opens on: default **model**, default **mode** (including YOLO), **effort**, and your **Auto-resume** preferences. Change what you like, hit **Save**, and new chats follow it; chats already going keep their own.\n\n**Auto-resume, simplified.** It's now just two switches \u2014 resume a chat when your usage limit resets (5-hour or weekly), and resume if a chat errors out.\n\n**\"New Session\" no longer bounces you back** to the chat you came from a moment after you open a fresh one.\n\nPlus a round of polish: a cleaner composer bottom bar, clearer status and usage display, and a tidier settings menu."
def patch_webview_js(webview_js: Path) -> bool:
    ...
    # When the chat whose channelId this is has YOLO on, return "allow" immediately
    # so the prompt never renders. Keyed by channelId (= the session's
    # claudeChannelId), so YOLO is scoped to ONE chat, exactly like the native
    # ask/plan/edit modes — NOT global. No host involvement: the state lives in the
    # webview (ccPatchAutoApprove map, keyed by channelId). updatedInput echoes the
    # original inputs so the tool runs unmodified.
    # ──────────────────────────────────────────────────────────────────────
    m_htp = re.search(
    ...
    _htp_ins = (
        f'if(globalThis.ccPatchAutoApproveOn&&globalThis.ccPatchAutoApproveOn({m_htp.group("c")}))'
    ...
    # Mode selector: keep the polished V2 popup (anchorRight) and RESTORE the effort
    # props — the standalone composer effort slider was removed (Jacob), so effort now
    # lives in the Default panel (new chats) + here in the Modes popup (current chat).
    ...
    # after "auto"). Its onSelect drives Orbit's REAL auto-approve — ccPatchAutoApprove
    # (host requestToolPermission short-circuit), with a one-time confirm — instead of
    # the broken native enforcement. The displayed `mode` is forced to bypassPermissions
    # whenever our auto-approve is on (ccPatchAutoApproveOn), so the menu shows "YOLO mode"
    # selected even after a reload (the native mode signal downgrades bypassPermissions).
    ...
    mode_new = (
        ...
        f'{RE}.default.createElement({GJE},{{mode:(ccPatchAutoApproveOn({XE}.claudeChannelId)?"bypassPermissions":{XT}),'
        f'availableModes:(function(ccA){{return ccA&&ccA.indexOf("bypassPermissions")<0?ccA.concat(["bypassPermissions"]):ccA}})({XI}),'
        f'onSelect:(ccPE)=>{{if(ccPE==="bypassPermissions"){{ccPatchYoloConfirm(function(){{ccPatchAutoApproveSet({XE}.claudeChannelId,!0);{XE}.setPermissionMode("bypassPermissions",!0)}})}}else{{ccPatchAutoApproveSet({XE}.claudeChannelId,!1);{XE}.setPermissionMode(ccPE,!0)}}}},'
        f'effortLevel:{XF},supportedEffortLevels:{XG},onSetEffort:{XB},ultracodeAvailable:{XUCA},ultracodeSelected:{XUCS},onSelectUltracode:{XOUC},'
        f'anchorRight:!0}}))'
    )
    ...
    # (Standalone composer effort slider removed per Jacob — effort is restored into
    # the Modes popup above and set as a new-chat default in the "Default" panel.)
    ...
    new_return = (
        ...
        f"return {g_open}"
        f"{g_attach},{mode_new},{model_new},{g_sel},{g_spacer},"
...
def verify_extension_dir(extension_dir: Path) -> None:
    ...
    checks = {
        ...
        # handleToolPermissionRequest, keyed by channelId — no host global, so it
        # can't leak across chats. (Always applied, independent of COMPOSER_REWORK.)
        "yolo per-chat handler": ("handleToolPermissionRequest" in js
                                  and "globalThis.ccPatchAutoApproveOn(" in js
        ...
        "yolo mode menu": ('label:"YOLO mode"' in js and 'label:"Bypass permissions"' not in js
                           ...
                           and "ccPatchAutoApproveSet" in js and "ccPatchAutoApproveOn" in js
                           and "ccPatchYoloConfirm" in js and "claudeChannelId" in js),
