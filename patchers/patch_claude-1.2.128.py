...
function ccPatchYoloDefaultOn(){try{return localStorage.getItem(`ccPatchYoloDefault`)===`1`}catch(e){return false}}
function ccPatchYoloDefaultSet(on){try{localStorage.setItem(`ccPatchYoloDefault`,on?`1`:`0`)}catch(e){}}
...
// then apply — model right away, YOLO once the chat has a channelId. Opening an
// EXISTING chat (messages already there at first sight) is never force-defaulted.
function ccPatchNewChatDefaultsTick(){try{var s=ccPatchActiveSession&&ccPatchActiveSession();if(!s)return;var sid=ccPatchSessionKey&&ccPatchSessionKey(s);if(!sid)return;var seen=globalThis.ccPatchDefSeen||(globalThis.ccPatchDefSeen={});var msgs=s.messages&&s.messages.value;var fresh=!msgs||msgs.length===0;if(seen[sid]===void 0)seen[sid]={yolo:fresh&&ccPatchYoloDefaultOn(),modelDone:false,yoloDone:false};var st=seen[sid];if(!st.modelDone&&fresh){st.modelDone=true;try{ccPatchApplyDefaultModel(s)}catch(e){}}if(st.yolo&&!st.yoloDone){var ch=s.claudeChannelId;if(ch){try{ccPatchAutoApproveSet(ch,true)}catch(e){}st.yoloDone=true}}}catch(e){}}
function ccPatchCloseDefaultsMenu(){var m=document.querySelector(`.ccPatchDefaultsMenu`);if(m){if(m._ccPatchOutsideHandler)document.removeEventListener(`mousedown`,m._ccPatchOutsideHandler);if(m._ccPatchKey)document.removeEventListener(`keydown`,m._ccPatchKey);m.remove()}}
function ccPatchShowDefaultsMenu(){if(document.querySelector(`.ccPatchDefaultsMenu`)){ccPatchCloseDefaultsMenu();return}try{ccPatchCloseSettingsMenu()}catch(e){}var btn=document.querySelector(`.ccPatchSettingsBtn`);var menu=document.createElement(`div`);menu.className=`ccPatchSettingsMenu ccPatchDefaultsMenu`;menu.style.left=`-9999px`;menu.style.top=`0px`;var head=document.createElement(`div`);head.className=`ccPatchContHead`;head.textContent=`Default for new chats`;menu.appendChild(head);var sub=document.createElement(`div`);sub.className=`ccPatchContSub`;sub.textContent=`What every NEW chat opens on. Chats already going keep their own.`;menu.appendChild(sub);(function(){var d=document.createElement(`div`);d.className=`ccPatchSettingsItem ccPatchDefRow`;var l=document.createElement(`span`);l.className=`ccPatchSettingsItemLabel`;l.textContent=`Model`;d.appendChild(l);var sel=document.createElement(`select`);sel.className=`ccPatchDefSelect`;var models=ccPatchAvailableModels();var o0=document.createElement(`option`);o0.value=``;o0.textContent=`Account default`;sel.appendChild(o0);for(var i=0;i<models.length;i++){var mo=models[i];if(!mo||!mo.value)continue;var op=document.createElement(`option`);op.value=mo.value;op.textContent=mo.displayName||mo.value;sel.appendChild(op)}sel.value=ccPatchDefaultModelGet();sel.onmousedown=function(v){v.stopPropagation()};sel.onchange=function(){ccPatchDefaultModelSet(sel.value)};d.appendChild(sel);d.onmousedown=function(v){v.stopPropagation()};menu.appendChild(d)})();(function(){var d=document.createElement(`div`);d.className=`ccPatchSettingsItem ccPatchEffItem ccPatchDefRow`;var l=document.createElement(`span`);l.className=`ccPatchSettingsItemLabel`;l.textContent=ccPatchDefaultEffortLabel()||`Auto`;d.appendChild(l);d.appendChild(ccPatchBuildEffortSlider(l));d.onmousedown=function(v){v.stopPropagation()};menu.appendChild(d)})();(function(){var d=document.createElement(`div`);d.className=`ccPatchSettingsItem ccPatchContItem`;var col=document.createElement(`div`);col.className=`ccPatchContText`;var l=document.createElement(`div`);l.className=`ccPatchContLabel`;l.textContent=`Start new chats on YOLO mode`;var de=document.createElement(`div`);de.className=`ccPatchContDesc`;de.textContent=`Each new chat auto-approves every prompt. You can still turn it off per chat.`;col.appendChild(l);col.appendChild(de);d.appendChild(col);var lb=document.createElement(`span`);lb.className=`ccPatchYoloToggle`;var cb=document.createElement(`input`);cb.type=`checkbox`;cb.checked=ccPatchYoloDefaultOn();var sl=document.createElement(`span`);sl.className=`ccPatchYoloSlider`;lb.appendChild(cb);lb.appendChild(sl);d.appendChild(lb);d.onmousedown=function(v){v.preventDefault();v.stopPropagation();cb.checked=!cb.checked;ccPatchYoloDefaultSet(cb.checked)};menu.appendChild(d)})();document.body.appendChild(menu);var mw=menu.offsetWidth||280,mh=menu.offsetHeight||220;var left=8,top=80;if(btn){var r=btn.getBoundingClientRect();left=Math.round(r.right)-mw;top=Math.round(r.bottom+4);if(left+mw>window.innerWidth-8)left=window.innerWidth-8-mw;if(left<8)left=8;if(top+mh>window.innerHeight-8)top=Math.max(8,Math.round(r.top)-mh-4)}menu.style.left=left+`px`;menu.style.top=top+`px`;setTimeout(function(){var h=function(g){if(!menu.contains(g.target))ccPatchCloseDefaultsMenu()};var k=function(g){if(g.key===`Escape`)ccPatchCloseDefaultsMenu()};menu._ccPatchOutsideHandler=h;menu._ccPatchKey=k;document.addEventListener(`mousedown`,h);document.addEventListener(`keydown`,k)},0)}
Object.assign(globalThis,{ccPatchTitle,ccPatchImgPreviewShow,ccPatchImgPreviewHide,ccPatchGetSS,ccPatchSetSS,ccPatchIsArchived,ccPatchIsPinned,ccPatchIsStarred,ccPatchToggleArchive,ccPatchTogglePin,ccPatchToggleStar,ccPatchRecency,ccPatchSortSessions,ccPatchSessionId,ccPatchTrackSessionStatus,ccPatchClearDone,ccPatchIsWaiting,ccPatchErrorReason,ccPatchSessionIndicator,ccPatchActivityText,ccPatchCloseMenu,ccPatchShowMenu,ccPatchTogglePane,ccPatchSetSearch,ccPatchToggleSearch,ccPatchStartResize,ccPatchFilterListeners,ccPatchAgeMsMap,ccPatchDefaultFilters,ccPatchReadFilters,ccPatchIsUntitledEmpty,ccPatchWriteFilters,ccPatchFiltersActive,ccPatchSessionMatchesFilters,ccPatchFilterSort,ccPatchCloseFilterMenu,ccPatchFilterIconSVG,ccPatchShowFilterMenu,ccPatchCloseSettingsMenu,ccPatchShowSettingsMenu,ccPatchAutoApproveMap,ccPatchAutoApproveOn,ccPatchAutoApproveSet,ccPatchYoloConfirm,ccPatchYoloConfirmedGet,ccPatchRpc,ccPatchOpenClaudeMd,ccPatchReadClaudeMd,ccPatchWriteClaudeMd,ccPatchInstructionsModal,ccPatchSwitchAccount,ccPatchSwitchAccountModal,ccPatchSendMenu,ccPatchCloseSendMenu,ccPatchTriggerSend,ccPatchQueueForm,ccPatchStopAndSendForm,ccPatchComposerForm,ccPatchComposerAction,ccPatchRenderQueue,ccPatchSetSendMode,ccPatchQueueAdd,ccPatchQueueRemove,ccPatchQueueEdit,ccPatchQueueCommitEdit,ccPatchQueueCancelEdit,ccPatchQueueSendNow,ccPatchQueueTick,ccPatchActiveSession,ccPatchSessionKey,ccPatchIsBusy,ccPatchQueueAddFull,ccPatchThemeModal,ccPatchApplyTheme,ccPatchSyncWsTheme,ccPatchWsTheme,ccPatchColorPalette,ccPatchColorHex,ccPatchGetColor,ccPatchSetColor,ccPatchShowColorMenu,ccPatchCloseColorMenu,ccPatchStickyOn,ccPatchStickyToggle,ccPatchUsageConn,ccPatchUsageData,ccPatchUsageRefresh,ccPatchUsageColor,ccPatchUsageResetText,ccPatchUsageEsc,ccPatchUsageRing,ccPatchUsageBtnIcon,ccPatchUsageWindows,ccPatchUsageRenderBody,ccPatchCloseUsageMenu,ccPatchShowUsageMenu,ccPatchUsageStartPolling,ccPatchContOn,ccPatchContSet,ccPatchContAnyOn,ccPatchContTick,ccPatchShowContinuationMenu,ccPatchCloseContMenu,ccPatchEffortStops,ccPatchEffortCwd,ccPatchEffortKey,ccPatchEffortSyncTick,ccPatchDefaultEffort,ccPatchDefaultEffortRaw,ccPatchDefaultUltracode,ccPatchSetDefaultEffort,ccPatchEffortIndex,ccPatchDefaultEffortLabel,ccPatchBuildEffortSlider,ccPatchDefaultModelGet,ccPatchDefaultModelSet,ccPatchYoloDefaultOn,ccPatchYoloDefaultSet,ccPatchModelCfg,ccPatchAvailableModels,ccPatchDefaultModelLabel,ccPatchApplyDefaultModel,ccPatchNewChatDefaultsTick,ccPatchShowDefaultsMenu,ccPatchCloseDefaultsMenu,ccPatchMsgText,ccPatchAddCopyBtn,ccPatchCopySweep,ccPatchTaskNoteSweep,ccPatchCopyDiagnostics,ccPatchBuildDiag,ccPatchDiagRpc,ccPatchToast,ccPatchClip});try{globalThis.ccOrbitCopyDiagnostics=function(){return ccPatchCopyDiagnostics(globalThis.ccPatchCtx)}}catch(ccdg){}
...
def patch_webview_css(webview_css: Path) -> bool:
    ...
    new = old + modal_overlay_rule + (
        ...
        # "Default" popup (new-chat defaults: model / effort / YOLO)
        ".ccPatchDefaultsMenu{min-width:282px;max-width:330px;padding:8px}"
        ".ccPatchDefRow{align-items:center;gap:8px}"
        ".ccPatchDefSelect{margin-left:auto;max-width:166px;font-size:12px;"
        "background:var(--app-secondary-background,#2a2a2a);color:var(--app-foreground,#e6e6e6);"
        "border:1px solid var(--app-border-color,rgba(255,255,255,.16));border-radius:6px;"
...
def verify_extension_dir(extension_dir: Path) -> None:
    ...
    checks = {
        ...
        "defaults popup": ("ccPatchShowDefaultsMenu" in js and "ccPatchDefaultModelGet" in js
                           and "ccPatchYoloDefaultOn" in js and "ccPatchNewChatDefaultsTick" in js
                           and "ccPatchAvailableModels" in js
                           and ".ccPatchDefaultsMenu" in css and ".ccPatchDefSelect" in css),
