...
def patch_webview_js(webview_js: Path) -> bool:
    ...
    # Fork wrapper fix: keep the real stock fork path, but make the context
    # wrapper return the forked session id. If same-pane activation cannot see
    # the fresh session yet, open a real editor tab pointed at the fork id.
    fork_wrapper_re = re.compile(
        rf'async forkConversation\((?P<a>{JS_ID}),(?P<prompt>{JS_ID}),(?P<resume>{JS_ID})\)\{{'
        rf'let (?P<conn>{JS_ID})=this\.comms\.connection\.value;'
        rf'if\((?P=conn)\)\{{'
        rf'let (?P<openTab>{JS_ID})=(?P=conn)\.config\.value\?\.openNewInTab,'
        rf'(?P<forked>{JS_ID})=await (?P=conn)\.forkConversation\((?P=a),(?P=resume)\);'
        rf'if\((?P=openTab)\)this\.startNewConversationTab\((?P=prompt),(?P=forked)\);'
        rf'else this\.viewSession\((?P=forked),(?P=prompt)\)'
        rf'\}}return""\}}'
    )
    m_fw = fork_wrapper_re.search(text)
    if m_fw is None:
        log("Note: fork context-wrapper anchor not found; skipping fork wrapper fix")
    else:
        a = m_fw.group("a")
        prompt = m_fw.group("prompt")
        resume = m_fw.group("resume")
        conn = m_fw.group("conn")
        open_tab = m_fw.group("openTab")
        forked = m_fw.group("forked")
        opened = "ccPatchForkOpened"
        new_fw = (
            f"async forkConversation({a},{prompt},{resume}){{"
            f"let {conn}=this.comms.connection.value;"
            f"if(!{conn})throw Error(\"Not connected\");"
            f"let {open_tab}={conn}.config.value?.openNewInTab,"
            f"{forked}=await {conn}.forkConversation({a},{resume});"
            f"if(!{forked})throw Error(\"No forked session id returned\");"
            f"if({open_tab}){{this.startNewConversationTab({prompt},{forked});return {forked}}}"
            f"let {opened}=await this.viewSession({forked},{prompt});"
            f"if(!{opened}){{{conn}.startNewConversationTab({prompt},{forked});return {forked}}}"
            f"return {forked}}}"
        )
        text = text[: m_fw.start()] + new_fw + text[m_fw.end() :]
        log("Fork context-wrapper patched - opens fork id directly when same-pane activation misses")

    # Note: there was previously no fork fix here.  The stock $1() handler
...
def verify_extension_dir(extension_dir: Path) -> None:
    ...
    checks = {
        ...
        "fork wrapper returns id": "No forked session id returned" in js and "ccPatchForkOpened=await this.viewSession" in js and "startNewConversationTab" in js[js.find("ccPatchForkOpened") : js.find("ccPatchForkOpened") + 260],
