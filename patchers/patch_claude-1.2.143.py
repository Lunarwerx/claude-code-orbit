...
function ccPatchSessionKey(s){try{return(s&&s.sessionId&&s.sessionId.value)||(s&&s.internalId)||null}catch(e){return null}}
...
function ccPatchDedupeSessions(e){if(!e)return[];if(e.length<2)return[...e];var out=[],seen=Object.create(null);for(var i=0;i<e.length;i++){var s=e[i];if(!s)continue;var k=ccPatchSessionKey(s);if(k==null){out.push(s);continue}var at=seen[k];if(at===void 0){seen[k]=out.length;out.push(s)}else{try{if(ccPatchDupeBetter(s,out[at]))out[at]=s}catch(err){}}}return out}
...
def verify_extension_dir(extension_dir: Path) -> None:
    ...
    checks = {
        ...
        "session dedup": ("function ccPatchDedupeSessions(" in js
                          and "function ccPatchSessionKey(" in js
