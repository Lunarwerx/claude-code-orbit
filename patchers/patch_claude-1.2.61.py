...
def patch_webview_js(webview_js: Path) -> bool:
    ...
    # wrapper return the forked session id and throw if it cannot open it.
    ...
    if m_fw is None:
        ...
        new_fw = (
            ...
            f"if(!{conn})throw Error(\"Failed to fork conversation: Not connected\");"
            f"let {open_tab}={conn}.config.value?.openNewInTab,"
            f"{forked}=await {conn}.forkConversation({a},{resume});"
            f"if(!{forked})throw Error(\"Failed to fork conversation: No forked session id returned\");"
            f"if({open_tab}){{this.startNewConversationTab({prompt},{forked});return {forked}}}"
            f"let {opened}=await this.viewSession({forked},{prompt});"
            f"if(!{opened})throw Error(\"Failed to fork conversation: Forked session could not be opened\");"
            f"return {forked}}}"
        )
        text = text[: m_fw.start()] + new_fw + text[m_fw.end() :]
        log("Fork context-wrapper patched - returns forked session id and verifies open")
...
def verify_extension_dir(extension_dir: Path) -> None:
    ...
    checks = {
        ...
        "fork wrapper returns id": "No forked session id returned" in js and "return\"\"" not in js[js.find("async forkConversation") : js.find("async forkConversation") + 500],
