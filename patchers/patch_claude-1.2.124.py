...
def patch_webview_js(webview_js: Path) -> bool:
    ...
    # ──────────────────────────────────────────────────────────────────────
    # (removed) Orbit used to relabel the native "Bypass permissions" mode to
    # "YOLO mode" and force it into the mode selector below. Removed — that native
    # mode is broken in the VS Code extension (prompts fire regardless of mode), so
    # the label is left native and the mode is no longer force-injected.
    ...
    # props so the in-popup effort row disappears (it is standalone now). Native
    # availableModes + native onSelect — the old Orbit "YOLO mode" force-injection
    # is gone, so the permission modes shown here are exactly the native set.
...
def verify_extension_dir(extension_dir: Path) -> None:
    ...
    checks = {
        ...
        "yolo auto-approve toggle": ("ccPatchAutoApproveOn" in js and "ccPatchAutoApprovePush" in js
                                     and "orbit_set_yolo" in js and "YOLO mode" in js
