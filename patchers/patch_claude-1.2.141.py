...
def patch_webview_css(webview_css: Path) -> bool:
    ...
    new = old + modal_overlay_rule + (
        ...
        "background:transparent;color:var(--app-primary-foreground);opacity:0;cursor:pointer;transition:opacity .12s ease}"
        ".ccPatchAcctRow:hover .ccPatchAcctRemove{opacity:.5}"
        ".ccPatchAcctRemove:hover{opacity:1;background:rgba(229,72,77,.18);color:#e5484d}"
...
let ccIdOf=function(c){try{return(c&&c.organizationUuid)||(c&&c.claudeAiOauth&&c.claudeAiOauth.accessToken?"acct-"+String(c.claudeAiOauth.accessToken).slice(-10):null)}catch(e){return null}};
...
let ccEnsure=async function(){let c=ccRJ(ccCredP);if(!c)return null;let id=ccIdOf(c);if(!id)return null;let ex=ccRJ(ccMP(id))||{};ccWJ(ccCP(id),c);let email=ex.email||"",name=ex.name||"";if(!email){let p=await ccProfile(ccTok(c));if(p){email=p.email||"";name=p.name||""}}let meta={id:id,label:ex.label||email||id,email:email,name:name,subscriptionType:(c.claudeAiOauth&&c.claudeAiOauth.subscriptionType)||ex.subscriptionType||"",usage:ex.usage||null,usageAt:ex.usageAt||0,savedAt:Date.now()};ccWJ(ccMP(id),meta);return meta};
try{
if(ccAct==="list"){await ccEnsure();let ms=ccMetas(),act=ccActive();ms.forEach(function(m){m.active=(m.id===act)});return{type:"orbit_accounts_response",ok:!0,accounts:ms,activeId:act};}
