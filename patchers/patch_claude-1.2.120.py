...
def patch_webview_js(webview_js: Path) -> bool:
    ...
    # New Session onClick — ALWAYS create + activate a fresh session in-pane.
    # We deliberately do NOT take the native `startNewConversationTab()` path the
    # stock button uses (`if(!Z.startNewConversationTab())S.createSession()`):
    ...
    # the user stays trapped on the current session ("I click new tab, it sends
    # me right back"). `createSession()` prepends the new session AND sets
    # `activeSession.value=t`, switching the pane reliably. We re-assert
    # activeSession once the promise resolves as belt-and-suspenders against the
    # documented un-awaited-navigation churn (see the fork mid-stream fix).
    new_session_onclick = (
        f'()=>{{try{{Promise.resolve({fe1_S}.createSession()).then('
...
def verify_extension_dir(extension_dir: Path) -> None:
    ...
    checks = {
        ...
        # editor-tab path that no-ops in Orbit's single-pane webview. Our marker is
        # the post-create re-assert.
