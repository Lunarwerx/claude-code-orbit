...
// A real API/transient error (e.g. "API Error: 500 Internal server error. This
// is a server-side issue ... try again ... status.claude.com") surfaces as
// RENDERED TEXT, not a mirror_error message — so detect that distinctive
// signature. Distinctive enough (the Claude Code error template) that it won't
// fire on normal assistant output.
...
// A "Default model" picker in the Settings gear. Whatever is picked here is the
// model every NEW chat opens on; a chat already in progress keeps its own model.
// Stored as the model id in localStorage and applied at session construction via
// the modelSelection field initializer (patcher block 16b), so a brand-new chat
// both shows and runs on it. No pick => "" => native account default. The model
// list comes from the live session's claudeConfig so it always matches the real
// bottom-bar picker. ccPatchDefaultModel MUST be exported (Object.assign) because
// the field initializer references it bare from class-construction scope.
function ccPatchDefaultModel(){try{return localStorage.getItem(`ccPatchDefaultModel`)||``}catch(e){return``}}
function ccPatchSetDefaultModel(id){try{if(id)localStorage.setItem(`ccPatchDefaultModel`,String(id));else localStorage.removeItem(`ccPatchDefaultModel`)}catch(e){}}
function ccPatchModelList(){try{var s=ccPatchActiveSession();var m=s&&s.claudeConfig&&s.claudeConfig.value&&s.claudeConfig.value.models;if(m&&m.length)return m}catch(e){}try{var R=globalThis.ccPatchSessionsRoot;var s2=R&&R.activeSession&&R.activeSession.value;var m2=s2&&s2.claudeConfig&&s2.claudeConfig.value&&s2.claudeConfig.value.models;if(m2&&m2.length)return m2}catch(e){}return[]}
function ccPatchDefaultModelLabel(){try{var id=ccPatchDefaultModel();if(!id)return null;var ms=ccPatchModelList();for(var i=0;i<ms.length;i++){if(ms[i]&&ms[i].value===id)return ms[i].displayName||ms[i].value}return id}catch(e){return null}}
function ccPatchCloseDefModelMenu(){var m=document.querySelector(`.ccPatchDefModelMenu`);if(m){if(m._ccPatchOutsideHandler)document.removeEventListener(`mousedown`,m._ccPatchOutsideHandler);if(m._ccPatchKey)document.removeEventListener(`keydown`,m._ccPatchKey);m.remove()}}
function ccPatchShowDefaultModelMenu(){if(document.querySelector(`.ccPatchDefModelMenu`)){ccPatchCloseDefModelMenu();return}try{ccPatchCloseSettingsMenu()}catch(e){}var btn=document.querySelector(`.ccPatchSettingsBtn`);var menu=document.createElement(`div`);menu.className=`ccPatchSettingsMenu ccPatchContMenu ccPatchDefModelMenu`;menu.style.left=`-9999px`;menu.style.top=`0px`;var head=document.createElement(`div`);head.className=`ccPatchContHead`;head.textContent=`Default model`;menu.appendChild(head);var sub=document.createElement(`div`);sub.className=`ccPatchContSub`;sub.textContent=`Every new chat opens on this model. Chats you're already in keep theirs.`;menu.appendChild(sub);var cur=ccPatchDefaultModel();var models=ccPatchModelList();function row(id,label,desc){var d=document.createElement(`div`);d.className=`ccPatchSettingsItem ccPatchContItem ccPatchDefModelItem`;if((cur||``)===(id||``))d.classList.add(`ccPatchDefModelOn`);var ck=document.createElement(`span`);ck.className=`ccPatchDefModelCheck`;ck.innerHTML=`<svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="3" stroke-linecap="round" stroke-linejoin="round"><path d="M5 13l4 4L19 7"/></svg>`;d.appendChild(ck);var col=document.createElement(`div`);col.className=`ccPatchContText`;var l=document.createElement(`div`);l.className=`ccPatchContLabel`;l.textContent=label;col.appendChild(l);if(desc){var de=document.createElement(`div`);de.className=`ccPatchContDesc`;de.textContent=desc;col.appendChild(de)}d.appendChild(col);d.onmousedown=function(v){v.preventDefault();v.stopPropagation();ccPatchSetDefaultModel(id);ccPatchCloseDefModelMenu()};menu.appendChild(d)}row(``,`Account default`,`Whatever Claude Code would normally start with.`);if(!models.length){var w=document.createElement(`div`);w.className=`ccPatchContSub`;w.style.padding=`2px 8px 4px`;w.textContent=`Open a chat once so Orbit can load your models, then reopen this.`;menu.appendChild(w)}else{models.forEach(function(m){if(!m||!m.value)return;row(m.value,m.displayName||m.value,null)})}document.body.appendChild(menu);var mw=menu.offsetWidth||252,mh=menu.offsetHeight||210;var left=8,top=80;if(btn){var r=btn.getBoundingClientRect();left=Math.round(r.right)-mw;top=Math.round(r.bottom+4);if(left+mw>window.innerWidth-8)left=window.innerWidth-8-mw;if(left<8)left=8;if(top+mh>window.innerHeight-8)top=Math.max(8,Math.round(r.top)-mh-4)}menu.style.left=left+`px`;menu.style.top=top+`px`;setTimeout(function(){var h=function(g){if(!menu.contains(g.target))ccPatchCloseDefModelMenu()};var k=function(g){if(g.key===`Escape`)ccPatchCloseDefModelMenu()};menu._ccPatchOutsideHandler=h;menu._ccPatchKey=k;document.addEventListener(`mousedown`,h);document.addEventListener(`keydown`,k)},0)}
...
def patch_webview_js(webview_js: Path) -> bool:
    ...
    # ──────────────────────────────────────────────────────────────────────
    # 16b. Default model — new chats open on the model chosen in Settings ▸
    #      "Default model". Mirrors the YOLO seam: the session-class field
    #      initializer `modelSelection=at(void 0)` becomes
    #      `modelSelection=at(ccPatchDefaultModel()||void 0)`, so a freshly
    #      constructed session shows AND runs on the saved default immediately.
    #      No pick (empty string) falls through to `void 0`, which the native
    #      connect path then fills from the account default — unchanged behavior.
    #      A chat the user is already in has a non-empty modelSelection, so it is
    #      never touched. Anchored on the modelSelection/currentMainLoopModel
    #      field pair (unique to the session class; robust to new fields being
    #      inserted before it, e.g. hasPersistedTitle in CC 2.1.179) and guarded
    #      with `typeof`
    #      so a missing helper can never break session construction.
    # ──────────────────────────────────────────────────────────────────────
    m_defmodel = re.search(
        rf"modelSelection=(?P<at>{JS_ID})\(void 0\);"
        rf"currentMainLoopModel=(?P=at)\(void 0\)",
        text,
    )
    if m_defmodel is None:
        log("Note: modelSelection init anchor not found; skipping default-model patch")
    else:
        dm_at = m_defmodel.group("at")
        dm_anchor = f"modelSelection={dm_at}(void 0);currentMainLoopModel={dm_at}(void 0)"
        dm_replace = (
            f'modelSelection={dm_at}((typeof ccPatchDefaultModel==="function"'
            f'?ccPatchDefaultModel():"")||void 0);currentMainLoopModel={dm_at}(void 0)'
        )
        if dm_anchor in text and dm_replace not in text:
            text = text.replace(dm_anchor, dm_replace, 1)
            log("Default model: new chats honor the Settings > Default model pick")
...
def patch_webview_css(webview_css: Path) -> bool:
    ...
    new = old + modal_overlay_rule + (
        ...
        # Settings-row right-hand value (e.g. Auto-continue "On", Default model name)
        ".ccPatchSettingsItemExtra{flex:0 0 auto;font-size:11px;opacity:.6;margin-left:6px;"
        "max-width:120px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}"
        # Default-model picker (reuses the Auto-continue menu chrome) -- a single-
        # select list with a leading check on the active row.
        ".ccPatchDefModelMenu{min-width:230px;max-width:300px;max-height:60vh;overflow-y:auto}"
        ".ccPatchDefModelItem{align-items:center;cursor:pointer}"
        ".ccPatchDefModelCheck{flex:0 0 auto;width:14px;height:14px;display:flex;"
        "align-items:center;justify-content:center;opacity:0;color:var(--app-primary-foreground)}"
        ".ccPatchDefModelOn .ccPatchDefModelCheck{opacity:1}"
        ".ccPatchDefModelOn .ccPatchContLabel{font-weight:600}"
...
def verify_extension_dir(extension_dir: Path) -> None:
    ...
    checks = {
        ...
        # Default model: the gear-menu picker + its exported getter (the `,` form
        # appears ONLY in the Object.assign export, so it asserts the export the
        # field initializer depends on) + the construction-time field patch.
        "default model": ("ccPatchShowDefaultModelMenu" in js and "ccPatchDefaultModel," in js
                          and ".ccPatchDefModelMenu" in css
                          and 'typeof ccPatchDefaultModel==="function"?ccPatchDefaultModel()' in js),
