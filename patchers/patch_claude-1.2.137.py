...
function ccPatchTrackSessionStatus(e,t){let n=ccPatchSessionId(e),r=!!e.busy?.value,a=ccPatchBusyBySession.get(n);if(r)ccPatchTouch(e);a===void 0?ccPatchBusyBySession.set(n,r):(a&&!r&&(ccPatchDoneSessions.add(n),setTimeout(t,0)),ccPatchBusyBySession.set(n,r))}
...
function ccPatchSessionIndicator(e,t){if(!t&&ccPatchIsWaiting(e))return`waiting`;if(e.busy?.value)return`running`;if(ccPatchErrorReason(e))return`error`;if(ccPatchDoneSessions.has(ccPatchSessionId(e)))return`done`;return``}
function ccPatchActivityText(e){if(e.permissionRequests?.value?.length)return`Waiting for input`;if(!e.busy?.value){let ccEr=ccPatchErrorReason(e);return ccEr||null}let m=e.messages?.value;if(m&&m.length>0){let l=m[m.length-1];if(l){let c=l.content;if(Array.isArray(c)){for(let j=c.length-1;j>=0;j--){let b=c[j];if(!b)continue;if(b.type===`tool_use`){let n=b.name||``,inp=b.input;if(inp&&inp.command)return`Running `+String(inp.command).replace(/\s+/g,` `).slice(0,50);if(inp&&inp.file_path)return(n===`Read`?`Reading `:`Editing `)+String(inp.file_path).split(/[/\\]/).pop();return`Running `+n.replace(/_/g,` `)+`...`}if(b.type===`thinking`)return`Thinking...`;if(b.type===`text`&&b.text)return String(b.text).replace(/\s+/g,` `).slice(0,50).trim()}}}}return`Thinking...`}
...
ORBIT_WHATSNEW: str = "**YOLO mode: when it's on, it really means it.**\n\nYOLO would sometimes still stop and ask for permission even with the mode switched on for a chat \u2014 the auto-approve depended on matching each permission request back to your chat by a lookup that could quietly miss, and a miss meant a prompt slipped through.\n\nNow the match is recorded at the source, the instant a turn starts, so every permission request in a YOLO chat is approved without asking \u2014 file edits, terminal commands, everything. Turn it on once for a chat and it stops interrupting you. Other chats keep asking like normal."
