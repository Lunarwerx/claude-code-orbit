...
Features:
  - Archive sessions: right-click or button replaces delete
    ...
    * Right-click menu: Rename / Pin / Star / Archive (or Unarchive)
...
Direct CLI runs keep generated artifacts in the repo-level builds/ folder,
beside BUILD_LOG.md and the wrapper VSIX artifacts.
"""
...
DEFAULT_ARTIFACTS_DIR = REPO_ROOT / "builds"
LOG_PATH: Path | None = None

# ── Patch-module gating ──────────────────────────────────────────────────────
# None = apply everything (default; byte-identical to the historical monolith).
# A set = the gateable feature ids that are ON; any gateable id NOT in the set is
# skipped. The core scaffold and the welded sidebar cluster are never gateable —
# they always apply. Set by main() from --disable/--enable; read by the gated
# blocks via feature_on() and by verify_extension_dir().
ENABLED_FEATURES: "set[str] | None" = None

# Independent, purely-additive feature modules that --disable/--enable can toggle.
# Mirrors the independent userFacing entries in patch_modules/catalog.json. Only
# ids listed here are skippable; everything else is always applied.
GATEABLE_FEATURES = {
    "usage-meter",
    "fork-row",
}

# CC 2.1.186 rebundled the webview to React's automatic JSX runtime. The core
# session pane has been fully re-ported to the new shape; the composer
# bottom-bar reorder + send-split + queued-message bubbles + the fork action row
# are larger jsx re-ports that land in a follow-up beta. Until then they are
# deferred so the core ships working on 2.1.186 (Claude's native composer / fork
# menu / show-more stay in place meanwhile). Flip to True once blocks 17, 18b,
# 19, 20 and 21 are re-derived for the jsx-runtime bundle.
COMPOSER_REWORK_2186 = True
...
function cfg(){
  ...
  const minAge=Math.max(5,Number(c?c.get("reaper.minAgeMinutes",6):6)||6);
  const interval=Math.max(1,Number(c?c.get("reaper.intervalMinutes",3):3)||3);
  const trimResumeEnabled=c?c.get("reaper.trimResumeSessions.enabled",false):false;
  const maxResumeSessions=Math.max(1,Number(c?c.get("reaper.trimResumeSessions.maxSessions",3):3)||3);
  const resumeMinAgeMinutes=Math.max(5,Number(c?c.get("reaper.trimResumeSessions.minAgeMinutes",15):15)||15);
...
$sampleSeconds = 2
if (-not (Get-Process -Name claude -ErrorAction SilentlyContinue)) { exit }
$processes = @(Get-CimInstance Win32_Process -Property ProcessId,ParentProcessId,Name,ExecutablePath,CommandLine,CreationDate)
$byId = @{}
...
def feature_on(fid: str) -> bool:
    ...
    only ids in GATEABLE_FEATURES can be switched off via ENABLED_FEATURES."""
...
function ccPatchColorPalette(){return[["","Default",""],["red","Tomato","#e5484d"],["orange","Tangerine","#f76b15"],["yellow","Banana","#f5c518"],["green","Basil","#30a46c"],["blue","Peacock","#0091ff"],["purple","Grape","#8e4ec6"]]}
function ccPatchColorHex(k){var p=ccPatchColorPalette();for(var i=0;i<p.length;i++){if(p[i][0]===k)return p[i][2]}return``}
...
function ccPatchRecency(e){try{var t=+ccPatchGetSS(e).touchedAt||0,m=(e.lastModifiedTime&&e.lastModifiedTime.value)||0;return t>m?t:m}catch(err){return(e&&e.lastModifiedTime&&e.lastModifiedTime.value)||0}}
function ccPatchTouch(e){try{if(e)ccPatchSetSS(e,{touchedAt:Date.now()})}catch(err){}}
function ccPatchSortSessions(e,t){let ae=ccPatchIsArchived(e),at=ccPatchIsArchived(t);if(ae!==at)return ae?1:-1;if(!ae){let se=ccPatchIsStarred(e),st=ccPatchIsStarred(t);if(se!==st)return se?-1:1;let pe=ccPatchIsPinned(e),pt=ccPatchIsPinned(t);if(pe!==pt)return pe?-1:1}return ccPatchRecency(t)-ccPatchRecency(e)}
...
function ccPatchTrackSessionStatus(e,t){let n=ccPatchSessionId(e),r=ccPatchRunState(e).running,a=ccPatchBusyBySession.get(n);if(r)ccPatchTouch(e);a===void 0?ccPatchBusyBySession.set(n,r):(a&&!r&&(ccPatchDoneSessions.add(n),setTimeout(t,0)),ccPatchBusyBySession.set(n,r))}
...
// Red-dot error state. Claude's webview does NOT persist a generic per-turn API
// error (the message object has no error field), but it DOES expose a reactive
// rateLimitWarning signal on the session — set to a human string only when the
// account is limited, cleared on the next "allowed" event. We red-dot only the
// HARD stop ("You've hit your … limit"), not the soft "Approaching/used X%"
// warnings, so the dot is true when it's red and never cries wolf. Reading
// .value here subscribes the row, so the dot flips live like busy/waiting do.
function ccPatchErrorReason(e){try{let r=e.rateLimitWarning?.value;if(typeof r===`string`&&/hit your|exceeded|exhausted/i.test(r))return r}catch(err){}return null}
// ── Live run-state from the host broadcast (fixes "sidebar says the chat is dead
//    while it's visibly still working") ──────────────────────────────────────
// The sidebar rows come from listSessions() — a LIVE disk readdir — but each row
// object's reactive `busy` signal only tracks the ATTACHED (streaming) session.
// A chat working in the BACKGROUND (and, because the row object is a different
// instance than the composer's live session, even the one on screen) therefore
// reads busy=false and the row looks dead/timed-out. The connection carries
// `sessionStates`, pushed by the host via `session_states_update` — the AUTHORITATIVE
// per-session run state for EVERY session regardless of which is attached. Each
// entry is {sessionId,state,title}; state!=="idle" means busy (Claude's own
// Cf.fromStateInfo sets busy=state!=="idle") and "waiting_input" means it needs a
// permission answer. Reading .value inside the row render subscribes the row, so it
// flips live the moment the host rebroadcasts — exactly like busy/waiting already do.
function ccPatchConn(){try{var c=ccPatchUsageConn();if(c)return c;var ss=ccPatchContSessions();for(var i=0;i<ss.length;i++){var cc=ss[i]&&ss[i].connection&&ss[i].connection.value;if(cc)return cc}}catch(e){}return null}
function ccPatchLiveState(e){try{var sid=e&&e.sessionId&&e.sessionId.value;if(!sid)return null;var c=ccPatchConn();var arr=c&&c.sessionStates&&c.sessionStates.value;if(!arr)return null;for(var i=0;i<arr.length;i++){if(arr[i]&&arr[i].sessionId===sid)return arr[i].state||null}}catch(err){}return null}
// Unified truth: local busy signal OR the host broadcast. Either is enough to call
// a chat "running", so a stale local signal can never mislabel a live chat as dead.
function ccPatchRunState(e){var st=null;try{st=ccPatchLiveState(e)}catch(_){}var b=false;try{b=!!(e.busy&&e.busy.value)}catch(_){}var w=false;try{w=ccPatchIsWaiting(e)}catch(_){}return{running:b||!!(st&&st!==`idle`),waiting:w||st===`waiting_input`,state:st}}
function ccPatchSessionIndicator(e,t){var rs=ccPatchRunState(e);if(!t&&rs.waiting)return`waiting`;if(rs.running)return`running`;if(ccPatchErrorReason(e))return`error`;if(ccPatchDoneSessions.has(ccPatchSessionId(e)))return`done`;return``}
function ccPatchActivityText(e){var rs=ccPatchRunState(e);if(rs.waiting)return`Waiting for input`;if(!rs.running){let ccEr=ccPatchErrorReason(e);return ccEr||null}let m=e.messages?.value;if(m&&m.length>0){let l=m[m.length-1];if(l){let c=l.content;if(Array.isArray(c)){for(let j=c.length-1;j>=0;j--){let b=c[j];if(!b)continue;if(b.type===`tool_use`){let n=b.name||``,inp=b.input;if(inp&&inp.command)return`Running `+String(inp.command).replace(/\s+/g,` `).slice(0,50);if(inp&&inp.file_path)return(n===`Read`?`Reading `:`Editing `)+String(inp.file_path).split(/[/\\]/).pop();return`Running `+n.replace(/_/g,` `)+`...`}if(b.type===`thinking`)return`Thinking...`;if(b.type===`text`&&b.text)return String(b.text).replace(/\s+/g,` `).slice(0,50).trim()}}}}return`Thinking...`}
function ccPatchCloseMenu(){document.querySelector(`.claudePatchContextMenu`)?.remove()}
function ccPatchShowMenu(e,t,pLabel,n,sLabel,r,a,sess,rn){ccPatchCloseMenu();let i=document.createElement(`div`);i.className=`claudePatchContextMenu`,i.style.left=`${Math.min(e,window.innerWidth-160)}px`,i.style.top=`${Math.min(t,window.innerHeight-180)}px`;let s=(o,c)=>{let u=document.createElement(`button`),f=!1,d=(v)=>{v.preventDefault(),v.stopPropagation();if(f)return;f=!0,ccPatchCloseMenu(),c()};return u.textContent=o,u.onmousedown=d,u.onclick=d,i.appendChild(u),u};if(rn)s(`Rename`,rn);s(pLabel,n);s(sLabel,r);if(a)s(a.label,a.fn);if(sess){var cw=document.createElement(`div`);cw.className=`ccPatchCtxColors`;var cur=ccPatchGetColor(sess);ccPatchColorPalette().forEach(function(p){var b=document.createElement(`button`);b.type=`button`;b.className=`ccPatchColorSwatch`+(p[0]===cur?` ccPatchColorSwatchOn`:``)+(p[0]===``?` ccPatchColorSwatchNone`:``);b.title=p[1];if(p[2])b.style.background=p[2];b.onmousedown=function(v){v.preventDefault();v.stopPropagation();ccPatchSetColor(sess,p[0]);ccPatchCloseMenu()};cw.appendChild(b)});i.appendChild(cw)}document.body.appendChild(i);setTimeout(()=>document.addEventListener(`mousedown`,ccPatchCloseMenu,{once:!0}),0)}
function ccPatchCloseColorMenu(){document.querySelector(`.ccPatchColorMenu`)?.remove()}
function ccPatchShowColorMenu(x,y,sess){ccPatchCloseColorMenu();ccPatchCloseMenu();var cur=ccPatchGetColor(sess);var m=document.createElement(`div`);m.className=`ccPatchColorMenu`;ccPatchColorPalette().forEach(function(p){var b=document.createElement(`button`);b.type=`button`;b.className=`ccPatchColorSwatch`+(p[0]===cur?` ccPatchColorSwatchOn`:``)+(p[0]===``?` ccPatchColorSwatchNone`:``);b.title=p[1];if(p[2])b.style.background=p[2];b.onmousedown=function(v){v.preventDefault();v.stopPropagation();ccPatchSetColor(sess,p[0]);ccPatchCloseColorMenu()};m.appendChild(b)});document.body.appendChild(m);var mw=m.offsetWidth||0,mh=m.offsetHeight||0,L=Math.min(x,window.innerWidth-mw-8),T=Math.min(y,window.innerHeight-mh-8);m.style.left=`${Math.max(8,L)}px`;m.style.top=`${Math.max(8,T)}px`;setTimeout(function(){document.addEventListener(`mousedown`,ccPatchCloseColorMenu,{once:!0})},0)}
...
try{if(localStorage.getItem(`ccPatchStickyPreview`)===`1`)document.documentElement.classList.add(`ccPatchStickyPreview`)}catch(e){}document.addEventListener(`scroll`,ccPatchStickyScheduleSweep,!0);
var ccPatchSearchQ=``;
...
// ── De-duplicate the session list by STABLE identity ────────────────────────
// Claude's host `S.sessions.value` can transiently hold the SAME conversation
// as several in-memory objects: the attached/active session gets re-added on
// each turn (so the list visibly "grows every time you send"), and a Developer
// Reload silently fixes it by rebuilding the array from disk. Orbit used to
// render that raw array, so ONE chat showed up as N rows — and because those
// duplicate rows collided on their React key, clicking one row drove a
// different session ("click one, another starts going; the one you clicked
// stops"). We collapse by stable id BEFORE filter/sort. Key = sessionId.value
// (the durable server id, == the on-disk .jsonl filename) primary, internalId
// as fallback. Two DIFFERENT conversations can NEVER share a sessionId, so this
// can never merge distinct chats (we deliberately never key by title). A truly
// id-less brand-new session (no sessionId AND no internalId) is kept as-is.
// When the same id appears twice we keep the RUNNING instance so a live chat is
// never dropped for a stale copy. Mirrors the account-switcher ccDedupe (:~4188)
// that cured the identical "same account listed twice" bug.
function ccPatchDedupeKey(s){try{return(s&&s.sessionId&&s.sessionId.value)||(s&&s.internalId)||null}catch(e){return null}}
// When the same conversation appears twice, which instance do we keep? Prefer the
// one the user is actually attached to (=== activeSession, so its isActive
// highlight — object-identity compared — never flickers onto a detached copy),
// then any RUNNING instance (so a live-streaming chat is never dropped for a
// stale disk snapshot), then the most recently touched. Returns true if `a` is
// the better representative than `b`.
function ccPatchDupeBetter(a,b){var R=null;try{R=globalThis.ccPatchSessionsRoot}catch(e){}var av=!1,bv=!1;try{if(R&&R.activeSession){av=R.activeSession.value===a;bv=R.activeSession.value===b}}catch(e){}if(av!==bv)return av;var ar=!1,br=!1;try{ar=ccPatchRunState(a).running}catch(e){}try{br=ccPatchRunState(b).running}catch(e){}if(ar!==br)return ar;var arc=0,brc=0;try{arc=ccPatchRecency(a)||0}catch(e){}try{brc=ccPatchRecency(b)||0}catch(e){}return arc>brc}
function ccPatchDedupeSessions(e){if(!e)return[];if(e.length<2)return[...e];var out=[],seen=Object.create(null);for(var i=0;i<e.length;i++){var s=e[i];if(!s)continue;var k=ccPatchDedupeKey(s);if(k==null){out.push(s);continue}var at=seen[k];if(at===void 0){seen[k]=out.length;out.push(s)}else{try{if(ccPatchDupeBetter(s,out[at]))out[at]=s}catch(err){}}}return out}
function ccPatchFilterSort(e){e=ccPatchDedupeSessions(e);let t=ccPatchReadFilters(),q=ccPatchSearchQ.trim().toLowerCase(),n=t.types.length||t.ages.length?e.filter((r)=>ccPatchSessionMatchesFilters(r,t)):[...e];if(t.hideUntitled)n=n.filter((r)=>!ccPatchIsUntitledEmpty(r));if(q)n=n.filter((r)=>ccPatchTitle(r).toLowerCase().includes(q));return n.sort(ccPatchSortSessions)}
...
function ccPatchShowSettingsMenu(e,ctx,s){try{globalThis.ccPatchCtx=ctx}catch(ccx){}if(document.querySelector(`.ccPatchSettingsMenu`)){ccPatchCloseSettingsMenu();return}let t=e.currentTarget;if(!t)return;t.classList.add(`ccPatchSettingsBtnOpen`);let n=t.getBoundingClientRect(),r=document.createElement(`div`);r.className=`ccPatchSettingsMenu`,r.style.top=`${Math.round(n.bottom+4)}px`,r.style.left=`-9999px`;function a(l,i,o,c,k){let d=document.createElement(`div`);d.className=`ccPatchSettingsItem`;let s=document.createElement(`span`);s.className=`ccPatchSettingsItemIcon`;s.innerHTML=i;d.appendChild(s);let p=document.createElement(`span`);p.className=`ccPatchSettingsItemLabel`;p.textContent=l;d.appendChild(p);if(o){let u=document.createElement(`span`);u.className=`ccPatchSettingsItemExtra`;u.textContent=o;d.appendChild(u)}d.onmousedown=function(v){v.preventDefault();v.stopPropagation();if(!k)ccPatchCloseSettingsMenu();if(c)c()};r.appendChild(d)}(function(){let d=document.createElement(`div`);d.className=`ccPatchSettingsItem`;let ic=document.createElement(`span`);ic.className=`ccPatchSettingsItemIcon`;ic.innerHTML=`<svg width="14" height="14" viewBox="0 0 14 14" fill="none" stroke="currentColor" stroke-width="1.3" stroke-linecap="round" stroke-linejoin="round"><rect x="2" y="2.5" width="10" height="9" rx="1.5"/><line x1="2" y1="5.5" x2="12" y2="5.5"/><line x1="4" y1="8" x2="8.5" y2="8"/></svg>`;d.appendChild(ic);let p=document.createElement(`span`);p.className=`ccPatchSettingsItemLabel`;p.textContent=`Chat preview header`;d.appendChild(p);let lb=document.createElement(`span`);lb.className=`ccPatchYoloToggle`;let cb=document.createElement(`input`);cb.type=`checkbox`;cb.checked=ccPatchStickyOn();let sl=document.createElement(`span`);sl.className=`ccPatchYoloSlider`;lb.appendChild(cb);lb.appendChild(sl);d.appendChild(lb);d.onmousedown=function(v){v.preventDefault();v.stopPropagation();cb.checked=!cb.checked;ccPatchStickyToggle()};r.appendChild(d)})();a(`Account & usage`,`<svg width="14" height="14" viewBox="0 0 14 14" fill="none" stroke="currentColor" stroke-width="1.4" stroke-linecap="round" stroke-linejoin="round"><circle cx="7" cy="7" r="5.5"/><path d="M8.7 5.2c-.4-.5-1.1-.8-1.8-.8-1 0-1.8.55-1.8 1.3 0 .8.8 1.1 1.8 1.3 1 .25 1.8.55 1.8 1.35 0 .75-.8 1.3-1.8 1.3-.75 0-1.4-.3-1.8-.8"/><line x1="7" y1="3.4" x2="7" y2="4.4"/><line x1="7" y1="9.6" x2="7" y2="10.6"/></svg>`,null,function(){try{ctx.commandRegistry.executeCommand(`account-usage`)}catch(e){}});a(`Default`,`<svg width="14" height="14" viewBox="0 0 14 14" fill="none" stroke="currentColor" stroke-width="1.4" stroke-linecap="round"><line x1="2" y1="4" x2="12" y2="4"/><line x1="2" y1="7.5" x2="12" y2="7.5"/><line x1="2" y1="11" x2="12" y2="11"/><circle cx="5" cy="4" r="1.6"/><circle cx="9.5" cy="7.5" r="1.6"/><circle cx="4.5" cy="11" r="1.6"/></svg>`,null,function(){setTimeout(ccPatchShowDefaultsMenu,0)});a(`Switch account`,`<svg width="14" height="14" viewBox="0 0 14 14" fill="none" stroke="currentColor" stroke-width="1.4" stroke-linecap="round" stroke-linejoin="round"><circle cx="7" cy="5" r="2.4"/><path d="M2.5 12.2c.7-2.2 2.5-3.4 4.5-3.4 1.3 0 2.4.5 3.3 1.3"/><polyline points="10,9.5 12,11.5 10,13.5"/><line x1="8.4" y1="11.5" x2="12" y2="11.5"/></svg>`,null,function(){ccPatchShowAccountsMenu(ctx)});a(`Custom instructions`,`<svg width="14" height="14" viewBox="0 0 14 14" fill="none" stroke="currentColor" stroke-width="1.3" stroke-linecap="round" stroke-linejoin="round"><path d="M10 1H4a1 1 0 0 0-1 1v10a1 1 0 0 0 1 1h6a1 1 0 0 0 1-1V2a1 1 0 0 0-1-1z"/><line x1="4.5" y1="4" x2="9.5" y2="4"/><line x1="4.5" y1="6.5" x2="9.5" y2="6.5"/><line x1="4.5" y1="9" x2="7.5" y2="9"/></svg>`,null,function(){ccPatchInstructionsModal(ctx)});a(`Color theme`,`<svg width="14" height="14" viewBox="0 0 14 14" fill="none" stroke="currentColor" stroke-width="1.3" stroke-linecap="round" stroke-linejoin="round"><path d="M7 1.3C3.8 1.3 1.3 3.7 1.3 6.9c0 2.8 2.1 4.8 4.7 4.8.8 0 1.3-.6 1.3-1.3 0-.4-.2-.6-.4-.9-.2-.2-.3-.5-.3-.8 0-.7.5-1.2 1.2-1.2h1.4c2 0 3.5-1.5 3.5-3.5C12.7 2.9 10.2 1.3 7 1.3z"/><circle cx="4" cy="6.4" r=".75" fill="currentColor" stroke="none"/><circle cx="6" cy="3.9" r=".75" fill="currentColor" stroke="none"/><circle cx="9" cy="4.3" r=".75" fill="currentColor" stroke="none"/></svg>`,null,function(){ccPatchThemeModal()});(function(){let d=document.createElement(`div`);d.className=`ccPatchSettingsItem`;let ic=document.createElement(`span`);ic.className=`ccPatchSettingsItemIcon`;ic.innerHTML=`<svg width="14" height="14" viewBox="0 0 14 14" fill="none" stroke="currentColor" stroke-width="1.4" stroke-linecap="round" stroke-linejoin="round"><circle cx="7" cy="7" r="5.5"/><circle cx="7" cy="7" r="2.3"/><circle cx="7" cy="7" r=".4" fill="currentColor" stroke="none"/></svg>`;d.appendChild(ic);let p=document.createElement(`span`);p.className=`ccPatchSettingsItemLabel`;p.textContent=`Goal enforcement`;d.appendChild(p);let lb=document.createElement(`span`);lb.className=`ccPatchYoloToggle`;let gcb=document.createElement(`input`);gcb.type=`checkbox`;gcb.checked=ccPatchGoalOn();let sl=document.createElement(`span`);sl.className=`ccPatchYoloSlider`;lb.appendChild(gcb);lb.appendChild(sl);d.appendChild(lb);d.onmousedown=function(v){v.preventDefault();v.stopPropagation();var want=!gcb.checked;if(want){ccPatchConfirmDialog({title:`Turn on Goal enforcement?`,desc:`Claude keeps working until each task is genuinely done — or it reaches a decision only you can make — so you're not stuck repeating “keep going.”`,note:`Applies to every project and will use more of your usage. It adds a rule to your Claude settings, removed the moment you turn it off. New chats use it right away — reload to apply it here.`,confirmLabel:`Turn on`,icon:`<svg width="22" height="22" viewBox="0 0 22 22" fill="none" stroke="currentColor" stroke-width="1.6" stroke-linecap="round" stroke-linejoin="round"><circle cx="11" cy="11" r="8"/><circle cx="11" cy="11" r="3.4"/><circle cx="11" cy="11" r=".6" fill="currentColor" stroke="none"/></svg>`,onConfirm:function(h){h.setBusy(true);h.setStatus(`Updating settings.json...`);ccPatchGoalApply(ctx,true,function(err){if(err){h.setStatus(`Failed: `+((err&&err.message)||err));h.setBusy(false);return}ccPatchGoalSet(true);gcb.checked=true;h.setStatus(`On - new chats will keep working until done.`);setTimeout(h.close,900)});return true}})}else{ccPatchGoalApply(ctx,false,function(err){if(err){ccPatchConfirmDialog({title:`Couldn't turn it off`,desc:((err&&err.message)||String(err)),confirmLabel:`OK`});return}ccPatchGoalSet(false);gcb.checked=false})}};r.appendChild(d)})();document.body.appendChild(r);var ccMW=r.offsetWidth||210,ccML=Math.round(n.right)-ccMW;if(ccML+ccMW>window.innerWidth-8)ccML=window.innerWidth-8-ccMW;if(ccML<8)ccML=8;r.style.left=`${ccML}px`;setTimeout(function(){let h=function(v){if(!r.contains(v.target)&&!t.contains(v.target))ccPatchCloseSettingsMenu()};r._ccPatchOutsideHandler=h;document.addEventListener(`mousedown`,h)},0)}
// ---- YOLO mode (PER-CHAT auto-approve) ------------------------------------
// YOLO is scoped to ONE chat — keyed by a STABLE per-chat id (ccPatchYoloKey =
// the session's internalId, which is set once at construction and constant for the
// chat's whole life). It is deliberately NOT keyed by claudeChannelId: that id is
// regenerated every turn (launchClaude) and wiped to void 0 when the stream ends,
// so keying off it silently dropped YOLO the instant a turn finished (and stored
// nothing at all when toggled while idle, since the id is void 0 then) — that was
// the 1.2.127→1.2.133 "YOLO does nothing" bug. The actual auto-approve runs in the
// webview's handleToolPermissionRequest (see patcher patch_webview_js), which only
// ever gets the ephemeral channelId — so ccPatchAutoApproveOnChannel maps that back
// to the live session (the one whose claudeChannelId matches) and reads YOLO by the
// stable key, returning "allow" before the prompt can render. State is a JSON map
// { yoloKey: 1 } in localStorage — no global flag, no host involvement, so YOLO can
// never leak across chats. (A legacy global "1"/"0" value from earlier builds parses
// to a non-object and is harmlessly ignored, resetting everyone to per-chat OFF.)
// ccPatchConfirmDialog is in-scope here; the mode selector (bundle scope) calls the
// EXPORTED ccPatchYoloConfirm/ccPatchYoloKey wrappers.
// (.ccPatchYoloToggle/.ccPatchYoloSlider CSS is generic toggle styling shared with
// the Chat-preview / Goal / Auto-resume switches.)
function ccPatchAutoApproveMap(){try{var o=JSON.parse(localStorage.getItem(`ccPatchAutoApprove`)||`{}`);return(o&&typeof o===`object`)?o:{}}catch(e){return{}}}
function ccPatchAutoApproveOn(ch){if(!ch)return false;try{return ccPatchAutoApproveMap()[ch]===1}catch(e){return false}}
function ccPatchAutoApproveSet(ch,on){if(!ch)return;try{var o=ccPatchAutoApproveMap();if(on)o[ch]=1;else delete o[ch];localStorage.setItem(`ccPatchAutoApprove`,JSON.stringify(o))}catch(e){}}
// Stable per-chat key for YOLO state. internalId is set once when the session
// object is built and never reassigned, so it survives every turn (claudeChannelId
// does not). Falls back to sessionId/title for any session shape lacking internalId.
function ccPatchYoloKey(s){try{if(!s)return null;return s.internalId||(s.sessionId&&s.sessionId.value)||null}catch(e){return null}}
// Read YOLO state from the ONLY id handleToolPermissionRequest is handed — the
// ephemeral per-turn channelId. Resolution is TWO-TIER so YOLO can never silently
// leak a prompt through (Jacob's #1 pain: "so it doesn't ask me"):
//  (1) PRIMARY — a channelId->yoloKey Map (ccPatchYoloChan) recorded at the SOURCE.
//      launchClaude() mints the session's fresh claudeChannelId, and the patched
//      assignment calls ccPatchYoloChanRecord(channelId, session) on the very next
//      statement. A tool-permission request can only fire AFTER its turn has
//      launched, so the map is ALWAYS populated before we're asked — O(1), and with
//      zero dependence on the session being in any list (the old scan could miss a
//      session that wasn't yet in sessionsRoot / wasn't the composer session, and
//      that miss = a prompt YOLO should have eaten).
//  (2) FALLBACK — the original scan over live sessions, for any channel we didn't
//      record (e.g. a turn that launched before this build's JS loaded); on a hit it
//      back-fills the map so the next request on that channel is O(1).
// Both tiers key off the STABLE ccPatchYoloKey and read YOLO LIVE from localStorage,
// so toggling YOLO mid-turn takes effect on the very next request. Unknown channel
// => false (safely ask). ccPatchYoloChan is capped FIFO so it can't grow unbounded.
globalThis.ccPatchYoloChan=globalThis.ccPatchYoloChan||new Map();
function ccPatchYoloChanRecord(ch,s){try{if(!ch||!s)return;var k=ccPatchYoloKey(s);if(!k)return;var m=globalThis.ccPatchYoloChan;m.set(ch,k);if(m.size>256){var it=m.keys().next();if(!it.done)m.delete(it.value)}}catch(e){}}
function ccPatchAutoApproveOnChannel(ch){if(!ch)return false;try{var k=globalThis.ccPatchYoloChan&&globalThis.ccPatchYoloChan.get(ch);if(k)return ccPatchAutoApproveOn(k);var ss=ccPatchContSessions();for(var i=0;i<ss.length;i++){var s=ss[i];if(s&&s.claudeChannelId===ch){ccPatchYoloChanRecord(ch,s);return ccPatchAutoApproveOn(ccPatchYoloKey(s))}}}catch(e){}return false}
// YOLO auto-approves ACTIONS, never QUESTIONS. A few "tools" that flow through the
// same tool-permission gate are not requests to DO something — they are the model
// ASKING the user to decide: AskUserQuestion (its permission UI IS a radio-option
// question) and ExitPlanMode (approve this plan?). Blanket-allowing those makes YOLO
// answer "yes" with no actual choice — the model then sees a content-free approval and
// the user loses the ability to be consulted (Jacob's report). So the short-circuit
// SKIPS these toolNames and lets their real UI render, even with YOLO on; every other
// tool (edits, bash, deletes — the destructive ones YOLO exists to wave through) still
// auto-approves. Map is on globalThis so it's easy to extend if Claude adds more.
globalThis.ccPatchYoloAskTools=globalThis.ccPatchYoloAskTools||{AskUserQuestion:1,ExitPlanMode:1};
function ccPatchYoloIsAsk(req){try{return!!(req&&req.toolName&&globalThis.ccPatchYoloAskTools[req.toolName])}catch(e){return false}}
// One-time confirm before the FIRST time YOLO mode is selected from the Modes menu
// (it skips ALL safety prompts). After the user confirms once, picking YOLO is
// instant. ccPatchYoloConfirm wraps ccPatchConfirmDialog (in-scope here); the mode
// selector lives in the bundle scope and calls the EXPORTED wrappers. If the dialog
// is somehow unavailable it falls through to the callback so YOLO is never blocked.
function ccPatchYoloConfirmedGet(){try{return localStorage.getItem(`ccPatchYoloConfirmed`)===`1`}catch(e){return false}}
function ccPatchYoloConfirm(cb,onCancel){try{ccPatchConfirmDialog({title:`Turn on YOLO mode?`,onCancel:onCancel,desc:`Every approval — file edits, terminal commands, MCP tools — is auto-approved. Claude never stops to ask while YOLO mode is on.`,note:`This skips ALL safety prompts, including destructive commands. Use it only when you trust what's running. Pick any other mode to turn it off.`,confirmLabel:`Turn on`,icon:`<svg width="22" height="22" viewBox="0 0 22 22" fill="none" stroke="currentColor" stroke-width="1.6" stroke-linecap="round" stroke-linejoin="round"><path d="M12 2 5 12h5l-2 8 9-12h-6z"/></svg>`,onConfirm:function(h){try{localStorage.setItem(`ccPatchYoloConfirmed`,`1`)}catch(e){}try{if(cb)cb()}catch(e){}if(h&&h.setStatus)h.setStatus(`YOLO mode on — every prompt is auto-approved.`);if(h&&h.close)setTimeout(h.close,750);return true}})}catch(e){try{if(cb)cb()}catch(e2){}}}
...
// ---- Copy diagnostics -----------------------------------------------------
// One gear-menu click bundles everything needed to identify an API error: the
// live webview signals (error.value, rateLimitWarning, usage meters), exactly
// what Auto-continue decided and why (ccPatchContDbg breadcrumb + per-session
// state), and the HOST-side API ring buffer (real HTTP status/type/headers,
// fetched over the orbit_diagnostics RPC). Copied as a readable Markdown report
// + a raw JSON block, so pasting it lets Claude name the error instead of guessing.
function ccPatchDiagSafe(fn,d){try{var v=fn();return(v==null)?d:v}catch(e){return d}}
function ccPatchDiagFmtAgo(ms){if(ms==null||isNaN(ms))return`?`;var neg=ms<0?`-`:``;var s=Math.round(Math.abs(ms)/1000);if(s<60)return neg+s+`s`;var m=Math.floor(s/60),rs=s%60;if(m<60)return neg+m+`m`+(rs?` `+rs+`s`:``);var h=Math.floor(m/60),rm=m%60;return neg+h+`h`+(rm?` `+rm+`m`:``)}
function ccPatchDiagWhen(t){try{if(t==null)return`?`;var d=new Date(t);return isNaN(d.getTime())?String(t):d.toLocaleTimeString()}catch(e){return String(t)}}
function ccPatchDiagRpc(ctx,cb){var r=ccPatchRpc(ctx);try{if(r){r.sendRequest({type:"orbit_diagnostics"}).then(function(o){cb(o||{})}).catch(function(e){cb({_rpcError:String((e&&e.message)||e)})});return}}catch(e){}cb({_rpcError:`no host connection`})}
function ccPatchToast(msg,bad){try{var t=document.createElement(`div`);t.textContent=msg;t.style.cssText=`position:fixed;left:50%;bottom:56px;transform:translateX(-50%) translateY(8px);max-width:80vw;background:`+(bad?`#5b1d1d`:`#1f2937`)+`;color:#fff;border:1px solid rgba(255,255,255,.14);padding:9px 15px;border-radius:9px;font-size:12.5px;font-weight:500;z-index:2147483647;opacity:0;transition:opacity .18s ease,transform .18s ease;box-shadow:0 8px 30px rgba(0,0,0,.45);pointer-events:none`;document.body.appendChild(t);requestAnimationFrame(function(){t.style.opacity=`1`;t.style.transform=`translateX(-50%) translateY(0)`});setTimeout(function(){t.style.opacity=`0`;t.style.transform=`translateX(-50%) translateY(8px)`;setTimeout(function(){try{t.remove()}catch(e){}},250)},2600)}catch(e){}}
function ccPatchClipFallback(text){try{var ta=document.createElement(`textarea`);ta.value=text;ta.style.position=`fixed`;ta.style.left=`-9999px`;document.body.appendChild(ta);ta.focus();ta.select();var ok=false;try{ok=document.execCommand(`copy`)}catch(e){}ta.remove();return ok}catch(e){return false}}
function ccPatchClip(text){try{if(navigator.clipboard&&navigator.clipboard.writeText)return navigator.clipboard.writeText(text).then(function(){return true}).catch(function(){return ccPatchClipFallback(text)})}catch(e){}return Promise.resolve(ccPatchClipFallback(text))}
function ccPatchBuildDiag(host){host=host||{};var now=Date.now();var L=[];function P(x){L.push(x==null?``:String(x))}var FENCE=String.fromCharCode(96,96,96);var ver=ccPatchDiagSafe(function(){return(typeof ccPatchBuildVersion!==`undefined`)?ccPatchBuildVersion:``},``);var chan=ccPatchDiagSafe(function(){return(typeof ccPatchChannel!==`undefined`)?ccPatchChannel:``},``);var s=ccPatchDiagSafe(function(){return ccPatchActiveSession()},null);var sid=s?ccPatchDiagSafe(function(){return ccPatchSessionId(s)},null):null;var errVal=s?ccPatchDiagSafe(function(){return s.error&&s.error.value},null):null;var rlw=s?ccPatchDiagSafe(function(){return s.rateLimitWarning&&s.rateLimitWarning.value},null):null;var hardReason=s?ccPatchDiagSafe(function(){return ccPatchErrorReason(s)},null):null;var transient=s?ccPatchDiagSafe(function(){return!!ccPatchContTransientErr(s)},false):false;var errLive=s?ccPatchDiagSafe(function(){return!!ccPatchContErrorStop(s)},false):false;var wasErr=sid?ccPatchDiagSafe(function(){return!!ccPatchContWasErrored(sid)},false):false;var busy=s?ccPatchDiagSafe(function(){return!!(s.busy&&s.busy.value)},false):false;var waiting=s?ccPatchDiagSafe(function(){return!!ccPatchIsWaiting(s)},false):false;var archived=s?ccPatchDiagSafe(function(){return!!ccPatchIsArchived(s)},false):false;var cwd=s?ccPatchDiagSafe(function(){return s.cwd&&s.cwd.value},null):null;var usage=ccPatchDiagSafe(function(){return ccPatchUsageData()},null);var maxedWin=ccPatchDiagSafe(function(){return ccPatchContUsageWindow()},null);var toggles={limit:ccPatchContOn(`limit`),error:ccPatchContOn(`error`)};var stt=sid?ccPatchDiagSafe(function(){return ccPatchContState.get(sid)},null):null;var dbg=sid?ccPatchDiagSafe(function(){return(globalThis.ccPatchContDbg||{})[sid]},null):null;var maxN=(typeof ccPatchContMax!==`undefined`)?ccPatchContMax:6;var banners=ccPatchDiagSafe(function(){var out=[];var eb=document.querySelectorAll(`[class*="errorBanner_"],[class*="errorMessage_"]`);for(var i=0;i<eb.length&&out.length<4;i++){var tx=(eb[i].innerText||eb[i].textContent||``).replace(/\s+/g,` `).trim();if(tx)out.push(tx.slice(0,300))}return out},[]);var apiLog=(host&&Array.isArray(host.apiLog))?host.apiLog:[];var others=[];try{if(ccPatchContState&&ccPatchContState.forEach)ccPatchContState.forEach(function(v,k){if(k===sid)return;if(v&&(v.pending||(v.tries||0)>0))others.push({sessionId:k,pending:v.pending||null,tries:v.tries||0,handled:!!v.handled})})}catch(e){}var recentMsgs=ccPatchDiagSafe(function(){var out=[];var m=s&&s.messages&&s.messages.value;if(m&&m.length){for(var i=Math.max(0,m.length-6);i<m.length;i++){var x=m[i];if(!x)continue;var txt=``;if(typeof x.text===`string`)txt=x.text;else if(typeof x.content===`string`)txt=x.content;else if(Array.isArray(x.content)){var parts=[];for(var j=0;j<x.content.length;j++){var b=x.content[j];if(b&&typeof b.text===`string`)parts.push(b.text);else if(b&&b.type)parts.push(`[`+b.type+`]`)}txt=parts.join(` `)}out.push({role:x.role||x.type||null,subtype:x.subtype||null,isError:!!x.isError,text:String(txt||``).replace(/\s+/g,` `).trim().slice(0,300)})}}return out},[]);var lastErrMsg=ccPatchDiagSafe(function(){var m=s&&s.messages&&s.messages.value;if(!m||!m.length)return null;for(var i=m.length-1;i>=0&&i>=m.length-8;i--){var x=m[i];if(!x)continue;var hit=(x.isError===true)||(x.subtype===`mirror_error`)||(typeof ccPatchContMsgErr===`function`&&ccPatchContMsgErr(x));if(hit){var t=``;if(typeof x.text===`string`)t=x.text;else if(typeof x.content===`string`)t=x.content;else if(Array.isArray(x.content)){for(var j=0;j<x.content.length;j++){var b=x.content[j];if(b&&typeof b.text===`string`)t+=(t?` `:``)+b.text}}return{fromEnd:m.length-1-i,subtype:x.subtype||null,isError:!!x.isError,text:String(t||``).replace(/\s+/g,` `).trim().slice(0,500)}}}return null},null);var lastErr=null;for(var ei=apiLog.length-1;ei>=0;ei--){var le=apiLog[ei];if(le&&((typeof le.status===`number`&&le.status>=400)||le.kind===`reject`||le.kind===`throw`)){lastErr=le;break}}var ip=[];if(maxedWin)ip.push(`Your `+(maxedWin===`fiveHour`?`5-hour`:`weekly`)+` usage window is MAXED (>=95%) right now — this is a REAL usage cap. Auto-continue parks and resumes only after it resets.`);if(hardReason&&!maxedWin)ip.push(`The chat shows a usage-limit banner ("`+hardReason+`") but the LIVE meter is NOT maxed (your 5h/weekly windows have headroom) — a STALE banner, not a real limit. A lingering banner like this is what stalls auto-continue (it parks for a reset that already happened); patcher 1.2.108 makes it resume instead.`);if(lastErrMsg&&!maxedWin&&!hardReason){var em=lastErrMsg.text||``;if(/\b529\b|overloaded/i.test(em))ip.push(`An error is sitting in the chat flow: a 529 "Overloaded" — Anthropic's servers are overloaded (server-side, TRANSIENT, not your usage limit). With the Auto-continue "error" toggle on, it should retry on its own ~6s after the chat goes idle; if you manually send before then, you beat it to it.`);else if(/API Error|rate limit|too many requests|\b5\d\d\b|temporarily/i.test(em))ip.push(`An error is sitting in the chat flow: "`+em.slice(0,180)+`" — a server/API error (transient). The "error" toggle should retry it ~6s after the chat goes idle.`);else ip.push(`The last in-chat message is error-flagged: "`+em.slice(0,180)+`".`)}if(lastErr){var st=lastErr.status,et=lastErr.errType,hd=lastErr.headers||{},ra=hd[`retry-after`];if(lastErr.kind===`reject`||lastErr.kind===`throw`)ip.push(`Last API call failed at the network layer (`+(lastErr.err||`connection error`)+`) — no HTTP response; a connectivity/proxy problem, not a limit.`);else if(st===429&&!maxedWin)ip.push(`Last API failure: HTTP 429 `+(et||`rate_limit_error`)+` — a SHORT-TERM rate limit / burst, NOT your 5h/weekly quota (the meter isn't maxed). Transient: a brief backoff + retry clears it.`);else if(st===429&&maxedWin)ip.push(`Last API failure: HTTP 429 `+(et||``)+` and the meter IS maxed — so this is genuinely your quota.`);else if(st===529||et===`overloaded_error`)ip.push(`Last API failure: HTTP `+(st||529)+` overloaded_error — Anthropic's servers are overloaded (server-side, transient). Retry; not your limit.`);else if(typeof st===`number`&&st>=500)ip.push(`Last API failure: HTTP `+st+` `+(et||`server error`)+` — server-side hiccup. Transient; retry.`);else if(st===401||st===403)ip.push(`Last API failure: HTTP `+st+` — an AUTH problem (token expired / signed out). Needs re-auth; retrying won't help.`);else if(typeof st===`number`)ip.push(`Last API failure: HTTP `+st+(et?` `+et:``)+(lastErr.errMsg?` — "`+lastErr.errMsg+`"`:``)+`.`);if(ra)ip.push(`The server asked to retry after `+ra+`s — that's the correct delay to wait.`)}else if(host&&host.captured===false)ip.push(`No host-side API capture on this build (the SDK isn't routing through global fetch) — falling back to webview signals only; HTTP status/headers aren't available.`);else if(!maxedWin&&!hardReason&&!lastErrMsg)ip.push(`No recent API failures recorded (host log + chat flow are clean) — if a chat is stuck, it isn't a server/limit error.`);if(transient&&!maxedWin)ip.push(`The current error text says "(not your usage limit)" and the meter isn't maxed → Auto-continue should fast-retry, not park.`);P(`# Orbit diagnostics`);P(``);P(`- Captured: `+new Date(now).toISOString());P(`- Orbit patcher: `+(ver||`?`)+` (`+(chan||`?`)+`)`);P(`- Claude Code: `+(host.claudeVersion||`?`)+`  ·  Node `+(host.node||`?`)+`  ·  `+(host.platform||`?`));if(host._rpcError)P(`- ⚠ host RPC error: `+host._rpcError);P(``);P(`## What this looks like`);if(!ip.length)ip=[`Nothing notable — no active error or usage cap detected on the open chat.`];for(var ii=0;ii<ip.length;ii++)P(`- `+ip[ii]);P(``);P(`## Active chat`);P(`- sessionId: `+(sid||`(none)`));P(`- state: `+(busy?`busy`:`idle`)+(waiting?`, waiting for input`:``)+(archived?`, archived`:``));P(`- cwd: `+(cwd||`?`));P(``);P(`## Error state (webview signals)`);P(`- error.value: `+(errVal?JSON.stringify(String(errVal).slice(0,400)):`(none)`));P(`- rateLimitWarning: `+(rlw?JSON.stringify(String(rlw).slice(0,300)):`(none)`));P(`- hard usage-limit reason: `+(hardReason?JSON.stringify(String(hardReason).slice(0,300)):`(none)`));P(`- classified transient ("not your usage limit"): `+(transient?`YES`:`no`));P(`- live error signal (errorStop): `+(errLive?`YES`:`no`));P(`- persisted-errored across reload: `+(wasErr?`YES`:`no`));if(banners.length){P(`- error banner(s) on screen:`);for(var bi=0;bi<banners.length;bi++)P(`    • `+banners[bi])}P(``);P(`## Usage windows (live meter)`);function winLine(name,w){if(!w)return;var u=(typeof w.utilization===`number`)?Math.round(w.utilization)+`%`:`?`;var rs=``;try{if(w.resetsAt!=null){var t=(typeof w.resetsAt===`number`)?(w.resetsAt<1e12?w.resetsAt*1000:w.resetsAt):new Date(w.resetsAt).getTime();if(!isNaN(t))rs=` · resets in `+ccPatchDiagFmtAgo(t-now)+` (`+ccPatchDiagWhen(t)+`)`}}catch(ee){rs=``}P(`- `+name+`: `+u+rs)}if(usage){winLine(`5-hour`,usage.fiveHour);winLine(`weekly (7d)`,usage.sevenDay);if(usage.sevenDaySonnet)winLine(`weekly Sonnet`,usage.sevenDaySonnet);if(usage.unavailableReason)P(`- unavailableReason: `+usage.unavailableReason)}else{P(`- (no usage data available)`)}P(`- maxed window (>=95%): `+(maxedWin||`none`));P(``);P(`## Auto-resume`);P(`- toggles: limit-reset=`+(toggles.limit?`on`:`off`)+`  error=`+(toggles.error?`on`:`off`));if(stt){P(`- pending: `+(stt.pending?JSON.stringify(stt.pending):`none`));P(`- tries this window: `+(stt.tries||0)+`/`+maxN);if(stt.lastSendAt)P(`- last auto-send: `+ccPatchDiagFmtAgo(now-stt.lastSendAt)+` ago`);P(`- handled: `+(!!stt.handled))}else{P(`- (no per-session auto-continue state for this chat yet)`)}if(dbg){P(`- last decision: `+ccPatchDiagFmtAgo(now-dbg.t)+` ago — hard=`+dbg.hard+` maxed=`+dbg.maxed+` errLive=`+dbg.errLive+` errRec=`+dbg.errRec+` transient=`+dbg.transient+` => `+(dbg.usageLimit?`PARK as usage limit (wait for reset)`:`error/transient retry or handled`))}else{P(`- (no decision breadcrumb — the tick hasn't classified this chat)`)}if(others.length){P(`- other chats with auto-continue activity:`);for(var oi=0;oi<others.length;oi++)P(`    • `+JSON.stringify(others[oi]))}P(``);P(`## In-chat error / recent messages`);if(lastErrMsg){P(`- last error-flagged message (`+lastErrMsg.fromEnd+` from end`+(lastErrMsg.subtype?`, subtype=`+lastErrMsg.subtype:``)+(lastErrMsg.isError?`, isError`:``)+`): `+JSON.stringify(lastErrMsg.text))}else{P(`- no error-flagged message found in the recent flow`)}if(recentMsgs.length){P(`- recent messages (newest last):`);for(var ri=0;ri<recentMsgs.length;ri++){var rm=recentMsgs[ri];P(`    • `+(rm.role||`?`)+(rm.subtype?`/`+rm.subtype:``)+(rm.isError?` [isError]`:``)+`: `+(rm.text?rm.text.slice(0,160):``))}}P(``);P(`## Host API log — last `+apiLog.length+` Anthropic responses`);if(host&&host.captured===false)P(`- ⚠ host capture not active (the global-fetch hook never fired) — no HTTP status/headers this build.`);if(!apiLog.length){P(`- (empty)`);if(host&&host.captured!==false)P(`  Note: Claude runs its API calls in a separate CLI process, so server errors (429/529/5xx) usually surface in the chat flow above — NOT in this host log. An empty log here is normal; trust the In-chat error section.`)}else{for(var li=apiLog.length-1;li>=0;li--){var e=apiLog[li];if(!e)continue;var hh=e.headers||{};var bits=[];bits.push(ccPatchDiagWhen(e.t));bits.push((e.method||`?`)+` `+(e.path||`?`));if(e.status!=null)bits.push(`=> `+e.status+(e.statusText?` `+e.statusText:``));else bits.push(`=> `+(e.kind||`no-response`));if(e.errType)bits.push(`[`+e.errType+`]`);if(e.errMsg)bits.push(`"`+e.errMsg+`"`);if(e.err)bits.push(`"`+e.err+`"`);if(hh[`retry-after`])bits.push(`retry-after=`+hh[`retry-after`]+`s`);var reset=hh[`anthropic-ratelimit-unified-reset`]||hh[`anthropic-ratelimit-requests-reset`];if(reset)bits.push(`reset=`+reset);var rid=hh[`request-id`]||hh[`x-request-id`];if(rid)bits.push(`req=`+rid);if(e.ms!=null)bits.push(`(`+e.ms+`ms)`);P(`- `+bits.join(`  `))}}P(``);P(`## Raw`);P(FENCE+`json`);var raw={capturedAt:new Date(now).toISOString(),orbit:{patcher:ver||null,channel:chan||null},host:{claudeVersion:host.claudeVersion||null,node:host.node||null,electron:host.electron||null,platform:host.platform||null,captured:(host.captured!==undefined?!!host.captured:null),rpcError:host._rpcError||null},chat:{sessionId:sid,busy:busy,waiting:waiting,archived:archived,cwd:cwd||null},errorState:{error:errVal?String(errVal).slice(0,1000):null,rateLimitWarning:rlw?String(rlw).slice(0,500):null,hardReason:hardReason?String(hardReason).slice(0,500):null,transient:transient,errorStop:errLive,wasErrored:wasErr,banners:banners},usage:usage||null,maxedWindow:maxedWin,inChat:{lastError:lastErrMsg,recent:recentMsgs},autoContinue:{toggles:toggles,state:stt||null,lastDecision:dbg||null,others:others,max:maxN},apiLog:apiLog.map(function(le){return le?{t:le.t,ms:le.ms,method:le.method,path:le.path,status:le.status,statusText:le.statusText,ok:le.ok,kind:le.kind||null,errType:le.errType||null,errMsg:le.errMsg||null,err:le.err||null,headers:le.headers||null}:null})};var rawStr;try{rawStr=JSON.stringify(raw,null,2)}catch(e){rawStr=`(could not serialize: `+String((e&&e.message)||e)+`)`}P(rawStr);P(FENCE);return L.join(`\n`)}
function ccPatchCopyDiagnostics(ctx){try{ccPatchUsageRefresh(true)}catch(e){}ccPatchToast(`Collecting diagnostics…`);try{ctx=ctx||globalThis.ccPatchCtx}catch(e){}var done=false;function finish(host){if(done)return;done=true;var text;try{text=ccPatchBuildDiag(host||{})}catch(e){text=`# Orbit diagnostics`+String.fromCharCode(10,10)+`(failed to build: `+String((e&&e.message)||e)+`)`}try{globalThis.ccOrbitLastDiagnostics=text}catch(e){}try{console.log(`[Orbit] diagnostics`+String.fromCharCode(10)+text)}catch(e){}Promise.resolve(ccPatchClip(text)).then(function(ok){if(ok)ccPatchToast(`Diagnostics copied — paste them to Claude.`);else ccPatchToast(`Copy blocked — run ccOrbitCopyDiagnostics() in DevTools (or grab ccOrbitLastDiagnostics).`,true)})}setTimeout(function(){finish({_rpcError:`host did not respond (2.5s)`})},2500);ccPatchDiagRpc(ctx,function(host){finish(host)})}
// ---- Goal enforcement (always-on "innate goal") --------------------------
// A gear-menu toggle that installs a generic, always-on prompt-based Stop hook
// into ~/.claude/settings.json. /goal is itself just a temporary session-scoped
// Stop hook; this makes the same idea permanent with a GENERIC condition so the
// user never has to type a specific goal. The same Haiku evaluator /goal uses
// judges, after every turn, whether Claude may stop (truly done or genuinely
// blocked on the user) — otherwise Claude keeps working. Claude Code's built-in
// 8-block cap (stop_hook_active) prevents an infinite loop. We identify OUR hook
// by the ORBIT-GOAL-ENFORCEMENT marker in the prompt, so enabling/disabling only
// ever touches our own entry — the user's other Stop hooks are preserved. State
// mirrors to localStorage for instant toggle UI; settings.json is the truth.
function ccPatchGoalOn(){try{return localStorage.getItem(`ccPatchGoalEnf`)===`1`}catch(e){return false}}
function ccPatchGoalSet(v){try{localStorage.setItem(`ccPatchGoalEnf`,v?`1`:`0`)}catch(e){}}
function ccPatchGoalHookPrompt(){return `ORBIT-GOAL-ENFORCEMENT: Decide whether Claude may stop now. Return {"ok":true} (allow stop) if ANY of these hold: (1) Claude has done all the work IT can do this turn without the user, and verified it; (2) the only things left are for the USER to do - test, review, run it, decide, approve, or provide a value - which is a handoff, NOT unfinished work for Claude; (3) Claude is waiting on a decision, value, approval, or confirmation of a destructive/irreversible step that only the user can give, and has said so; (4) the turn is a question to the user or a conversational answer to what was asked; (5) the only open work is a background task, workflow or sub-agent Claude started: the harness wakes Claude when it finishes, so waiting is not work. Return {"ok":false,"reason":"<specific reversible work Claude itself can do RIGHT NOW with no user input>"} ONLY when such concrete work actually exists. Never keep Claude working to make the user perform a future action, to re-verify something already proven, or because it merely feels like more could be done.`;}
function ccPatchGoalRead(ctx,cb){ccPatchReadClaudeMd(ctx,`~/.claude/settings.json`,function(err,content,exists){if(err)return cb(err,null);var obj={};if(exists&&content&&content.trim()){try{obj=JSON.parse(content)}catch(e){return cb(new Error(`~/.claude/settings.json is not plain JSON (comments?) - can't edit it automatically`),null)}}if(!obj||typeof obj!==`object`||Array.isArray(obj))obj={};cb(null,obj)})}
function ccPatchGoalApply(ctx,on,cb){ccPatchGoalRead(ctx,function(err,obj){if(err)return cb(err);if(!obj.hooks||typeof obj.hooks!==`object`||Array.isArray(obj.hooks))obj.hooks={};var arr=Array.isArray(obj.hooks.Stop)?obj.hooks.Stop:[];var MARK=`ORBIT-GOAL-ENFORCEMENT`;arr=arr.filter(function(g){if(!g||!Array.isArray(g.hooks))return true;g.hooks=g.hooks.filter(function(h){return !(h&&typeof h.prompt===`string`&&h.prompt.indexOf(MARK)>=0)});return g.hooks.length>0});if(on)arr.push({hooks:[{type:`prompt`,prompt:ccPatchGoalHookPrompt(),model:`claude-opus-5-5`}]});if(arr.length)obj.hooks.Stop=arr;else if(obj.hooks.Stop)delete obj.hooks.Stop;if(obj.hooks&&!Object.keys(obj.hooks).length)delete obj.hooks;var out;try{out=JSON.stringify(obj,null,2)}catch(e){return cb(new Error(`could not serialize settings.json`))}ccPatchWriteClaudeMd(ctx,`~/.claude/settings.json`,out,function(werr,ok){if(werr||!ok)return cb(werr||new Error(`write failed`));cb(null)})})}
...
// ── Multi-account switcher (local, Google-style) ────────────────────────────
// "Switch account" opens a wider dropdown listing every account you've signed in
// with — each with name, email, and live 5h/weekly usage bars. Click one and it
// swaps ~/.claude/.credentials.json to that account's saved copy. ALL credential
// I/O + the per-account profile/usage fetches happen HOST-SIDE (orbit_accounts
// RPC) — tokens NEVER enter the webview. getOAuthTokens() reads that file fresh
// per request, so a switch takes effect on the next message/new chat with NO
// reload. Your current login is auto-saved before any switch so it's never lost.
// Active account's bars use the LIVE usage signal; other accounts are fetched
// with their own token (falling back to the last cached value).
function ccPatchAcctRpc(action,extra,cb){var r=ccPatchRpc(globalThis.ccPatchCtx);var req=Object.assign({type:`orbit_accounts`,action:action},extra||{});try{if(r){r.sendRequest(req).then(function(o){if(cb)cb(null,o||{})}).catch(function(e){if(cb)cb(e,null)});return}}catch(e){}if(cb)cb(new Error(`no rpc`),null)}
function ccPatchAcctNormUsage(raw){function w(x){if(!x)return null;var u=(typeof x.utilization===`number`)?x.utilization:(typeof x.utilization===`string`?parseFloat(x.utilization):null);if(u==null||isNaN(u))return null;return{utilization:u,resetsAt:x.resets_at||x.resetsAt||null}}if(!raw)return null;return{fiveHour:w(raw.five_hour||raw.fiveHour),sevenDay:w(raw.seven_day||raw.sevenDay)}}
function ccPatchAcctInitials(m){var s=((m&&(m.name||m.email||m.label))||``)+``;s=s.trim();if(!s)return`?`;var parts=s.split(/[\s@._-]+/).filter(Boolean);if(parts.length>=2)return(parts[0][0]+parts[1][0]).toUpperCase();return s.slice(0,2).toUpperCase()}
function ccPatchAcctBar(label,pct){var row=document.createElement(`div`);row.className=`ccPatchAcctBarRow`;var lab=document.createElement(`span`);lab.className=`ccPatchAcctBarLab`;lab.textContent=label;var track=document.createElement(`div`);track.className=`ccPatchAcctBarTrack`;var fill=document.createElement(`div`);fill.className=`ccPatchAcctBarFill`;var val=document.createElement(`span`);val.className=`ccPatchAcctBarVal`;if(pct==null||isNaN(pct)){fill.style.width=`0%`;val.textContent=`—`;track.classList.add(`ccPatchAcctBarEmpty`)}else{var p=Math.max(0,Math.min(100,Math.round(pct)));fill.style.width=p+`%`;fill.style.background=ccPatchUsageColor(p);val.textContent=p+`%`}track.appendChild(fill);row.appendChild(lab);row.appendChild(track);row.appendChild(val);return row}
function ccPatchAcctUsageFor(m,activeId){try{if(m&&m.id===activeId){var d=ccPatchUsageData&&ccPatchUsageData();if(d)return d}}catch(e){}return ccPatchAcctNormUsage(m&&m.usage)}
function ccPatchCloseAccountsMenu(){var m=document.querySelector(`.ccPatchAcctMenu`);if(m){if(m._ccKey)document.removeEventListener(`keydown`,m._ccKey);if(m._ccOut)document.removeEventListener(`mousedown`,m._ccOut);m.remove()}}
function ccPatchAcctBars(m,activeId){var wrap=document.createElement(`div`);wrap.className=`ccPatchAcctBars`;var u=ccPatchAcctUsageFor(m,activeId);wrap.appendChild(ccPatchAcctBar(`5h`,u&&u.fiveHour?u.fiveHour.utilization:null));wrap.appendChild(ccPatchAcctBar(`Wk`,u&&u.sevenDay?u.sevenDay.utilization:null));return wrap}
function ccPatchAcctRow(m,activeId,onSwitch,onRemove){var row=document.createElement(`div`);row.className=`ccPatchAcctRow`+(m.active?` ccPatchAcctRowActive`:``);var av=document.createElement(`div`);av.className=`ccPatchAcctAvatar`;av.textContent=ccPatchAcctInitials(m);row.appendChild(av);var body=document.createElement(`div`);body.className=`ccPatchAcctBody`;var top=document.createElement(`div`);top.className=`ccPatchAcctTop`;var nm=document.createElement(`span`);nm.className=`ccPatchAcctName`;nm.textContent=m.name||m.label||m.email||`Account`;top.appendChild(nm);if(m.subscriptionType){var pill=document.createElement(`span`);pill.className=`ccPatchAcctPlan`;pill.textContent=m.subscriptionType;top.appendChild(pill)}if(m.active){var chk=document.createElement(`span`);chk.className=`ccPatchAcctCheck`;chk.innerHTML=`<svg width="12" height="12" viewBox="0 0 12 12" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><polyline points="2.5,6.5 5,9 9.5,3.5"/></svg>`;top.appendChild(chk)}body.appendChild(top);var em=document.createElement(`div`);em.className=`ccPatchAcctEmail`;em.textContent=m.email||``;body.appendChild(em);body.appendChild(ccPatchAcctBars(m,activeId));row.appendChild(body);if(!m.active){var rm=document.createElement(`button`);rm.type=`button`;rm.className=`ccPatchAcctRemove`;rm.title=`Forget this account`;rm.innerHTML=`<svg width="12" height="12" viewBox="0 0 12 12" fill="none" stroke="currentColor" stroke-width="1.4" stroke-linecap="round"><line x1="3" y1="3" x2="9" y2="9"/><line x1="9" y1="3" x2="3" y2="9"/></svg>`;rm.onmousedown=function(e){e.preventDefault();e.stopPropagation();onRemove(m)};row.appendChild(rm);row.onmousedown=function(e){e.preventDefault();e.stopPropagation();onSwitch(m,row)}}return row}
function ccPatchShowAccountsMenu(ctx){try{globalThis.ccPatchCtx=ctx||globalThis.ccPatchCtx}catch(e){}if(document.querySelector(`.ccPatchAcctMenu`)){ccPatchCloseAccountsMenu();return}try{ccPatchCloseSettingsMenu()}catch(e){}var menu=document.createElement(`div`);menu.className=`ccPatchAcctMenu`;menu.style.left=`-9999px`;menu.style.top=`0px`;var head=document.createElement(`div`);head.className=`ccPatchAcctHead`;head.textContent=`Accounts`;menu.appendChild(head);var sub=document.createElement(`div`);sub.className=`ccPatchAcctSub`;sub.textContent=`Switch between accounts you've signed into. Saved only on this PC.`;menu.appendChild(sub);var listEl=document.createElement(`div`);listEl.className=`ccPatchAcctList`;menu.appendChild(listEl);var foot=document.createElement(`div`);foot.className=`ccPatchAcctFoot`;var add=document.createElement(`button`);add.type=`button`;add.className=`ccPatchAcctAdd`;add.innerHTML=`<svg width="13" height="13" viewBox="0 0 13 13" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round"><line x1="6.5" y1="2.5" x2="6.5" y2="10.5"/><line x1="2.5" y1="6.5" x2="10.5" y2="6.5"/></svg><span>Add account</span>`;add.onmousedown=function(e){e.preventDefault();e.stopPropagation();ccPatchConfirmDialog({title:`Add another account?`,desc:`You'll be signed out so you can sign in with a different account. Your current account stays saved here — switch back anytime.`,confirmLabel:`Sign out to add`,onConfirm:function(h){h.setBusy(true);h.setStatus(`Signing out…`);ccPatchAcctRpc(`addlogin`,null,function(err,o){h.setStatus(`Signed out — sign in with the other account.`);setTimeout(function(){h.close();ccPatchCloseAccountsMenu()},900)});return true}})};foot.appendChild(add);menu.appendChild(foot);document.body.appendChild(menu);function place(){var w=menu.offsetWidth||360,h=menu.offsetHeight||300;var pane=document.querySelector(`.claudePatchInlineSessions`)||document.querySelector(`.ccPatchSettingsBtn`);var r=pane?pane.getBoundingClientRect():{left:8,right:360,top:60};var left=Math.round(r.right)-w;if(left<8)left=8;if(left+w>window.innerWidth-8)left=window.innerWidth-8-w;var top=Math.round((r.top||60)+30);if(top+h>window.innerHeight-8)top=Math.max(8,window.innerHeight-8-h);if(top<8)top=8;menu.style.left=left+`px`;menu.style.top=top+`px`}function render(accounts,activeId){listEl.innerHTML=``;if(!accounts||!accounts.length){var empty=document.createElement(`div`);empty.className=`ccPatchAcctEmptyMsg`;empty.textContent=`No saved accounts yet.`;listEl.appendChild(empty);place();return}accounts.sort(function(a,b){return(b.active?1:0)-(a.active?1:0)});accounts.forEach(function(m){var row=ccPatchAcctRow(m,activeId,function(sel,rowEl){rowEl.classList.add(`ccPatchAcctRowBusy`);ccPatchAcctRpc(`switch`,{id:sel.id},function(err,o){if(err||!o||!o.ok){rowEl.classList.remove(`ccPatchAcctRowBusy`);if(ccPatchToast)ccPatchToast(`Couldn't switch: `+((o&&o.error)||(err&&err.message)||`error`));return}if(ccPatchToast)ccPatchToast(`Switched to `+(sel.email||sel.name||`account`)+` — new chats use it now`);load()})},function(sel){ccPatchAcctRpc(`remove`,{id:sel.id},function(){load()})});listEl.appendChild(row);if(!m.active&&(!m.usageAt||Date.now()-m.usageAt>60000)){ccPatchAcctRpc(`usage`,{id:m.id},function(err,o){if(o&&o.ok&&o.usage){var nu=ccPatchAcctNormUsage(o.usage);var bars=row.querySelector(`.ccPatchAcctBars`);if(bars&&nu){bars.innerHTML=``;bars.appendChild(ccPatchAcctBar(`5h`,nu.fiveHour?nu.fiveHour.utilization:null));bars.appendChild(ccPatchAcctBar(`Wk`,nu.sevenDay?nu.sevenDay.utilization:null))}}if(o&&o.email){var em=row.querySelector(`.ccPatchAcctEmail`);if(em&&!em.textContent)em.textContent=o.email}})}});place()}function load(){ccPatchAcctRpc(`list`,null,function(err,o){if(err||!o||!o.ok){listEl.innerHTML=``;var e2=document.createElement(`div`);e2.className=`ccPatchAcctEmptyMsg`;e2.textContent=`Couldn't load accounts.`;listEl.appendChild(e2);place();return}render(o.accounts||[],o.activeId)})}var loading=document.createElement(`div`);loading.className=`ccPatchAcctEmptyMsg`;loading.textContent=`Loading…`;listEl.appendChild(loading);place();load();setTimeout(function(){var out=function(g){if(!menu.contains(g.target))ccPatchCloseAccountsMenu()};var key=function(g){if(g.key===`Escape`)ccPatchCloseAccountsMenu()};menu._ccOut=out;menu._ccKey=key;document.addEventListener(`mousedown`,out);document.addEventListener(`keydown`,key)},0)}
function ccPatchConfirmDialog(o){o=o||{};var orig=o.onConfirm;try{var prev=document.querySelector(".ccPatchConfirmOverlay");if(prev)prev.remove();var overlay=document.createElement("div");overlay.className="ccPatchConfirmOverlay";var box=document.createElement("div");box.className="ccPatchConfirmBox";var iconWrap=document.createElement("div");iconWrap.className="ccPatchConfirmIcon";iconWrap.innerHTML=o.icon||'<svg width="22" height="22" viewBox="0 0 22 22" fill="none" stroke="currentColor" stroke-width="1.6" stroke-linecap="round" stroke-linejoin="round"><path d="M11 2.5l8 4v5c0 4.5-3.2 7.6-8 9-4.8-1.4-8-4.5-8-9v-5z"/><line x1="11" y1="8" x2="11" y2="12"/><circle cx="11" cy="15.3" r=".7" fill="currentColor" stroke="none"/></svg>';var title=document.createElement("div");title.className="ccPatchConfirmTitle";title.textContent=o.title||"Are you sure?";var desc=document.createElement("div");desc.className="ccPatchConfirmDesc";desc.textContent=o.desc||"";var note=null;if(o.note){note=document.createElement("div");note.className="ccPatchConfirmNote";note.textContent=o.note}var status=document.createElement("div");status.className="ccPatchConfirmStatus";var actions=document.createElement("div");actions.className="ccPatchConfirmActions";var cancelBtn=document.createElement("button");cancelBtn.className="ccPatchConfirmCancelBtn";cancelBtn.type="button";cancelBtn.textContent=o.cancelLabel||"Cancel";var confirmBtn=document.createElement("button");confirmBtn.className="ccPatchConfirmConfirmBtn";confirmBtn.type="button";confirmBtn.textContent=o.confirmLabel||"Confirm";var ccCfmd=false;function close(){if(!ccCfmd){try{if(o.onCancel)o.onCancel()}catch(ce){}}overlay.remove();document.removeEventListener("keydown",onKey)}function onKey(e){if(e.key==="Escape"){e.preventDefault();close()}else if(e.key==="Enter"){e.preventDefault();confirmBtn.click()}}cancelBtn.onclick=function(){close()};confirmBtn.onclick=function(){ccCfmd=true;var helpers={close:close,setStatus:function(t){status.textContent=t||""},setBusy:function(b){confirmBtn.disabled=!!b;cancelBtn.disabled=!!b}};try{var keep=(typeof orig==="function")&&orig(helpers)===true;if(!keep)close()}catch(e){status.textContent="Error: "+((e&&e.message)||e)}};overlay.onclick=function(e){if(e.target===overlay)close()};document.addEventListener("keydown",onKey);actions.appendChild(cancelBtn);actions.appendChild(confirmBtn);box.appendChild(iconWrap);box.appendChild(title);box.appendChild(desc);if(note)box.appendChild(note);box.appendChild(status);box.appendChild(actions);overlay.appendChild(box);document.body.appendChild(overlay);setTimeout(function(){confirmBtn.focus()},80);return true}catch(err){console.error("ccPatchConfirmDialog error:",err);return false}}
function ccPatchConfirmCompact(orig){var run=function(){try{if(typeof orig==="function")orig()}catch(e){}};var shown=ccPatchConfirmDialog({title:"Compact this chat?",desc:"Compacting summarizes the conversation so far and drops the older messages to free up context. The full history cannot be restored afterward.",confirmLabel:"Compact",icon:'<svg width="22" height="22" viewBox="0 0 22 22" fill="none" stroke="currentColor" stroke-width="1.7" stroke-linecap="round" stroke-linejoin="round"><line x1="4" y1="11" x2="18" y2="11"/><polyline points="7.5,5 11,8.5 14.5,5"/><polyline points="7.5,17 11,13.5 14.5,17"/></svg>',onConfirm:function(){run()}});if(!shown)run()}
...
function ccPatchSessionKey(s){if(!s)return null;try{return(s.sessionId&&s.sessionId.value)||null}catch(e){return null}}
function ccPatchQueueOwns(it,s){if(!s)return!1;if(it.sess)return it.sess===s;var k=s.sessionId&&s.sessionId.value;if(k&&it.skey===k){it.sess=s;return!0}return!1}
function ccPatchIsBusy(){var s=ccPatchActiveSession();try{return!!(s&&s.busy&&s.busy.value)}catch(e){return!1}}
function ccPatchSendQueued(it){var s=it&&it.sess;if(!s||typeof s.send!==`function`)return!1;if(!it.text&&!(it.files&&it.files.length))return!1;try{s.send(it.text||``,it.files||[],!!it.sel);return!0}catch(e){console.error(`Orbit queue send failed:`,e);return!1}}
function ccPatchQueueAddFull(t,files,sel,sess){t=(t||``).trim();if(!t&&!(files&&files.length))return;var s=sess||ccPatchActiveSession();globalThis.ccPatchQueue=(globalThis.ccPatchQueue||[]).concat([{id:`q`+Date.now()+`_`+Math.round(Math.random()*1e6),text:t,files:files||[],sel:!!sel,sess:s,skey:ccPatchSessionKey(s)}]);ccPatchQueueNotify()}
...
function ccPatchQueueNotify(){try{localStorage.setItem(`ccPatchQueue:`+(globalThis.ccPatchWsKey||``),JSON.stringify((globalThis.ccPatchQueue||[]).map(function(it){var k=(it.sess&&it.sess.sessionId&&it.sess.sessionId.value)||it.skey||null;it.skey=k;return{id:it.id,text:it.text,sel:it.sel,skey:k}}).filter(function(r){return r.skey})))}catch(e){}globalThis.ccPatchQueueListeners.forEach(function(f){try{f()}catch(err){}})}
function ccPatchSetSendMode(m){globalThis.ccPatchSendMode=m;try{localStorage.setItem(`ccPatchSendMode`,m)}catch(e){}}
// Flush PER SESSION, not per "active session". Each queued item holds a live
// reference to its own session, so we watch THAT session's busy signal directly
// and send on its OWN busy->idle edge. The old design used one global edge
// detector on ccPatchActiveSession()+ccPatchQueueWasBusy, which could only ever
// track a SINGLE session: queueing into chat A then viewing or streaming chat B
// hijacked the global, A's idle edge was never observed, and — because an idle
// chat has no future edge to re-trigger it — A's item was stranded forever.
// Map<session,wasBusy> tracks every queued session at once and is immune to
// which chat is on screen; each item sends to its OWN session, never "active".
function ccPatchQueueTick(){var q=globalThis.ccPatchQueue||[];var map=globalThis.ccPatchQueueBusyMap;if(!map||typeof map.get!==`function`){map=globalThis.ccPatchQueueBusyMap=new Map}if(!q.length){if(map.size)map.clear();return}var sess=[];for(var i=0;i<q.length;i++){var s0=q[i]&&q[i].sess;if(s0&&sess.indexOf(s0)<0)sess.push(s0)}map.forEach(function(v,k){if(sess.indexOf(k)<0)map.delete(k)});var flush=[];for(var j=0;j<sess.length;j++){var s=sess[j],busy=!1;try{busy=!!(s&&s.busy&&s.busy.value)}catch(e){}if(busy){map.set(s,!0);continue}if(!map.get(s))continue;map.set(s,!1);flush.push(s)}if(!flush.length)return;for(var f=0;f<flush.length;f++){var sx=flush[f],cur=globalThis.ccPatchQueue||[],idx=-1;for(var k2=0;k2<cur.length;k2++){if(cur[k2].sess===sx){idx=k2;break}}if(idx<0)continue;var item=cur[idx];globalThis.ccPatchQueue=cur.slice(0,idx).concat(cur.slice(idx+1));ccPatchSendQueued(item)}ccPatchQueueNotify()}
if(!globalThis.ccPatchQueueTimer)globalThis.ccPatchQueueTimer=setInterval(ccPatchQueueTick,250);
function ccPatchQueueAdd(t){t=(t||``).trim();if(!t)return;var s=ccPatchActiveSession();globalThis.ccPatchQueue=(globalThis.ccPatchQueue||[]).concat([{id:`q`+Date.now()+`_`+Math.round(Math.random()*1e6),text:t,sess:s,skey:ccPatchSessionKey(s)}]);ccPatchQueueNotify()}
...
function ccPatchQueueEdit(id){var q=globalThis.ccPatchQueue||[],item=null;for(var i=0;i<q.length;i++){if(q[i].id===id){item=q[i];break}}if(!item)return;var inp=ccPatchComposerInput();if(inp){try{inp.textContent=item.text||``;inp.dispatchEvent(new Event(`input`,{bubbles:!0}));inp.focus();var s=window.getSelection&&window.getSelection();if(s){var rng=document.createRange();rng.selectNodeContents(inp);rng.collapse(!1);s.removeAllRanges();s.addRange(rng)}}catch(e){}}ccPatchQueueRemove(id)}
...
function ccPatchQueueSendNow(id){var item=null;globalThis.ccPatchQueue=(globalThis.ccPatchQueue||[]).filter(function(x){if(x.id===id){item=x;return!1}return!0});ccPatchQueueNotify();if(!item)return;var s=item.sess;try{if(globalThis.ccPatchQueueBusyMap&&typeof globalThis.ccPatchQueueBusyMap.set===`function`&&s)globalThis.ccPatchQueueBusyMap.set(s,!1)}catch(e){}try{if(s&&s.interrupt)s.interrupt()}catch(e){}setTimeout(function(){ccPatchSendQueued(item)},90)}
...
function ccPatchWsTheme(){try{return localStorage.getItem(`ccPatchTheme:`+(globalThis.ccPatchWsKey||``))||`default`}catch(e){return`default`}}
function ccPatchSyncWsTheme(sess,ctx){var cwd=``;try{cwd=(sess&&sess.defaultCwd&&sess.defaultCwd.value)||(ctx&&ctx.defaultCwd&&ctx.defaultCwd.value)||``}catch(e){}if(cwd===globalThis.ccPatchWsKey)return;globalThis.ccPatchWsKey=cwd;ccPatchApplyTheme(ccPatchWsTheme());try{var qa=JSON.parse(localStorage.getItem(`ccPatchQueue:`+(globalThis.ccPatchWsKey||``))||`[]`);globalThis.ccPatchQueue=Array.isArray(qa)?qa:[]}catch(e){globalThis.ccPatchQueue=[]}setTimeout(function(){globalThis.ccPatchQueueListeners.forEach(function(f){try{f()}catch(e){}})},0)}
...
function ccPatchUsageResetText(r){try{if(r==null)return``;var t=typeof r===`number`?(r<1e12?r*1000:r):new Date(r).getTime();if(!t||isNaN(t))return``;var d=t-Date.now();if(d<=0)return`resets now`;var dy=Math.floor(d/864e5),h=Math.floor(d%864e5/36e5),m=Math.floor(d%36e5/6e4),p=function(n){return n<10?`0`+n:``+n};if(dy>=1)return dy+`d `+h+`h left`;if(h>=1)return h+`h `+p(m)+`m left`;if(m>=1)return m+`m left`;return`under 1m left`}catch(e){return``}}
...
// ---- Auto-resume ("Continuation") -----------------------------------------
// Optional, default OFF. Two triggers only: (1) a usage-limit reset (5-hour or
// weekly), (2) a turn error. There is NO "resume on any stop" -- a cleanly
// finished chat is never touched. When a chat STOPS, classify WHY using the same usage
...
// it fires ONLY on a positively-detected error -- primarily the native session's
// reactive .error signal (set in a catch, cleared to void 0 at each turn start),
// or a mirror_error/isError message -- never on a normal idle.
...
function ccPatchContAnyOn(){return ccPatchContOn(`limit`)||ccPatchContOn(`error`)}
// Migration: the old menu had three separate usage toggles (usage/fiveHour/weekly).
// They are now ONE `limit` toggle (covers 5-hour AND weekly). One-time, idempotent:
// if any legacy usage toggle was on, turn `limit` on, then drop the legacy keys so
// the menu can never re-read them. `error` is unchanged and carries over as-is.
function ccPatchContMigrate(){try{if(localStorage.getItem(`ccPatchCont:migratedV2`)===`1`)return;if(ccPatchContOn(`usage`)||ccPatchContOn(`fiveHour`)||ccPatchContOn(`weekly`))ccPatchContSet(`limit`,true);try{localStorage.removeItem(`ccPatchCont:usage`);localStorage.removeItem(`ccPatchCont:fiveHour`);localStorage.removeItem(`ccPatchCont:weekly`)}catch(e){}localStorage.setItem(`ccPatchCont:migratedV2`,`1`)}catch(e){}}
...
// A real API/transient error (e.g. "Server is temporarily limiting requests
// (not your usage limit) · Rate limited", or "API Error: 500 ... status.claude.com")
// is streamed from the SDK and lands in the native session's reactive .error
// signal (rendered as the errorBanner_/errorMessage_ DOM). The exact wording/prefix
// is SERVER-supplied ("API Error" is not even a literal in the bundle), so
// ccPatchContErrorStop reads s.error.value directly -- any non-empty value is, by
// construction, a real turn error (it's only ever set in a catch). BUT that reactive
// signal does NOT survive a webview refresh, while the rendered errorBanner DOES --
// so ccPatchContErrorStop ALSO scrapes the errorBanner_/errorMessage_ nodes (not just
// messageContainer_, which never holds the banner). That refresh-proof DOM scan is the
// fix for "auto-continue did nothing after I reloaded on a rate-limit". ccPatchContErrSig
// below is the text-signature used when scraping that DOM/message text; it's distinctive
// enough not to fire on normal assistant output.
function ccPatchContErrSig(t){t=String(t||``);if(/status\.claude\.com|server-?side issue|temporarily unavailable|temporarily limiting|not your usage limit|\brate limited\b/i.test(t))return true;return /API Error\b/i.test(t)&&/Internal server error|try again|overloaded|rate limit|too many requests|\b(5\d\d|429)\b/i.test(t)}
function ccPatchContMsgErr(x){try{if(!x)return false;if(typeof x.text===`string`&&ccPatchContErrSig(x.text))return true;var c=x.content;if(typeof c===`string`&&ccPatchContErrSig(c))return true;if(Array.isArray(c)){for(var j=0;j<c.length;j++){var b=c[j];if(b&&typeof b.text===`string`&&ccPatchContErrSig(b.text))return true}}}catch(e){}return false}
// Server-supplied "transient" errors say so in plain text: "Server is temporarily
// limiting requests (not your usage limit) · Rate limited". That parenthetical is
// the SERVER itself telling us this is NOT a quota -- retrying promptly is correct
// and safe. We read it from the reactive .error signal AND (crucially, post-refresh
// when that signal is gone but the banner still renders) from the errorBanner DOM.
// When true, the tick FORCES the prompt error-retry branch even if a usage banner or
// high-usage window is also present -- so a transient 429/503 is never mis-parked for
// hours waiting on a reset that wouldn't help anyway. The errorBanner DOM is the
// ground truth the user actually sees on screen, so detecting it there is refresh-proof.
function ccPatchContTransientErr(s){function sig(t){return /not your usage limit|temporarily limiting/i.test(String(t||``))}try{var ev=s&&s.error&&s.error.value;if(typeof ev===`string`&&sig(ev))return true}catch(e){}try{var m=s&&s.messages&&s.messages.value;if(m&&m.length){for(var i=m.length-1;i>=0&&i>=m.length-3;i--){var x=m[i];if(x&&((typeof x.text===`string`&&sig(x.text))||(typeof x.content===`string`&&sig(x.content))))return true}}}catch(e){}try{if(s===ccPatchActiveSession()){var eb=document.querySelectorAll(`[class*="errorBanner_"],[class*="errorMessage_"]`);for(var b=0;b<eb.length;b++){if(eb[b]&&sig(eb[b].textContent||``))return true}}}catch(e){}return false}
function ccPatchContErrorStop(s){try{try{var ev=s.error&&s.error.value;if(typeof ev===`string`&&ev.trim())return true}catch(e){}var m=s.messages&&s.messages.value;if(m&&m.length){for(var i=m.length-1;i>=0&&i>=m.length-3;i--){var x=m[i];if(!x)continue;if(x.subtype===`mirror_error`)return true;if(x.type===`system`&&x.subtype&&/error/i.test(String(x.subtype)))return true;if(x.isError===true)return true;if(ccPatchContMsgErr(x))return true}}try{if(s===ccPatchActiveSession()){var eb=document.querySelectorAll(`[class*="errorBanner_"],[class*="errorMessage_"]`);for(var b=0;b<eb.length;b++){if(eb[b]&&ccPatchContErrSig(eb[b].textContent||``))return true}var ns=document.querySelectorAll(`[class*="messageContainer_"]`);for(var k=ns.length-1;k>=0&&k>=ns.length-5;k--){if(ns[k]&&ccPatchContErrSig(ns[k].textContent||``))return true}}}catch(e){}return false}catch(e){return false}}
function ccPatchContEligible(w){return ccPatchContOn(`limit`)}
...
// Usage-resume readiness: past the captured reset time AND the LIVE usage window is
// no longer maxed. Gate on the window (ccPatchContUsageWindow, refreshed by the usage
// poll), NOT on rateLimitWarning (ccPatchErrorReason) — that banner string lingers
// until the next turn starts, so gating resume on it deadlocks (resume waits for the
// banner to clear; the banner only clears once a turn starts; a turn only starts when
// resume fires). The window utilization drops on its own after a reset, breaking the loop.
// Positive-evidence guard: resume only when a window was actually OBSERVED maxed
// (p.sawWin) or a real resetsAt deadline (p.at) was known — never on "no window data",
// so a missing-usage-data case waits/re-checks instead of blind-resuming into a loop.
function ccPatchContReady(s,p){var now=Date.now();if(p.kind===`error`)return now>=p.at;if(p.at!=null&&now<p.at)return false;var win=null;try{win=ccPatchContUsageWindow()}catch(e){}if(win)return false;return !!(p.sawWin||p.at!=null)}
// ---- Reload-survival for errored chats ------------------------------------
// A turn error lives ONLY on the volatile session.error.value (set in a catch) and a
// developer-reload WIPES it, so a background chat parked on an error has no live signal
// afterward and auto-continue can't see it. So we persist it OURSELVES: the always-running
// tick records a session's id to localStorage the moment it shows a RELIABLE stop — a
// non-empty s.error.value OR a "you've hit your limit" rateLimitWarning (ccPatchErrorReason).
// NEVER the content scan, so a chat that merely MENTIONED a 429 is never recorded -> no
// misfire on a cleanly-finished chat. After a reload the tick ORs this back into the error
// signal, so the chat flows through the SAME usage-wait / error-retry branch using CURRENT
// signals (it still waits out a live usage window, still respects the toggles). Cleared the
// moment the session next goes busy (a turn started = handled) or after 24h; archived
// sessions are never recorded or resumed (Jacob's rule). Keyed by sessionId (stable across reload).
var ccPatchContErrTTL=864e5;
function ccPatchContErrLoad(){try{var o=JSON.parse(localStorage.getItem(`ccPatchContErr`)||`{}`);return(o&&typeof o===`object`)?o:{}}catch(e){return{}}}
function ccPatchContErrSave(o){try{localStorage.setItem(`ccPatchContErr`,JSON.stringify(o||{}))}catch(e){}}
function ccPatchContErrMark(id){if(!id)return;try{var o=ccPatchContErrLoad();if(o[id]==null){o[id]=Date.now();ccPatchContErrSave(o)}}catch(e){}}
function ccPatchContErrClear(id){if(!id)return;try{var o=ccPatchContErrLoad();if(o[id]!=null){delete o[id];ccPatchContErrSave(o)}}catch(e){}}
function ccPatchContWasErrored(id){if(!id)return false;try{var o=ccPatchContErrLoad();var t=o[id];if(t==null)return false;if(Date.now()-t>ccPatchContErrTTL){ccPatchContErrClear(id);return false}return true}catch(e){return false}}
// Branch order matters. A REAL usage limit (native rateLimitWarning "you've hit your
// limit", via ccPatchErrorReason -> hard) waits for the window's reset, then resumes.
// Resume is gated on the LIVE usage window dropping out of "maxed" (see ccPatchContReady),
// NOT on rateLimitWarning — that banner never clears until a turn starts, so gating on
// it deadlocked and the chat never resumed after the 5h reset (the bug this fixes).
// Detection stays on `hard` ONLY (a definitive limit signal): a bare errStop must NOT
// route here, because ccPatchContErrorStop can match assistant text that merely mentions
// a 429/rate-limit, and combining that with a coincidentally-maxed window would fire
// "keep going" on a cleanly-finished chat. We ARM a usage pending only when we actually
// have usage data (a maxed window OR a known resetsAt); with no data we can't tell when
// the limit lifts, so we re-check next tick rather than fire blind (which re-error-loops).
// A transient "(not your usage limit)" 429 only ESCAPES the usage-park branch when the
// LIVE usage window is NOT maxed (!(transient&&!maxed)): a transient with no maxed window
// is a real server hiccup -> quick backoff retry. But when the live window IS maxed, you
// are genuinely out of quota RIGHT NOW, so the "(not your usage limit)" parenthetical is
// incidental (or the rateLimitWarning banner is lingering from the real limit) -> we must
// park and wait for the reset, NOT fast-retry "keep going" into an input the limit has
// disabled (which silently no-ops, burns the 6/30min cap, then goes dead). The live
// window (>=95%) is the ground truth that the limit is current, not stale. (Fixed: the
// old `&&!transient` let the server's "(not your usage limit)" text divert a REAL maxed
// limit away from the working usage-resume path, so the chat never resumed.)
// globalThis.ccPatchContDbg[sessionId] holds the last decision snapshot for debugging.
function ccPatchContTick(){if(!ccPatchContAnyOn())return;var now=Date.now();var list=ccPatchContSessions();for(var i=0;i<list.length;i++){var s=list[i];if(!s)continue;var id;try{id=ccPatchSessionId(s)}catch(e){continue}var st=ccPatchContState.get(id);if(!st){st={pending:null,tries:0,winStart:now,lastSendAt:0,handled:false,idleSince:0};ccPatchContState.set(id,st)}try{var _ev=s.error&&s.error.value;var _rl=null;try{_rl=ccPatchErrorReason(s)}catch(e){}if(((typeof _ev===`string`&&_ev.trim())||_rl)&&!ccPatchIsArchived(s))ccPatchContErrMark(id)}catch(e){}var busy=false;try{busy=!!(s.busy&&s.busy.value)}catch(e){}if(busy){st.handled=false;st.pending=null;st.idleSince=0;ccPatchContErrClear(id);continue}if(ccPatchIsWaiting(s))continue;if(!st.idleSince)st.idleSince=now;if(now-st.winStart>ccPatchContWinMs){st.winStart=now;st.tries=0}if(st.pending){if(st.pending.kind===`usage`&&st.pending.at!=null&&now>=st.pending.at){try{ccPatchUsageRefresh(true)}catch(e){}}if(now-st.idleSince>=ccPatchContDwellMs&&ccPatchContReady(s,st.pending)){if(st.tries<ccPatchContMax&&now-st.lastSendAt>=ccPatchContGapMs){if(ccPatchContSend(s)){st.tries++;st.lastSendAt=now}}st.pending=null;st.handled=true}continue}if(st.handled)continue;var hard=null;try{hard=ccPatchErrorReason(s)}catch(e){}var win=null;try{win=ccPatchContUsageWindow()}catch(e){}var maxed=!!win;var errLive=false;try{errLive=ccPatchContErrorStop(s)}catch(e){}var errRec=false;try{errRec=ccPatchContWasErrored(id)&&!ccPatchIsArchived(s)}catch(e){}var errStop=errLive||errRec;var transient=false;try{transient=ccPatchContTransientErr(s)}catch(e){}var usageLimit=((hard&&maxed)||(errRec&&maxed))&&!(transient&&!maxed);var staleHard=!!(hard&&!maxed&&!errLive&&!transient);try{(globalThis.ccPatchContDbg=globalThis.ccPatchContDbg||{})[id]={t:now,hard:!!hard,maxed:maxed,errLive:errLive,errRec:errRec,transient:transient,usageLimit:usageLimit,staleHard:staleHard,tries:st.tries}}catch(e){}if(usageLimit){var w=win||`fiveHour`;if(!ccPatchContEligible(w)){st.handled=true;continue}var rt=ccPatchContResetMs(win);if(win||rt!=null){st.pending={kind:`usage`,at:(rt!=null?rt+ccPatchContPad:null),sawWin:!!win}}else{st.pending={kind:`error`,at:now+ccPatchContGapMs}}}else if(staleHard){}else if(errStop||transient){if(!ccPatchContOn(`error`)){st.handled=true;continue}var bo=transient?Math.min(ccPatchContGapMs+st.tries*4000,2e4):ccPatchContGapMs*Math.pow(2,Math.min(st.tries,4));st.pending={kind:`error`,at:now+bo}}else{st.handled=true}}}
...
// chat starts on the saved default. Empty ("Auto") => "" => unset => native
// default. Effort is still constrained at use-time by the model's
// supportedEffortLevels, so an unsupported pick simply has no visible effect on
...
// EXACTLY Claude's native ladder (V8t/W8t enum + the ultracode top): Low, Medium,
// High, Extra high (xhigh), Max (max), Ultracode (xhigh + workflows). "ultracode"
// is the only sentinel — every other id is a real native effort level.
function ccPatchEffortStops(){return[[``,`Auto`],[`low`,`Low`],[`medium`,`Medium`],[`high`,`High`],[`xhigh`,`Extra high`],[`max`,`Max`],[`ultracode`,`Ultracode`]]}
// Raw stored id (keeps the "ultracode" sentinel as-is). Drives the slider
// position + label. ccPatchDefaultEffort() coerces ONLY the ultracode sentinel to
// its native effort id (xhigh); every real level (incl. "max") passes through.
// PER-PROJECT key: each workspace (cwd) keeps its OWN default effort, so a choice
// in one project never bleeds into another. cwd is read from the active session
// (constant per VS Code window) and cached in ccPatchCwd so it resolves even during
// session construction (when no session is active yet). The legacy unkeyed value is
// still read as a fallback so existing users don't lose their setting on upgrade.
// Set always writes the key (even "" = explicit Auto) so a project pinned to Auto
// is distinguishable from "never set" (null) and won't re-read the legacy value.
function ccPatchEffortCwd(){try{if(globalThis.ccPatchCwd)return String(globalThis.ccPatchCwd);var s=ccPatchActiveSession&&ccPatchActiveSession();if(s&&s.cwd&&s.cwd.value)return String(s.cwd.value)}catch(e){}return``}
function ccPatchEffortKey(){var c=ccPatchEffortCwd();return c?`ccPatchDefaultEffort::`+c:`ccPatchDefaultEffort`}
function ccPatchDefaultEffortRaw(){try{var k=ccPatchEffortKey();var v=localStorage.getItem(k);if(v!=null)return v;if(k!==`ccPatchDefaultEffort`){var lg=localStorage.getItem(`ccPatchDefaultEffort`);if(lg!=null)return lg}}catch(e){}return``}
...
function ccPatchSetDefaultEffort(id){try{localStorage.setItem(ccPatchEffortKey(),id?String(id):``)}catch(e){}}
...
// FRESH-CHAT re-apply: on a brand-new chat (no messages yet) push THIS project's
// stored default to the host, so the ACTUAL effort matches the slider instead of
// inheriting whatever another project last wrote to the shared native effort
// setting. Gated on messages.length===0 so a conversation in progress is NEVER
// touched, and fires once per sessionId (ccPatchEffortSynced). Also caches the live
// cwd for the per-project key. Fully defensive — any throw is swallowed.
function ccPatchEffortSyncTick(){try{var s=ccPatchActiveSession&&ccPatchActiveSession();if(!s)return;try{if(s.cwd&&s.cwd.value)globalThis.ccPatchCwd=String(s.cwd.value)}catch(e){}var sid=ccPatchSessionKey(s);if(!sid)return;var msgs=s.messages&&s.messages.value;if(!msgs||msgs.length)return;var seen=globalThis.ccPatchEffortSynced||(globalThis.ccPatchEffortSynced={});if(seen[sid])return;seen[sid]=1;var raw=ccPatchDefaultEffortRaw();var wantUC=(raw===`ultracode`);var wantEff=wantUC?`xhigh`:(raw||``);var curEff=(s.effortLevel&&s.effortLevel.value)||``;var curUC=!!(s.ultracodeEnabled&&s.ultracodeEnabled.value);if(curEff!==wantEff||curUC!==wantUC)ccPatchApplyDefaultEffort(raw)}catch(e){}}
...
function ccPatchBuildEffortSlider(labelEl,onChange,initialId){var stops=ccPatchEffortStops(),n=stops.length;var wrap=document.createElement(`div`);wrap.className=`ccPatchEffWrap`;var track=document.createElement(`div`);track.className=`ccPatchEffTrack`;var fill=document.createElement(`div`);fill.className=`ccPatchEffFill`;var thumb=document.createElement(`div`);thumb.className=`ccPatchEffThumb`;track.appendChild(fill);for(var i=0;i<n;i++){var nt=document.createElement(`div`);nt.className=`ccPatchEffNotch`;if(stops[i][0]===`ultracode`)nt.classList.add(`ccPatchEffNotchUltra`);track.appendChild(nt)}track.appendChild(thumb);wrap.appendChild(track);function render(idx){var pct=n>1?idx/(n-1)*100:0;thumb.style.left=pct+`%`;fill.style.width=pct+`%`;if(labelEl)labelEl.textContent=stops[idx][1];wrap.classList.toggle(`ccPatchEffUltra`,stops[idx][0]===`ultracode`)}function setIdx(idx,persist){idx=Math.max(0,Math.min(n-1,idx));render(idx);if(persist){if(onChange)onChange(stops[idx][0]);else ccPatchSetDefaultEffort(stops[idx][0])}}function pctFromX(cx){var r=track.getBoundingClientRect();var f=r.width>0?(cx-r.left)/r.width:0;return Math.max(0,Math.min(1,f))*100}function idxFromX(cx){return Math.round(pctFromX(cx)/100*(n-1))}function renderRaw(pct,idx){thumb.style.left=pct+`%`;fill.style.width=pct+`%`;if(labelEl)labelEl.textContent=stops[idx][1];wrap.classList.toggle(`ccPatchEffUltra`,stops[idx][0]===`ultracode`)}track.classList.add(`ccPatchEffDragging`);setIdx(ccPatchEffortIndex(onChange?(initialId||``):ccPatchDefaultEffortRaw()),false);requestAnimationFrame(function(){track.classList.remove(`ccPatchEffDragging`)});track.onmousedown=function(e){e.preventDefault();e.stopPropagation();setIdx(idxFromX(e.clientX),true);var mv=function(g){track.classList.add(`ccPatchEffDragging`);renderRaw(pctFromX(g.clientX),idxFromX(g.clientX))};var up=function(g){document.removeEventListener(`pointermove`,mv);document.removeEventListener(`pointerup`,up);track.classList.remove(`ccPatchEffDragging`);setIdx(idxFromX((g&&g.clientX!=null)?g.clientX:e.clientX),true);if(!onChange){try{ccPatchApplyDefaultEffort(ccPatchDefaultEffortRaw())}catch(e){}}};document.addEventListener(`pointermove`,mv);document.addEventListener(`pointerup`,up)};return wrap}
...
function ccPatchAddCopyBtn(el){try{if(!el)return;if(el.querySelector(`:scope > .ccPatchCopyBtn`)){if(!el.hasAttribute(`data-cc-copy`))el.setAttribute(`data-cc-copy`,`1`);return}if(el.querySelector(`[class*="userMessageContainer_"],[class*="messageContainer_"]`)){el.setAttribute(`data-cc-copy`,`w`);return}el.setAttribute(`data-cc-copy`,`1`);var cs=getComputedStyle(el);if(cs&&cs.position===`static`)el.style.position=`relative`;var b=document.createElement(`button`);b.type=`button`;b.className=`ccPatchCopyBtn`;b.title=`Copy message`;b.innerHTML=`<svg width="13" height="13" viewBox="0 0 14 14" fill="none" stroke="currentColor" stroke-width="1.3" stroke-linecap="round" stroke-linejoin="round"><rect x="4.5" y="4.5" width="7.5" height="7.5" rx="1.5"/><path d="M9.5 4.5V3a1.5 1.5 0 0 0-1.5-1.5H3A1.5 1.5 0 0 0 1.5 3v5A1.5 1.5 0 0 0 3 9.5h1.5"/></svg>`;b.onmousedown=function(ev){ev.preventDefault();ev.stopPropagation()};b.onclick=function(ev){ev.preventDefault();ev.stopPropagation();var t=ccPatchMsgText(el);try{if(navigator.clipboard&&navigator.clipboard.writeText)navigator.clipboard.writeText(t);else{var ta=document.createElement(`textarea`);ta.value=t;document.body.appendChild(ta);ta.select();try{document.execCommand(`copy`)}catch(e){}ta.remove()}}catch(e){}b.classList.add(`ccPatchCopied`);b.title=`Copied`;setTimeout(function(){b.classList.remove(`ccPatchCopied`);b.title=`Copy message`},1200)};el.appendChild(b)}catch(e){}}
// PERF: the fast sweep (every 1.5s) only touches containers NOT yet stamped
// data-cc-copy, so steady-state cost is O(new messages), not O(whole chat).
// Every 20th tick runs a FULL pass (all containers) as a self-heal: if a React
// re-render strips an appended button but keeps the stamped element, the full
// pass re-adds it within ~30s. Wrappers (containers holding other containers)
// are stamped "w" once and never rescanned by the fast pass — previously their
// full-subtree nested-container query re-ran every 1.5s forever.
function ccPatchCopySweep(full){try{var sel=full?`[class*="userMessageContainer_"],[class*="messageContainer_"]`:`[class*="userMessageContainer_"]:not([data-cc-copy]),[class*="messageContainer_"]:not([data-cc-copy])`;var nodes=document.querySelectorAll(sel);for(var i=0;i<nodes.length;i++)ccPatchAddCopyBtn(nodes[i])}catch(e){}}
// Background task-notifications are NATIVE Claude Code: when a command runs in
// the background, its completion is fed back into the chat as a <task-notification>
// user-role message. Raw, it reads like a debug log the user typed. We tag those
// bubbles (data-attr survives React re-renders; :not() keeps the scan O(new nodes))
// and pull the <summary> so CSS can render them as a tidy "Background task" card.
function ccPatchTaskNoteSweep(){try{var ns=document.querySelectorAll(`[class*="userMessageContainer_"]:not([data-cc-tnk])`);for(var i=0;i<ns.length;i++){var el=ns[i];el.setAttribute(`data-cc-tnk`,`1`);var tx=el.textContent||``;if(tx.indexOf(`<task-notification`)!==-1||(tx.indexOf(`<task-id>`)!==-1&&tx.indexOf(`<tool-use-id>`)!==-1)){el.setAttribute(`data-cc-tasknote`,`1`);var sm=tx.match(/<summary>([\s\S]*?)<\/summary>/);el.setAttribute(`data-cc-tasknote-label`,sm&&sm[1]?sm[1].trim().slice(0,160):`Background task completed`)}}}catch(e){}}
// ---- "Default" popup (new-chat defaults: model · effort · YOLO) -----------
// Jacob's gear ▸ "Default" (replaces "Switch model"): an Orbit-coded popup that
// sets what every NEW chat opens on — default MODEL, default EFFORT (same ladder
// as the inline slider), and whether new chats START on YOLO mode. Applied by
// ccPatchNewChatDefaultsTick on a FRESH chat (mirrors the effort tick): model via
// session.setModel, YOLO via ccPatchAutoApproveSet under the stable ccPatchYoloKey
// (internalId, always present — no need to wait for a channel). Model list = the account's
// claudeConfig.models; an unavailable pick just doesn't apply (native keeps its
// own default — the natural "downgrade").
function ccPatchDefaultModelGet(){try{return localStorage.getItem(`ccPatchDefaultModel`)||``}catch(e){return``}}
function ccPatchDefaultModelSet(v){try{localStorage.setItem(`ccPatchDefaultModel`,v?String(v):``)}catch(e){}}
function ccPatchDefaultModeGet(){try{var v=localStorage.getItem(`ccPatchDefaultMode`);if(v!=null)return v;if(localStorage.getItem(`ccPatchYoloDefault`)===`1`)return`bypassPermissions`}catch(e){}return``}
function ccPatchDefaultModeSet(v){try{localStorage.setItem(`ccPatchDefaultMode`,v?String(v):``)}catch(e){}}
// The native permission modes a new chat can default to (value -> label), same
// set as the composer Modes menu. "" = leave Claude's own default.
function ccPatchModeStops(){return[[``,`Leave default`],[`default`,`Ask before edits`],[`acceptEdits`,`Edit automatically`],[`plan`,`Plan mode`],[`auto`,`Auto mode`],[`bypassPermissions`,`YOLO mode`]]}
function ccPatchModeLabel(v){var st=ccPatchModeStops();for(var i=0;i<st.length;i++)if(st[i][0]===(v||``))return st[i][1];return st[0][1]}
function ccPatchModelCfg(s){try{if(!s)return null;if(s.claudeConfig&&s.claudeConfig.value)return s.claudeConfig.value;if(s.connection&&s.connection.value&&s.connection.value.claudeConfig&&s.connection.value.claudeConfig.value)return s.connection.value.claudeConfig.value}catch(e){}return null}
function ccPatchAvailableModels(){try{var s=ccPatchActiveSession&&ccPatchActiveSession();var c=ccPatchModelCfg(s);var m=c&&c.models;return Array.isArray(m)?m:[]}catch(e){return[]}}
function ccPatchDefaultModelLabel(){try{var v=ccPatchDefaultModelGet();if(!v)return`Account default`;var m=ccPatchAvailableModels();for(var i=0;i<m.length;i++){if(m[i]&&m[i].value===v)return m[i].displayName||v}return v}catch(e){return`Account default`}}
function ccPatchApplyDefaultModel(s){try{s=s||(ccPatchActiveSession&&ccPatchActiveSession());if(!s||typeof s.setModel!==`function`)return;var v=ccPatchDefaultModelGet();if(!v)return;var cur=(s.modelSelection&&s.modelSelection.value)||``;if(cur===v)return;var m=ccPatchAvailableModels();for(var i=0;i<m.length;i++){if(m[i]&&m[i].value===v){s.setModel(m[i]);return}}}catch(e){}}
// Capture default intent the first time a session is seen FRESH (0 messages),
// then apply — model + permission MODE right away; for YOLO mode
// (bypassPermissions) also set the per-chat auto-approve under the stable
// ccPatchYoloKey. Opening an EXISTING chat (messages already there) is never
// force-defaulted.
function ccPatchNewChatDefaultsTick(){try{var s=ccPatchActiveSession&&ccPatchActiveSession();if(!s)return;var sid=ccPatchSessionKey&&ccPatchSessionKey(s);if(!sid)return;var seen=globalThis.ccPatchDefSeen||(globalThis.ccPatchDefSeen={});var msgs=s.messages&&s.messages.value;var fresh=!msgs||msgs.length===0;if(seen[sid]===void 0)seen[sid]={mode:fresh?(ccPatchDefaultModeGet()||``):``,modelDone:false,modeDone:false,yoloDone:false};var st=seen[sid];if(!st.modelDone&&fresh){st.modelDone=true;try{ccPatchApplyDefaultModel(s)}catch(e){}}if(st.mode&&!st.modeDone&&fresh){st.modeDone=true;try{if(typeof s.setPermissionMode===`function`)s.setPermissionMode(st.mode,true)}catch(e){}}if(st.mode===`bypassPermissions`&&!st.yoloDone){var yk=ccPatchYoloKey(s);if(yk){try{ccPatchAutoApproveSet(yk,true)}catch(e){}st.yoloDone=true}}}catch(e){}}
// Custom dropdown (nicer than a native <select>): a button showing the current
// label + a styled list that toggles open. onPick(value) fires on choice.
function ccPatchBuildSelect(current,options,onPick){var wrap=document.createElement(`div`);wrap.className=`ccPatchSel`;var cur=current;function labelFor(v){for(var i=0;i<options.length;i++)if(options[i][0]===v)return options[i][1];return options.length?options[0][1]:``}var btn=document.createElement(`button`);btn.type=`button`;btn.className=`ccPatchSelBtn`;var lbl=document.createElement(`span`);lbl.className=`ccPatchSelLabel`;lbl.textContent=labelFor(cur);btn.appendChild(lbl);var chev=document.createElement(`span`);chev.className=`ccPatchSelChev`;chev.innerHTML=`<svg width="11" height="11" viewBox="0 0 12 12" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"><polyline points="3,4.5 6,7.5 9,4.5"/></svg>`;btn.appendChild(chev);var menu=document.createElement(`div`);menu.className=`ccPatchSelMenu`;menu.style.display=`none`;options.forEach(function(o){var it=document.createElement(`div`);it.className=`ccPatchSelItem`+(o[0]===cur?` ccPatchSelItemOn`:``);it.textContent=o[1];it.onmousedown=function(e){e.preventDefault();e.stopPropagation();cur=o[0];lbl.textContent=o[1];menu.style.display=`none`;wrap.classList.remove(`ccPatchSelOpen`);for(var j=0;j<menu.children.length;j++)menu.children[j].classList.remove(`ccPatchSelItemOn`);it.classList.add(`ccPatchSelItemOn`);try{onPick(o[0])}catch(err){}};menu.appendChild(it)});btn.onmousedown=function(e){e.preventDefault();e.stopPropagation();var open=menu.style.display===`none`;menu.style.display=open?`block`:`none`;wrap.classList.toggle(`ccPatchSelOpen`,open)};wrap.appendChild(btn);wrap.appendChild(menu);wrap.ccSet=function(v){cur=v;lbl.textContent=labelFor(v);for(var q=0;q<menu.children.length;q++)menu.children[q].classList.toggle(`ccPatchSelItemOn`,!!(options[q]&&options[q][0]===v))};return wrap}
function ccPatchDefRow(labelText,control){var d=document.createElement(`div`);d.className=`ccPatchDefRow`;var l=document.createElement(`span`);l.className=`ccPatchDefRowLabel`;l.textContent=labelText;d.appendChild(l);d.appendChild(control);return d}
function ccPatchCloseDefaultsMenu(){var m=document.querySelector(`.ccPatchDefaultsOverlay`);if(m){if(m._ccPatchKey)document.removeEventListener(`keydown`,m._ccPatchKey);m.remove()}}
// Centered modal. Model + Mode are custom dropdowns, Effort the slider; all three
// STAGE changes and only commit on Save (disabled until something changes), so a
// stray drag never sticks. Picking Mode = YOLO asks a confirm (reverts on cancel).
// Auto-resume + Copy diagnostics live here too (moved out of the gear menu). The
// Escape handler is guarded so closing the confirm doesn't also close the modal.
function ccPatchShowDefaultsMenu(){if(document.querySelector(`.ccPatchDefaultsOverlay`)){ccPatchCloseDefaultsMenu();return}try{ccPatchCloseSettingsMenu()}catch(e){}var ctx=globalThis.ccPatchCtx;var ov=document.createElement(`div`);ov.className=`ccPatchDefaultsOverlay`;var card=document.createElement(`div`);card.className=`ccPatchDefaultsCard`;ov.appendChild(card);var head=document.createElement(`div`);head.className=`ccPatchDefHead`;head.textContent=`Default for new chats`;card.appendChild(head);var sub=document.createElement(`div`);sub.className=`ccPatchDefSub`;sub.textContent=`What every new chat opens on. Chats already going keep their own.`;card.appendChild(sub);var saved={model:ccPatchDefaultModelGet(),mode:ccPatchDefaultModeGet(),effort:ccPatchDefaultEffortRaw(),arLimit:ccPatchContOn(`limit`),arError:ccPatchContOn(`error`)};var staged={model:saved.model,mode:saved.mode,effort:saved.effort,arLimit:saved.arLimit,arError:saved.arError};var saveBtn;function dirty(){return staged.model!==saved.model||staged.mode!==saved.mode||staged.effort!==saved.effort||staged.arLimit!==saved.arLimit||staged.arError!==saved.arError}function refreshSave(){if(saveBtn){var d=dirty();saveBtn.disabled=!d;saveBtn.classList.toggle(`ccPatchSaveOn`,d)}}var models=ccPatchAvailableModels();var modelOpts=[[``,`Account default`]];for(var i=0;i<models.length;i++){if(models[i]&&models[i].value)modelOpts.push([models[i].value,models[i].displayName||models[i].value])}card.appendChild(ccPatchDefRow(`Model`,ccPatchBuildSelect(staged.model,modelOpts,function(v){staged.model=v;refreshSave()})));var modeSel=ccPatchBuildSelect(staged.mode,ccPatchModeStops(),function(v){if(v===`bypassPermissions`){ccPatchYoloConfirm(function(){staged.mode=v;refreshSave()},function(){modeSel.ccSet(staged.mode)})}else{staged.mode=v;refreshSave()}});card.appendChild(ccPatchDefRow(`Mode`,modeSel));(function(){var w=document.createElement(`div`);w.className=`ccPatchDefEffWrap`;var vl=document.createElement(`span`);vl.className=`ccPatchDefEffVal`;w.appendChild(vl);w.appendChild(ccPatchBuildEffortSlider(vl,function(id){staged.effort=id;refreshSave()},staged.effort));card.appendChild(ccPatchDefRow(`Effort`,w))})();(function(){var parentCb,c1,c2;function any(){return!!(staged.arLimit||staged.arError)}function mkRow(cls,label,onTog){var d=document.createElement(`div`);d.className=`ccPatchDefToggleRow `+cls;var l=document.createElement(`span`);l.className=`ccPatchDefToggleLabel`;l.textContent=label;d.appendChild(l);var lb=document.createElement(`span`);lb.className=`ccPatchYoloToggle`;var cb=document.createElement(`input`);cb.type=`checkbox`;var sl=document.createElement(`span`);sl.className=`ccPatchYoloSlider`;lb.appendChild(cb);lb.appendChild(sl);d.appendChild(lb);d.onmousedown=function(e){e.preventDefault();e.stopPropagation();onTog()};card.appendChild(d);return cb}parentCb=mkRow(`ccPatchDefToggleParent`,`Auto-resume`,function(){var nv=!parentCb.checked;staged.arLimit=nv;staged.arError=nv;parentCb.checked=nv;c1.checked=nv;c2.checked=nv;refreshSave()});c1=mkRow(`ccPatchDefToggleChild`,`When my usage limit resets`,function(){var nv=!c1.checked;staged.arLimit=nv;c1.checked=nv;parentCb.checked=any();refreshSave()});c2=mkRow(`ccPatchDefToggleChild`,`If the chat errors out`,function(){var nv=!c2.checked;staged.arError=nv;c2.checked=nv;parentCb.checked=any();refreshSave()});parentCb.checked=any();c1.checked=staged.arLimit;c2.checked=staged.arError})();(function(){var row=document.createElement(`div`);row.className=`ccPatchDefSaveRow`;saveBtn=document.createElement(`button`);saveBtn.type=`button`;saveBtn.className=`ccPatchSaveBtn`;saveBtn.textContent=`Save`;saveBtn.disabled=true;saveBtn.onmousedown=function(e){e.preventDefault();e.stopPropagation();if(!dirty())return;ccPatchDefaultModelSet(staged.model);ccPatchDefaultModeSet(staged.mode);ccPatchSetDefaultEffort(staged.effort);ccPatchContSet(`limit`,staged.arLimit);ccPatchContSet(`error`,staged.arError);saved={model:staged.model,mode:staged.mode,effort:staged.effort,arLimit:staged.arLimit,arError:staged.arError};refreshSave();saveBtn.textContent=`Saved`;setTimeout(function(){if(saveBtn)saveBtn.textContent=`Save`},900)};row.appendChild(saveBtn);card.appendChild(row)})();var divEl=document.createElement(`div`);divEl.className=`ccPatchDefDivider`;card.appendChild(divEl);(function(){var d=document.createElement(`div`);d.className=`ccPatchDefActionRow`;var l=document.createElement(`span`);l.textContent=`Copy diagnostics`;d.appendChild(l);d.onmousedown=function(e){e.preventDefault();e.stopPropagation();try{ccPatchCopyDiagnostics(ctx||globalThis.ccPatchCtx)}catch(er){}ccPatchCloseDefaultsMenu()};card.appendChild(d)})();document.body.appendChild(ov);var k=function(g){if(g.key===`Escape`&&!document.querySelector(`.ccPatchConfirmOverlay`))ccPatchCloseDefaultsMenu()};ov._ccPatchKey=k;document.addEventListener(`keydown`,k);ov.onmousedown=function(g){if(g.target===ov)ccPatchCloseDefaultsMenu()};card.onmousedown=function(g){g.stopPropagation()}}
Object.assign(globalThis,{ccPatchTitle,ccPatchImgPreviewShow,ccPatchImgPreviewHide,ccPatchGetSS,ccPatchSetSS,ccPatchIsArchived,ccPatchIsPinned,ccPatchIsStarred,ccPatchToggleArchive,ccPatchTogglePin,ccPatchToggleStar,ccPatchRecency,ccPatchSortSessions,ccPatchSessionId,ccPatchTrackSessionStatus,ccPatchClearDone,ccPatchIsWaiting,ccPatchErrorReason,ccPatchSessionIndicator,ccPatchActivityText,ccPatchCloseMenu,ccPatchShowMenu,ccPatchTogglePane,ccPatchSetSearch,ccPatchToggleSearch,ccPatchStartResize,ccPatchFilterListeners,ccPatchAgeMsMap,ccPatchDefaultFilters,ccPatchReadFilters,ccPatchIsUntitledEmpty,ccPatchWriteFilters,ccPatchFiltersActive,ccPatchSessionMatchesFilters,ccPatchFilterSort,ccPatchCloseFilterMenu,ccPatchFilterIconSVG,ccPatchShowFilterMenu,ccPatchCloseSettingsMenu,ccPatchShowSettingsMenu,ccPatchAutoApproveMap,ccPatchAutoApproveOn,ccPatchAutoApproveSet,ccPatchYoloKey,ccPatchAutoApproveOnChannel,ccPatchYoloChanRecord,ccPatchYoloIsAsk,ccPatchYoloConfirm,ccPatchYoloConfirmedGet,ccPatchRpc,ccPatchOpenClaudeMd,ccPatchReadClaudeMd,ccPatchWriteClaudeMd,ccPatchInstructionsModal,ccPatchSwitchAccount,ccPatchSwitchAccountModal,ccPatchAcctRpc,ccPatchAcctNormUsage,ccPatchAcctInitials,ccPatchAcctBar,ccPatchAcctBars,ccPatchAcctUsageFor,ccPatchAcctRow,ccPatchShowAccountsMenu,ccPatchCloseAccountsMenu,ccPatchSendMenu,ccPatchCloseSendMenu,ccPatchTriggerSend,ccPatchQueueForm,ccPatchStopAndSendForm,ccPatchComposerForm,ccPatchComposerAction,ccPatchRenderQueue,ccPatchSetSendMode,ccPatchQueueAdd,ccPatchQueueRemove,ccPatchQueueEdit,ccPatchQueueCommitEdit,ccPatchQueueCancelEdit,ccPatchQueueSendNow,ccPatchQueueTick,ccPatchActiveSession,ccPatchSessionKey,ccPatchIsBusy,ccPatchQueueAddFull,ccPatchThemeModal,ccPatchApplyTheme,ccPatchSyncWsTheme,ccPatchWsTheme,ccPatchColorPalette,ccPatchColorHex,ccPatchGetColor,ccPatchSetColor,ccPatchShowColorMenu,ccPatchCloseColorMenu,ccPatchStickyOn,ccPatchStickyToggle,ccPatchUsageConn,ccPatchUsageData,ccPatchUsageRefresh,ccPatchUsageColor,ccPatchUsageResetText,ccPatchUsageEsc,ccPatchUsageRing,ccPatchUsageBtnIcon,ccPatchUsageWindows,ccPatchUsageRenderBody,ccPatchCloseUsageMenu,ccPatchShowUsageMenu,ccPatchUsageStartPolling,ccPatchContOn,ccPatchContSet,ccPatchContAnyOn,ccPatchContTick,ccPatchShowContinuationMenu,ccPatchCloseContMenu,ccPatchEffortStops,ccPatchEffortCwd,ccPatchEffortKey,ccPatchEffortSyncTick,ccPatchDefaultEffort,ccPatchDefaultEffortRaw,ccPatchDefaultUltracode,ccPatchSetDefaultEffort,ccPatchEffortIndex,ccPatchDefaultEffortLabel,ccPatchBuildEffortSlider,ccPatchDefaultModelGet,ccPatchDefaultModelSet,ccPatchDefaultModeGet,ccPatchDefaultModeSet,ccPatchModeStops,ccPatchModeLabel,ccPatchBuildSelect,ccPatchDefRow,ccPatchModelCfg,ccPatchAvailableModels,ccPatchDefaultModelLabel,ccPatchApplyDefaultModel,ccPatchNewChatDefaultsTick,ccPatchShowDefaultsMenu,ccPatchCloseDefaultsMenu,ccPatchMsgText,ccPatchAddCopyBtn,ccPatchCopySweep,ccPatchTaskNoteSweep,ccPatchCopyDiagnostics,ccPatchBuildDiag,ccPatchDiagRpc,ccPatchToast,ccPatchClip});try{globalThis.ccOrbitCopyDiagnostics=function(){return ccPatchCopyDiagnostics(globalThis.ccPatchCtx)}}catch(ccdg){}
...
try{ccPatchContMigrate()}catch(e){}
try{if(!globalThis.ccPatchContTimer)globalThis.ccPatchContTimer=setInterval(ccPatchContTick,4000)}catch(e){}
try{if(!globalThis.ccPatchCopyTimer){globalThis.ccPatchCopyTickN=0;globalThis.ccPatchCopyTimer=setInterval(function(){globalThis.ccPatchCopyTickN++;ccPatchCopySweep(globalThis.ccPatchCopyTickN%20===0)},1500)}}catch(e){}
try{if(!globalThis.ccPatchTaskNoteTimer){ccPatchTaskNoteSweep();globalThis.ccPatchTaskNoteTimer=setInterval(ccPatchTaskNoteSweep,1500)}}catch(e){}
// PERF: was 7s and hit listRemoteSessions (a network call) every tick. 25s keeps
// the pane fresh for external/new sessions (live sessions update via signals, not
// this refresh); the remote list rides every OTHER tick (50s); a hidden webview
// (document.hidden) skips the tick entirely.
try{if(!globalThis.ccPatchAutoRefreshTimer){globalThis.ccPatchAutoRefreshN=0;globalThis.ccPatchAutoRefreshTimer=setInterval(function(){try{if(document.hidden)return;var r=globalThis.ccPatchSessionsRoot;if(r&&!document.documentElement.classList.contains(`ccPatchPaneHidden`)){globalThis.ccPatchAutoRefreshN++;if(r.listSessions)r.listSessions();if(r.listRemoteSessions&&globalThis.ccPatchAutoRefreshN%2===0)r.listRemoteSessions()}}catch(e){}},25000)}}catch(e){}
try{if(!globalThis.ccPatchEffortTimer)globalThis.ccPatchEffortTimer=setInterval(ccPatchEffortSyncTick,1200)}catch(e){}
try{if(!globalThis.ccPatchDefaultsTimer)globalThis.ccPatchDefaultsTimer=setInterval(ccPatchNewChatDefaultsTick,1200)}catch(e){}
...
def _capture_react_aliases(text: str) -> dict[str, str]:
    """Capture the module-global React primitives (CC 2.1.186+ rebundle).

    CC 2.1.186 moved the webview to React's automatic JSX runtime and dropped
    the namespace-style access the patcher used to rely on. Hooks, forwardRef
    and createElement are now bare, scope-hoisted top-level aliases:

        useState   -> e.g. `oe`  (`oe=function(e){return cf.current.useState(e)}`)
        useRef     -> e.g. `Ie`
        useCallback-> e.g. `Vt`
        useEffect  -> e.g. `pe`
        forwardRef -> e.g. `Jt`  (`Jt=function(e){return{$$typeof:..,render:e}}`)
        createElement (classic) -> e.g. `OQe` (the lone `arguments.length-2` factory)

    Rendering itself uses the auto-runtime (`b`=jsx single-child, `E`=jsxs
    static-children, `Oe`=Fragment), but those names are reused as locals all
    over the bundle, so we capture the jsx fn PER-ANCHOR from stable context
    instead of globally. The primitives above ARE module-global and safe to
    capture once here. We then inject a synthetic `ccReact` namespace object so
    the existing emission (`{S0}.default.createElement(...)`, `{S0}.useState(...)`)
    keeps working byte-for-byte — only the read anchors are re-derived.
    """

    def grab(pattern: str, label: str):
        m = re.search(pattern, text)
        if m is None:
            raise RuntimeError(f"Could not capture React alias: {label}")
        return m

    # createElement (classic React.createElement): the only factory that
    # collects variadic children via `arguments.length-2`.
    m_ce = grab(
        rf"function (?P<ce>{JS_ID})\({JS_ID},{JS_ID},{JS_ID}\)\{{"
        rf"var {JS_ID},{JS_ID}=\{{\}},{JS_ID}=null,{JS_ID}=null;if\(",
        "createElement",
    )
    if "arguments.length-2" not in text[m_ce.start() : m_ce.start() + 400]:
        raise RuntimeError("createElement candidate missing arguments.length-2 body")

    m_us = grab(rf"(?P<us>{JS_ID})=function\((?P<a>{JS_ID})\)\{{return {JS_ID}\.current\.useState\((?P=a)\)\}}", "useState")
    m_ur = grab(rf"(?P<ur>{JS_ID})=function\((?P<a>{JS_ID})\)\{{return {JS_ID}\.current\.useRef\((?P=a)\)\}}", "useRef")
    m_uc = grab(rf"(?P<uc>{JS_ID})=function\((?P<a>{JS_ID}),(?P<b>{JS_ID})\)\{{return {JS_ID}\.current\.useCallback\((?P=a),(?P=b)\)\}}", "useCallback")
    m_ue = grab(rf"(?P<ue>{JS_ID})=function\((?P<a>{JS_ID}),(?P<b>{JS_ID})\)\{{return {JS_ID}\.current\.useEffect\((?P=a),(?P=b)\)\}}", "useEffect")
    m_fwd = grab(rf"(?P<fwd>{JS_ID})=function\((?P<e>{JS_ID})\)\{{return\{{\$\$typeof:{JS_ID},render:(?P=e)\}}\}}", "forwardRef")
    ...
    # Fragment (best-effort — only used if a native read anchor needs it; no
    # emission references it). Capture off the react.fragment symbol.
    m_fragsym = re.search(rf"(?P<sym>{JS_ID})=Symbol\.for\(\"react\.fragment\"\)", text)
    if m_fragsym:
        m_frag = re.search(rf"(?P<frag>{JS_ID})={re.escape(m_fragsym.group('sym'))}[,;]", text)
        if m_frag:
            al["frag"] = m_frag.group("frag")
    return al


def _find_obj_var(text: str, required: "list[str]") -> str | None:
    """Return the minified name of a `var <id>={...}` object literal whose body
    contains ALL of the ``required`` substrings (e.g. CSS-module class maps like
    `mn`/`h6`/`ws`). Brace-walks each candidate so nested objects are handled."""
    for m in re.finditer(rf"\bvar\s+(?P<id>{JS_ID})=\{{", text):
        ...
        if end_idx < 0:
            continue
        body = text[i:end_idx]
        if all(key in body for key in required):
            return m.group("id")
    return None


def _capture_rs_component(text: str, al: "dict[str, str]") -> dict[str, str]:
    ...
    Returns a dict mapping logical-name → captured minified identifier:
      {fn, S0, w2, _R0, wR0, localSessions, localSessionsLoaded,
       ...
       H, B, q, z}
    """
    # 1. The session-list CSS class-name object (`mn`, was `w2`). Captured by
    #    content — the keys are stable across releases — instead of the old
    #    `;var _R0=16,wR0=1000;` timing-constant walk-back, which CC 2.1.186 no
    #    longer ships beside this object.
    mn = _find_obj_var(
        text, ['root:"', 'sessionItem:"', 'sessionsList:"', 'sessionName:"', 'sessionMeta:"', 'actionButton:"']
    )
    if mn is None:
        raise RuntimeError("Could not find sessions-list class-name object (mn)")

    US, UR, UC = al["us"], al["ur"], al["uc"]
    ...
    m_body = re.search(
        ...
        rf"\[(?P<tab>{JS_ID}),(?P<setTab>{JS_ID})\]={re.escape(US)}\(\"local\"\),"
        rf"\[(?P<w>{JS_ID}),(?P<O>{JS_ID})\]={re.escape(US)}\(0\),"
        rf"\[(?P<N>{JS_ID}),(?P<E>{JS_ID})\]={re.escape(US)}\(null\),"
        rf"\[(?P<y>{JS_ID}),(?P<x>{JS_ID})\]={re.escape(US)}\(\"\"\),"
        rf"\[(?P<p>{JS_ID}),(?P<o>{JS_ID})\]={re.escape(US)}\(null\),"
        rf"(?P<refMap>{JS_ID})={re.escape(UR)}\(new Map\),"
        rf"(?P<inputRef>{JS_ID})={re.escape(UR)}\(null\),"
        rf"(?P<s>{JS_ID})={re.escape(UC)}\(",
    ...
    S0 = "ccReact"  # emission goes through the injected ccReact namespace shim
    ...
    m_rest = re.search(
        rf",(?P<K1>{JS_ID})={re.escape(UC)}\("
        rf".*?,(?P<H1>{JS_ID})={re.escape(UC)}\("
    ...
    _1 = m_rest.group("_1")
    m_sort = re.search(
        rf"\}}\):{re.escape(_1)},(?P<t>{JS_ID})={re.escape(UR)}\((?P<F>{JS_ID})\);",
        text[body_start : body_start + 6000],
    )
    if m_sort is None:
        raise RuntimeError("Could not find Rs sort anchor `}):<_1>,t=useRef(F);`")
...
def _capture_or0_component(text: str, al: "dict[str, str]", w2: str) -> dict[str, str]:
    """Capture the inner session-row forwardRef (`OR0`, now `zLt`).

    Returns a dict with the row component identifier and every destructured
    param the patch references inside the row body.
    """
    FWD, UR = al["fwd"], al["ur"]
    # `var zLt=Jt(function({session:t,isActive:i,...,currentCwd:p},f){`  (CC 2.1.186:
    #  forwardRef is the bare alias `Jt`, no longer `S0.default.forwardRef`).
    m = re.search(
        rf"var\s+(?P<OR0>{JS_ID})={re.escape(FWD)}\(function\(\{{"
    ...
    # Status hook prelude: `zn();let g=Ie(null),v=Ie(!0);`
    m_hook = re.search(
        rf"(?P<init>{JS_ID})\(\);"
        rf"let\s+(?P<j>{JS_ID})={re.escape(UR)}\(null\),"
        rf"(?P<D>{JS_ID})={re.escape(UR)}\(!0\);",
        body,
    )
    if m_hook is None:
    ...
    # Title fn (`MM`, was Kk) and search-highlight fn (`iQe`, was Le1) from the
    # non-renaming name span — now jsx-runtime, children-in-props:
    # `}):b("span",{className:mn.sessionName,children:iQe(MM(t),r)}),`
    session = m.group("session")
    isRenaming = m.group("isRenaming")
    searchQuery = m.group("searchQuery")
    m_titlefn = re.search(
        rf"\}}\):{JS_ID}\(\"span\",\{{className:{re.escape(w2)}\.sessionName,"
        rf"children:(?P<Le1>{JS_ID})\((?P<Kk>{JS_ID})\({re.escape(session)}\),{re.escape(searchQuery)}\)\}}\),",
        body,
    )
    if m_titlefn is None:
        raise RuntimeError("Could not find OR0 title/highlight fn names")

    # Rename and delete icon components from the action buttons (jsx-runtime):
    # `title:"Rename session",children:b(sq,{className:mn.actionIcon})`
    m_renameicon = re.search(
        rf'title:"Rename session",children:{JS_ID}\((?P<sa>{JS_ID}),'
        rf"\{{className:{re.escape(w2)}\.actionIcon\}}\)",
        body,
    )
    m_delicon = re.search(
        rf'title:"Delete session",children:{JS_ID}\((?P<Ne1>{JS_ID}),'
        rf"\{{className:{re.escape(w2)}\.actionIcon\}}\)",
        body,
    )
...
def _capture_fe1_component(text: str, al: "dict[str, str]") -> dict[str, str | int]:
    """Capture the top-level chat-tab container (`fe1`, now `dQe`) and its locals.
    ...
    # fe1 signature (CC 2.1.186, bare hooks):
    # `function dQe({sessions:e,context:t}){zn();let i=Ie(null),n=Ie(null),o=Ie(null),r=Ie(null),[s,a]=oe(!1),`
    m_fe1 = re.search(
        rf"function\s+(?P<fn>{JS_ID})\(\{{sessions:(?P<S>{JS_ID}),context:(?P<C>{JS_ID})\}}\)\{{"
        rf"(?P<init>{JS_ID})\(\);"
        rf"let\s+(?P<refJ>{JS_ID})={re.escape(UR)}\(null\),"
        rf"(?P<refY>{JS_ID})={re.escape(UR)}\(null\),"
        rf"(?P<refX>{JS_ID})={re.escape(UR)}\(null\),"
        rf"(?P<refQ>{JS_ID})={re.escape(UR)}\(null\),"
        rf"\[(?P<G>{JS_ID}),(?P<setG>{JS_ID})\]={re.escape(US)}\(!1\),",
        text,
    )
    if m_fe1 is None:
        raise RuntimeError("Could not find fe1 chat-tab container signature")

    p0 = "ccReact"  # emission goes through the injected ccReact namespace shim
    fe1_start = m_fe1.start()
    fe1_end = min(len(text), m_fe1.end() + 14000)
    body = text[fe1_start:fe1_end]

    # The history toggle button (jsx-runtime):
    # `b(Cf,{ref:o,ariaLabel:"Session history",iconSize:20,onClick:()=>a(!s),children:b(nZe,{})})`
    m_hist = re.search(
        rf"{JS_ID}\((?P<QQ>{JS_ID}),"
        rf"\{{ref:(?P<refUsed>{JS_ID}),ariaLabel:\"Session history\",iconSize:20,"
        rf"onClick:\(\)=>(?P<setterUsed>{JS_ID})\(!(?P<stateUsed>{JS_ID})\),"
        rf"children:{JS_ID}\((?P<in1>{JS_ID}),\{{\}}\)\}}\)",
        body,
    )
    if m_hist is None:
        raise RuntimeError("Could not find session-history toggle createElement")

    # The body/content anchor in fe1 (jsx-runtime: `b("div",{className:ws.body,children:`).
    body_anchor_rel = body.find(f'("div",{{className:{h6}.body,children:')
    ...
    # inline Rs invocation in block 12 (jsx-runtime):
    # `b(nQe,{isOpen:s,onClose:()=>a(!1),onOpen:...,localSessions:[...e.sessions.value].sort(...),...})`
    m_re1 = re.search(
        rf"{JS_ID}\((?P<Re1>{JS_ID}),"
    ...
    # Filter hook anchor (bare hooks):
    # `[k,M]=oe(null),D=Vt((N)=>{setTimeout(()=>{let te=GGe(N.milestoneId)`
    m_filter = re.search(
        rf"\[(?P<N>{JS_ID}),(?P<E>{JS_ID})\]={re.escape(US)}\(null\),"
        rf"(?P<y>{JS_ID})={re.escape(UC)}\(\((?P<bp>{JS_ID})\)=>"
...
# The release channel this patcher SHIPS as ("experimental" | "stable").
# SINGLE SOURCE OF TRUTH for the tag: it lives INSIDE the patcher file, gets
# stamped into the patched webview as `ccPatchChannel`, and is mirrored into the
# rollback manifest at archive time. tools/ship.py rewrites this line per release
...
# Release notes for THIS patcher version ("What's New"). Ships INSIDE the patcher
# exactly like ORBIT_CHANNEL: tools/ship.py stamps it from whats_new.md (JSON-
# encoded so it stays a single safe line), and tools/archive_patcher.py mirrors it
...
def patch_webview_js(webview_js: Path) -> bool:
    ...
    al = _capture_react_aliases(text)
    rs = _capture_rs_component(text, al)
    or0 = _capture_or0_component(text, al, w2=rs["w2"])
    nr0 = _capture_nr0(text)
    fe1 = _capture_fe1_component(text, al)
    ...
    # ──────────────────────────────────────────────────────────────────────
    # 1. Inject the ccReact namespace shim + helper JS + build version marker at
    #    module scope, just before the sessions-list component (`function Rs(`).
    #
    #    CC 2.1.186 rebundled the webview to React's automatic JSX runtime, so
    #    the old `;var _R0=16,wR0=1000;` injection anchor is gone and there is no
    #    per-component React namespace to emit through. Instead we synthesize one:
    #    `ccReact` maps `.default.createElement`/`.useState`/etc onto the bundle's
    #    bare top-level aliases (captured in `al`). Every emission below keeps its
    #    `{S0}.default.createElement(...)` / `{S0}.useState(...)` shape verbatim —
    #    S0/p0/RE all point at "ccReact" — so only the READ anchors change.
    # ──────────────────────────────────────────────────────────────────────
    CE, US, UR, UC, UE, FWD = al["ce"], al["us"], al["ur"], al["uc"], al["ue"], al["fwd"]
    frag = al.get("frag")
    frag_field = f",Fragment:{frag}" if frag else ""
    cc_react = (
        f"var ccReact={{default:{{createElement:{CE},forwardRef:{FWD}{frag_field}}},"
        f"useState:{US},useRef:{UR},useCallback:{UC},useEffect:{UE}}};"
        f"try{{globalThis.ccReact=ccReact}}catch(ccE){{}}"
    )
    helper_anchor = f"function {rs['fn']}("
    h_idx = text.find(helper_anchor)
    if h_idx < 0:
        raise RuntimeError("Lost helper-injection anchor (function Rs) after capture")
    version_marker = f'var ccPatchBuildVersion="{PATCHER_VERSION}";var ccPatchChannel="{PATCHER_CHANNEL}";'
    text = text[:h_idx] + ";" + version_marker + cc_react + CLAUDE_HELPER_JS + text[h_idx:]
    ...
    # ──────────────────────────────────────────────────────────────────────
    sort_re = re.compile(
        rf"\}}\):{re.escape(_1)},{re.escape(t_outer)}={re.escape(UR)}\({re.escape(F_outer)}\);"
    )
    ...
    D = or0["D"]
    hook_re = re.compile(
        rf"(?P<init>{JS_ID})\(\);let\s+{re.escape(j)}={re.escape(UR)}\(null\),"
        rf"{re.escape(D)}={re.escape(UR)}\(!0\);"
    )
    ...
    # ──────────────────────────────────────────────────────────────────────
    # CC 2.1.186: the row is `return E("button",{...,children:[...]})` (jsxs,
    # children-in-props). We keep the jsxs wrapper + its `children:[` array open
    # and only rewrite the button's props (the native row children follow).
    btn_re = re.compile(
        rf'return (?P<j>{JS_ID})\("button",'
        ...
        rf"onClick:{re.escape(X)}\?void 0:{re.escape(G)},onMouseMove:{re.escape(q)},children:\["
    )
    m_btn = btn_re.search(text)
    if m_btn is None:
        raise RuntimeError("Lost OR0 row-button anchor")
    JSXS_btn = m_btn.group("j")
    new_btn = (
        f'return {JSXS_btn}("button",'
        f"{{ref:{F_row},className:`${{{w2}.sessionItem}} ccPatchSessionItem "
        f'${{{J}?{w2}.active+" ccPatchActive":""}} ${{{Y}?{w2}.focused:""}}`,'
        f'"data-cc-color":ccPatchGetColor({Z}),'
        ...
        f"ccPatchIsArchived({Z})?{{label:`Unarchive`,fn:()=>ccPatchToggleArchive({Z})}}:{{label:`Archive`,fn:()=>ccPatchToggleArchive({Z})}},{Z},()=>{z}({Z}))}},children:["
    )
    ...
    # ──────────────────────────────────────────────────────────────────────
    # CC 2.1.186 jsx-runtime: the name ternary is
    #   o?b("span",{ref:g,...,onClick:(y)=>y.stopPropagation(),children:MM(t)})
    #    :b("span",{className:mn.sessionName,children:iQe(MM(t),r)}),h&&t.worktree...
    si_re = re.compile(
        rf'{re.escape(X)}\?{JS_ID}\("span",'
        ...
        rf"onClick:\((?P<msp>{JS_ID})\)=>(?P=msp)\.stopPropagation\(\),children:{re.escape(Kk)}\({re.escape(Z)}\)\}}\):"
        rf'{JS_ID}\("span",\{{className:{re.escape(w2)}\.sessionName,'
        rf"children:{re.escape(Le1)}\({re.escape(Kk)}\({re.escape(Z)}\),{re.escape(Q)}\)\}}\),"
    ...
    new_si = (
        ...
        f'+(R1==="error"?"claudePatchStatusError":R1==="running"?"claudePatchStatusRunning":R1==="waiting"?"claudePatchStatusWaiting":R1==="done"?"claudePatchStatusDone":"claudePatchStatusIdle"),'
        f'title:R1==="error"?(ccPatchErrorReason({Z})||"Error"):R1==="running"?"Running":R1==="waiting"?"Waiting for input":R1==="done"?"Completed":""}}),'
        f"{X}?"
        ...
        f'R1==="error"||R1==="running"||R1==="waiting"?ccPatchActivityText({Z})||{nr0}(ccPatchRecency({Z})):{nr0}(ccPatchRecency({Z})))'
        f"),"
    ...
    icon_color = (
        f'{S0}.default.createElement("svg",{{width:12,height:12,viewBox:"0 0 12 12","aria-hidden":true}},'
        f'{S0}.default.createElement("circle",{{cx:6,cy:6,r:4,'
        f'fill:ccPatchColorHex(ccPatchGetColor({Z}))||"none",'
        f'stroke:"currentColor",strokeWidth:1.4}}))'
    )
    # CC 2.1.186 jsx-runtime: the sessionMeta block is
    #   E("span",{className:mn.sessionMeta,children:[ <time>, <actions> ]})
    # The old single mega-regex no longer matches (children-in-props), so we
    # anchor on the sessionMeta start and bracket-walk to its matching close,
    # then swap the whole element for our ccPatchRowActions element below.
    meta_start_re = re.compile(
        rf',{JS_ID}\("span",\{{className:{re.escape(w2)}\.sessionMeta,children:\['
    )
    m_meta = meta_start_re.search(text)
    if m_meta is None:
        raise RuntimeError("Lost OR0 sessionMeta hover-actions anchor")
    meta_call_paren = text.index("(", m_meta.start() + 1)
    depth = 0
    meta_end = -1
    ii = meta_call_paren
    while ii < len(text):
        ...
        ii += 1
    if meta_end < 0:
        raise RuntimeError("Could not bracket the sessionMeta call")
    new_meta = (
        f',{S0}.default.createElement("div",{{className:"ccPatchRowActions"}},'
        f'{S0}.default.createElement("span",{{role:"button",tabIndex:0,'
        f'className:`${{{w2}.actionButton}} ccPatchActionBtn ccPatchColorBtn ${{ccPatchGetColor({Z})?"ccPatchActive":""}}`,'
        f"onClick:(M)=>{{M.stopPropagation(),ccPatchShowColorMenu(M.clientX,M.clientY,{Z})}},"
        f'title:"Set color"}},' + icon_color + "),"
        f'{S0}.default.createElement("span",{{role:"button",tabIndex:0,'
    ...
    text = text[: m_meta.start()] + new_meta + text[meta_end:]
    ...
    # New Session onClick — clear the ?session= URL param, THEN create + activate
    # a fresh session in-pane.
    #
    # (1) We deliberately do NOT take the native `startNewConversationTab()` path
    # the stock button uses (`if(!Z.startNewConversationTab())S.createSession()`):
    # when the connection's `openNewInTab` config is on, that fires a
    # `new_conversation_tab` host RPC to open the conversation in a separate VS
    # Code EDITOR tab. Orbit's webview is a single embedded pane with no editor
    # tabs, so the RPC produces nothing visible and the click is a silent no-op.
    #
    # (2) But `createSession()` alone is NOT enough, and that gap caused a worse
    # bug: the button would open the new chat, then bounce the user right back to
    # the prior chat ~seconds later. Root cause — `createSession()` makes a LOCAL
    # session that has NO server `sessionId` yet, and the native session-
    # restoration in `doListSessions` runs on EVERY `listSessions()`:
    #     s = new URLSearchParams(location.search).get("session");   // Tbe()
    #     if(s){ a = sessions.find(x=>x.sessionId.value===s);
    #            if(a) activeSession.value = a; }                     // <-- re-assert
    # The native URL-sync effect only WRITES `?session=` when the active session
    # has a sessionId — so an id-less brand-new session never updates the URL, the
    # URL keeps pointing at the PRIOR chat, and the next `listSessions()` (Orbit
    # auto-refreshes the list every 7s, plus native events) re-asserts
    # activeSession back to that prior chat. (Clicking an EXISTING chat doesn't
    # bounce because those sessions have ids, so the URL tracks them.)
    #
    # Fix: blow away `?session=` up front so `Tbe()` returns null and the whole
    # restoration block is skipped, leaving our new session active. Once the user
    # sends a message the session gets a real id and the native effect repopulates
    # the URL correctly. We also re-assert activeSession to the resolved session as
    # belt-and-suspenders against the documented un-awaited-navigation churn.
    new_session_onclick = (
        f'()=>{{try{{'
        f'try{{var ccU=new URL(window.location.toString());'
        f'if(ccU.searchParams.has("session")){{ccU.searchParams.delete("session");'
        f'window.history.replaceState({{}},"",ccU.toString())}}}}catch(ccUe){{}}'
        f'Promise.resolve({fe1_S}.createSession()).then('
        f'function(ccNs){{if(ccNs&&{fe1_S}.activeSession){fe1_S}.activeSession.value=ccNs}}'
        f').catch(function(){{}})}}catch(ccE){{}}}}'
    )
    ...
    inline_body = (
        ...
        f'{usage_hdr_btn}'
        # Refresh — the session list is NOT push-live (Claude only re-lists on
        # certain events), so stale rows linger / new ones don't appear. This button
        # re-lists local + remote on demand; a gentle auto-refresh poll (init IIFE)
        # keeps it fresh on its own.
        f'{p0}.default.createElement("button",{{className:"ccPatchHeaderBtn ccPatchRefreshBtn",title:"Refresh sessions",'
        f"onClick:function(){{try{{{fe1_S}.listSessions(),{fe1_S}.listRemoteSessions()}}catch(ccE){{}}}}}},"
        f'{p0}.default.createElement("svg",{{width:14,height:14,viewBox:"0 0 14 14",fill:"none",'
        f'stroke:"currentColor",strokeWidth:1.4,strokeLinecap:"round",strokeLinejoin:"round","aria-hidden":true}},'
        f'{p0}.default.createElement("path",{{d:"M12.5 7a5.5 5.5 0 1 1-1.7-4"}}),'
        f'{p0}.default.createElement("polyline",{{points:"12.8,1.4 12.8,4.2 10,4.2"}}))),'
        # Search
    ...
    # CC 2.1.186 jsx-runtime: body/content nest via children-in-props:
    #   b("div",{className:ws.body,children:b("div",{className:ws.content,children:<X>})})
    # Match the body->content opening, bracket-walk out the content child <X>,
    # then rebuild as: body div -> [sidebar, resize, content div(<X>)].
    body_open_re = re.compile(
        rf'{JS_ID}\("div",\{{className:{re.escape(h6)}\.body,children:'
        rf'{JS_ID}\("div",\{{className:{re.escape(h6)}\.content,children:'
    )
    m_body_open = body_open_re.search(text)
    if m_body_open is None:
        raise RuntimeError("Lost h6.body/h6.content marker")
    x_start = m_body_open.end()
    depth = 0
    in_str: "str | None" = None
    in_tpl = False
    x_end = -1
    ...
    while k < len(text):
        ...
        k += 1
    if x_end < 0:
        raise RuntimeError("Could not bracket the h6.content child expression")
    content_child = text[x_start:x_end]
    tail = text[x_end : x_end + 4]
    if tail != "})})":
        raise RuntimeError(f"Unexpected h6.body/content tail: {tail!r}")
    text = text[: m_body_open.start()] + inline_body + content_child + "))" + text[x_end + 4 :]

    # ──────────────────────────────────────────────────────────────────────
    # 14. Sessions list — wrap the b.map block in star/pin/active/archive sections.
    # ──────────────────────────────────────────────────────────────────────
    # CC 2.1.186 jsx-runtime: the list is
    #   b("div",{className:mn.sessionsList,children:N.map((Ue,Je)=>{let Di=...;return b(zLt,{ref:..,...},key)})})
    # key is now the jsx 3rd arg (not a `key:` prop). We match the map prefix to
    # capture the loop locals, then bracket-walk the `.map(...)` call and swap it
    # for our sectioned-items IIFE.
    map_re = re.compile(
        rf"{re.escape(w2)}\.sessionsList,children:{re.escape(b_var)}\.map\(\((?P<g1>{JS_ID}),(?P<o1>{JS_ID})\)=>\{{"
        rf"let\s+(?P<h5>{JS_ID})=(?P=o1)==={re.escape(w_var)},"
        rf"(?P<k2>{JS_ID})={re.escape(p_var)}===(?P=g1)\.sessionId\.value;"
        rf"return {JS_ID}\({re.escape(OR0)},\{{"
    ...
    k2 = m_map.group("k2")
    W5 = m_map.group("W5")
    # Bracket-walk the `<b>.map(...)` call so we can replace exactly it.
    map_call = f"{b_var}.map("
    map_call_start = text.index(map_call, m_map.start())
    map_paren = map_call_start + len(map_call) - 1
    depth = 0
    map_end = -1
    ii = map_paren
    while ii < len(text):
        ...
        ii += 1
    if map_end < 0:
        raise RuntimeError("Could not bracket the sessions-list .map() call")

    or0_row = (
        f"{S0}.default.createElement({OR0},{{"
        # Key by STABLE identity (server sessionId → internalId), never the
        # positional idx: the list re-sorts on every message (running chats jump
        # to the top by recency), so an idx-keyed row rebinds its DOM/state to a
        # different session on each sort. internalId is set once at construction
        # and constant for the chat's life (same id ccPatchYoloKey trusts).
        f"key:{g1}.sessionId.value??{g1}.internalId??{o1},"
    ...
    text = text[:map_call_start] + new_map + text[map_end:]
    ...
    # ──────────────────────────────────────────────────────────────────────
    # 16. (removed) YOLO mode — Orbit used to default new sessions into the
    #     native `bypassPermissions` mode here. Removed: that mode is broken
    #     upstream in the VS Code extension (the host `requestToolPermission`
    #     ignores the permission mode and prompts anyway — Anthropic issues
    #     #43953 / #49173), so forcing it in only produced a "YOLO mode" button
    #     that never actually skipped prompts. Permission modes are now 100%
    #     native; there is no official, working way to enable it from the IDE.
    # ──────────────────────────────────────────────────────────────────────

    # ──────────────────────────────────────────────────────────────────────
    # 16b. Default reasoning effort — new chats open on the effort chosen by the
    #      Settings ▸ "Default" slider. Same session-class field-initializer seam:
    #      the field initializer `effortLevel=at(void 0)` becomes
    #      `effortLevel=at(ccPatchDefaultEffort()||void 0)`, so a freshly
    #      constructed session starts on the saved default. No pick ("Auto" =>
    #      empty string) falls through to `void 0`, the native unset state —
    #      unchanged behavior. Effort is still clamped at use-time by the model's
    #      supportedEffortLevels, so an unsupported pick simply has no effect on
    #      that model. Anchored ROBUSTLY on the effort field's OWN token
    #      `effortLevel=<at>(void 0)` (it appears exactly once and is independent
    #      of unrelated field churn like hasPersistedTitle in CC 2.1.179, which
    #      sits beside modelSelection, not effortLevel) and guarded with `typeof`
    #      so a missing helper can never break session construction.
    # ──────────────────────────────────────────────────────────────────────
    m_defeffort = re.search(
        rf"effortLevel=(?P<at>{JS_ID})\(void 0\)",
        text,
    )
    if m_defeffort is None:
        log("Note: effortLevel init anchor not found; skipping default-effort patch")
    else:
        de_at = m_defeffort.group("at")
        de_anchor = f"effortLevel={de_at}(void 0)"
        de_replace = (
            f'effortLevel={de_at}((typeof ccPatchDefaultEffort==="function"'
            f'?ccPatchDefaultEffort():"")||void 0)'
        )
        if de_anchor in text and de_replace not in text:
            text = text.replace(de_anchor, de_replace, 1)
    ...
    if m_root is None:
        ...
        # CC 2.1.186 jsx-runtime: `E("div",{className:ws.header,children:[...]})`.
        header_re = re.compile(
            rf'(?P<hj>{JS_ID})\("div",\{{className:{re.escape(h6)}\.header,children:'
        )
        m_header = header_re.search(text)
        if m_header:
            new_header = f'{m_header.group("hj")}("div",{{className:`${{{h6}.header}} claudePatchHeader`,children:'
    ...
    # ──────────────────────────────────────────────────────────────────────
    QQ = fe1["QQ"]
    # CC 2.1.186 jsx-runtime: `,b(Cf,{ariaLabel:"New session",iconSize:20,onClick:()=>{if(!t.startNewConversationTab())e.createSession()},children:b(oZe,{})})`
    new_sesh_re = re.compile(
        rf',{JS_ID}\({re.escape(QQ)},\{{ariaLabel:"New session",iconSize:\d+,'
        rf'onClick:\(\)=>\{{if\(!{re.escape(fe1["Z"])}\.startNewConversationTab\(\)\){re.escape(fe1["$"])}\.createSession\(\)\}},'
        rf'children:{JS_ID}\({JS_ID},\{{\}}\)\}}\)'
    )
    ...
    # ──────────────────────────────────────────────────────────────────────
    # CC 2.1.186 jsx-runtime: the popup options are
    #   b("button",{className:cu.popupOption,onClick:Y,children:b("span",{className:cu.optionText,children:"Fork conversation from here"})}),
    #   W&&E(Oe,{children:[<rewind>,<forkrewind>]})
    # We capture the handlers off the stable option strings, then string-aware
    # paren-walk the ENCLOSING message-actions container div (its popup style has
    # a `translateX(${u}px)` template, so a naive paren count would miscount) and
    # swap the whole container for our always-visible inline ccPatchForkRow.
    fork_re = re.compile(
        rf'{JS_ID}\("button",\{{className:(?P<CZ>{JS_ID})\.popupOption,onClick:(?P<S>{JS_ID}),'
        rf'children:{JS_ID}\("span",\{{className:(?P=CZ)\.optionText,children:"Fork conversation from here"\}}\)\}}\),'
        rf'(?P<P>{JS_ID})&&{JS_ID}\({JS_ID},\{{children:\['
        rf'{JS_ID}\("button",\{{className:(?P=CZ)\.popupOption,onClick:(?P<U>{JS_ID}),'
        rf'children:{JS_ID}\("span",\{{className:(?P=CZ)\.optionText,children:"Rewind code to here"\}}\)\}}\),'
        rf'{JS_ID}\("button",\{{className:(?P=CZ)\.popupOption,onClick:(?P<K>{JS_ID}),'
        rf'children:{JS_ID}\("span",\{{className:(?P=CZ)\.optionText,children:"Fork conversation and rewind code"\}}\)\}}\)',
        re.S,
    )
    m_fork = fork_re.search(text) if feature_on("fork-row") else None
    m_cont = None
    if not feature_on("fork-row"):
        log("Skipping fork action row (feature disabled)")
    elif m_fork is None:
        log("Note: fork/rewind action-row anchor not found; skipping")
    else:
        CZ = m_fork.group("CZ")
        cont_re = re.compile(rf'{JS_ID}\("div",\{{className:`\$\{{{re.escape(CZ)}\.container\}} ')
        for cm in cont_re.finditer(text):
            if cm.start() < m_fork.start():
                m_cont = cm
            else:
                break
        if m_cont is None:
            log("Note: fork container div not found; skipping fork row")
    if m_fork is not None and m_cont is not None:
        RE = "ccReact"
        S = m_fork.group("S")
        U = m_fork.group("U")
        K = m_fork.group("K")
        P = m_fork.group("P")
        # String-aware paren-walk the container div to its matching close.
        cont_paren = text.index("(", m_cont.start())
        f_depth = 0
        f_str: "str | None" = None
        f_tpl = False
        cont_end = -1
        ...
        while kk < len(text):
            ...
            kk += 1
        if cont_end < 0:
            raise RuntimeError("Could not bracket the fork message-actions container")
        ico = (
        ...
        new_fork = (
            ...
            f'{RE}.default.createElement("button",{{type:"button",className:"ccPatchForkBtn",'
            f'onClick:(ccE)=>{{ccE.preventDefault();ccE.stopPropagation();{S}()}},'
            ...
            f'{P}&&{RE}.default.createElement("button",{{type:"button",className:"ccPatchForkBtn",'
            f'onClick:(ccE)=>{{ccE.preventDefault();ccE.stopPropagation();{U}()}},'
            ...
            f'{P}&&{RE}.default.createElement("span",{{className:"ccPatchForkDot"}},"\\u00b7"),'
            f'{P}&&{RE}.default.createElement("button",{{type:"button",className:"ccPatchForkBtn ccPatchForkBtnAlt",'
            f'onClick:(ccE)=>{{ccE.preventDefault();ccE.stopPropagation();{K}()}},'
            f'title:"Fork conversation and rewind code"}},'
            f'{ico}{RE}.default.createElement("path",{{d:"M3 4v5h5M3.05 13A9 9 0 1 0 6 5.3L3 9"}}),'
            f'{RE}.default.createElement("path",{{d:"M16 14v6M13 17h6"}})),'
            f'{RE}.default.createElement("span",null,"Fork + rewind"))'
            f')'
        )
        text = text[: m_cont.start()] + new_fork + text[cont_end:]
    ...
    # Note: there is intentionally no fork wrapper fix here. The stock $1() handler
    # calls $.forkConversation() which opens a proper forked session (full
    # conversation history) via startNewConversationTab / viewSession inside
    # the context wrapper.  The wrapper ALWAYS returns "" (even on success),
    # so any return-value guard would clobber a working fork with a blank
    # new session.  Leave it stock — fork either works or the .catch() shows
    # a notification.  No patcher intervention needed.

    # ──────────────────────────────────────────────────────────────────────
    # 18. "Show more" always visible. CC 2.1.186's expandable text (component
    #     `zZe`) gates the button on `!s&&o&&l` = !expanded && overflowing &&
    #     HOVERED (l = the hover state: c(!0) onMouseEnter / c(!1) onMouseLeave).
    #     So the link flickers in/out on hover — Jacob's complaint. Drop ONLY the
    #     hover term so it shows whenever the text is genuinely clipped (`!s&&o`)
    #     and is absent otherwise. Anchored on the expandButton "Show more" jsx.
    # ──────────────────────────────────────────────────────────────────────
    showmore_re = re.compile(
        rf'(?P<keep>!?{JS_ID}&&{JS_ID})&&{JS_ID}&&(?P<jdiv>{JS_ID})\("div",'
        rf'\{{className:(?P<ZG>{JS_ID})\.buttonContainer,children:'
        rf'(?P<jbtn>{JS_ID})\("button",'
    ...
    if m_sm is None:
        ...
        new_sm = (
            f'{m_sm.group("keep")}&&{m_sm.group("jdiv")}("div",'
            f'{{className:{m_sm.group("ZG")}.buttonContainer,children:'
            f'{m_sm.group("jbtn")}("button",'
            f'{{className:{m_sm.group("ZG")}.expandButton,"aria-label":"Show more"'
        )
        text = text[: m_sm.start()] + new_sm + text[m_sm.end() :]
        log("Show-more: dropped the hover gate — always visible when clipped")

    # ──────────────────────────────────────────────────────────────────────
    # YOLO mode auto-approve — PER CHAT. The webview's handleToolPermissionRequest
    # (channelId, request, signal) is where EVERY tool / file-edit / bash approval
    # is decided — it returns {type:"tool_permission_response",result:{behavior:…}}.
    # When the chat this request belongs to has YOLO on, return "allow" immediately
    # so the prompt never renders. The only id we get here is the EPHEMERAL per-turn
    # channelId, so ccPatchAutoApproveOnChannel maps it back to its live session and
    # reads YOLO by a STABLE per-chat key (ccPatchYoloKey/internalId) — keying the
    # store directly on channelId broke after every turn (it is regenerated per turn
    # and wiped to void 0 at stream end). Per-chat, no global flag, no host
    # involvement. updatedInput echoes the original inputs so the tool runs unmodified.
    # ──────────────────────────────────────────────────────────────────────
    m_htp = re.search(
        rf"async handleToolPermissionRequest\((?P<c>{JS_ID}),(?P<r>{JS_ID}),(?P<s>{JS_ID})\)\{{",
        text,
    )
    if m_htp is None:
        raise RuntimeError("Lost handleToolPermissionRequest anchor (YOLO per-chat auto-approve)")
    # Auto-approve when the chat has YOLO on — EXCEPT for question/decision tools
    # (AskUserQuestion / ExitPlanMode via ccPatchYoloIsAsk): those aren't actions, so
    # blanket-"allow" would answer a real question with a non-answer and rob the model
    # of the ability to consult the user. Skipping them lets their real UI render.
    _htp_ins = (
        f'if(globalThis.ccPatchAutoApproveOnChannel&&globalThis.ccPatchAutoApproveOnChannel({m_htp.group("c")})'
        f'&&!(globalThis.ccPatchYoloIsAsk&&globalThis.ccPatchYoloIsAsk({m_htp.group("r")})))'
        f'return{{type:"tool_permission_response",result:{{behavior:"allow",updatedInput:{m_htp.group("r")}.inputs}}}};'
    )
    text = text[: m_htp.end()] + _htp_ins + text[m_htp.end():]
    log("YOLO mode: per-chat auto-approve wired into handleToolPermissionRequest")

    # YOLO robustness — record channelId->session at the SOURCE. launchClaude() mints
    # the session's fresh claudeChannelId (`this.claudeChannelId=e`); the moment it does,
    # we stamp channelId->stable yoloKey into ccPatchYoloChan. That map is what the
    # handleToolPermissionRequest short-circuit reads FIRST (O(1)), so YOLO no longer
    # depends on the resolver scan finding the session in a list — the scan could miss
    # (session not yet in sessionsRoot / not the composer session) and a missed lookup
    # is exactly a prompt YOLO was supposed to auto-approve. Recording here can't miss:
    # a tool-permission request for a turn always fires AFTER that turn's launchClaude.
    m_lc = re.search(
        rf"let (?P<e>{JS_ID})=Math\.random\(\)\.toString\(36\)\.slice\(2\);this\.claudeChannelId=(?P=e);",
        text,
    )
    if m_lc is None:
        raise RuntimeError("Lost launchClaude channel-assign anchor (YOLO channel record)")
    _lc_ins = (
        f'try{{globalThis.ccPatchYoloChanRecord&&globalThis.ccPatchYoloChanRecord({m_lc.group("e")},this)}}catch(_){{}}'
    )
    text = text[: m_lc.end()] + _lc_ins + text[m_lc.end():]
    log("YOLO mode: channelId->session recorded at launchClaude (O(1) resolution)")

    # ──────────────────────────────────────────────────────────────────────
    # Composer rework deferral (CC 2.1.186). Blocks 18b–21 (bottom-bar reorder,
    # model picker, effort slider, send-split, queued-message bubbles) target the
    # composer footer/transcript, which CC 2.1.186 rebundled to the jsx-runtime.
    # Their jsx re-port lands in a follow-up beta; until then we write the
    # core-patched webview (session pane fully active) and return early so the
    # patch completes cleanly with Claude's native composer in place.
    # ──────────────────────────────────────────────────────────────────────
    if not COMPOSER_REWORK_2186:
        log("Composer bottom-bar/send/queue rework deferred for CC 2.1.186 (core session pane active)")
    ...
    # ──────────────────────────────────────────────────────────────────────
    # YOLO mode as a native Modes entry: relabel + re-describe the bypassPermissions
    # mode descriptor so the composer Modes menu shows "YOLO mode" (it sorts right
    # below "Auto mode") with a clear auto-approve description. Selecting it drives
    # Orbit's REAL per-chat auto-approve (the webview handleToolPermissionRequest
    # short-circuit, keyed by channelId), NOT the broken native bypassPermissions
    # enforcement — see the mode-selector onSelect in block 18b below. The label
    # anchor is required (the whole entry hinges on it); the description is best-effort.
    if 'label:"Bypass permissions"' not in text:
        raise RuntimeError("Lost bypassPermissions label anchor (YOLO mode entry)")
    text = text.replace('label:"Bypass permissions"', 'label:"YOLO mode"', 1)
    _yolo_desc = 'description:"Claude will not ask for approval before running potentially dangerous commands"'
    if _yolo_desc in text:
        text = text.replace(_yolo_desc, 'description:"Like Auto mode, but auto-approves every prompt"', 1)
    ...
    # CC 2.1.186 jsx-runtime: the footer return is
    #   E("div",{className:`${Id.inputFooter} ${Id.inputFooterV2}`,children:[attach,cmd,usage,sel,spacer,mode,send]})
    # Each child is a `b(...)`/`n&&b(...)` slice; we capture the slices verbatim
    # (they re-slot as positional children of a reconstructed createElement
    # container) and the mode-selector's inner vars. RE -> ccReact for emission.
    footer_re = re.compile(
        rf'return {JS_ID}\("div",\{{className:`\$\{{(?P<lu>{JS_ID})\.inputFooter\}} \$\{{(?P=lu)\.inputFooterV2\}}`,children:\['
        rf'(?P<attach>{JS_ID}\({JS_ID},\{{onInsertAtMention:{JS_ID},onAttachFile:{JS_ID},browserIntegrationSupported:{JS_ID},onTerminalCollaborator:void 0\}}\)),'
        rf'(?P<cmd>{JS_ID}\("button",\{{type:"button",className:(?P=lu)\.menuButton,title:"Show command menu \(/\)",onClick:{JS_ID},children:{JS_ID}\({JS_ID},\{{\}}\)\}}\)),'
        rf'(?P<usage>{JS_ID}\({JS_ID},\{{usedTokens:(?P<e>{JS_ID})\.usageData\.value\.totalTokens,contextWindow:(?P=e)\.usageData\.value\.contextWindow-(?P=e)\.usageData\.value\.maxOutputTokens-13000,onCompact:{JS_ID},buttonClassName:(?P=lu)\.usageButtonV2\}}\)),'
        rf'(?P<sel>(?P<n>{JS_ID})&&{JS_ID}\("div",\{{className:(?P=lu)\.divider\}}\),(?P=n)&&{JS_ID}\({JS_ID},\{{includeSelection:{JS_ID}\?\?!1,currentSelection:(?P=n),onToggle:{JS_ID}\?\?\(\(\)=>\{{\}}\)\}}\)),'
        rf'(?P<spacer>{JS_ID}\("div",\{{className:(?P=lu)\.spacer\}}\)),'
        rf'{JS_ID}\((?P<gje>{JS_ID}),\{{mode:(?P<t>{JS_ID}),availableModes:(?P<i>{JS_ID}),onSelect:\((?P<E>{JS_ID})\)=>void (?P=e)\.setPermissionMode\((?P=E),!0\),anchorRight:!0,effortLevel:(?P<f>{JS_ID}),supportedEffortLevels:(?P<g>{JS_ID}),onSetEffort:(?P<b>{JS_ID}),ultracodeAvailable:(?P<uca>{JS_ID}),ultracodeSelected:(?P<ucs>{JS_ID}),onSelectUltracode:(?P<ouc>{JS_ID})\}}\),'
        rf'(?P<sendbtn>{JS_ID}\("button",\{{type:"submit",disabled:!(?P=e)\.busy\.value&&!(?P<o>{JS_ID}),className:(?P=lu)\.sendButton,"data-permission-mode":(?P=t),onClick:\((?P<E2>{JS_ID})\)=>\{{if\((?P=e)\.busy\.value&&!(?P=o)\)(?P=E2)\.preventDefault\(\),(?P=e)\.interrupt\(\)\}},children:(?P<p>{JS_ID})\}}\))\]\}}\)'
    )
    m_ft = footer_re.search(text)
    if m_ft is None:
        raise RuntimeError("Lost XJe composer-footer return anchor (bottom-bar reorder)")
    RE = "ccReact"  # emission flows through the injected namespace shim
    lu = m_ft.group("lu")
    XE = m_ft.group("e")
    XT = m_ft.group("t")
    XI = m_ft.group("i")
    ...
    XOUC = m_ft.group("ouc")
    GJE = m_ft.group("gje")
    # Reconstruct the footer container as a classic createElement (positional
    # children) so the reordered emission below stays byte-for-byte.
    g_open = f'{RE}.default.createElement("div",{{className:`${{{lu}.inputFooter}} ${{{lu}.inputFooterV2}}`}},'
    ...
    # Mode selector: keep the polished V2 popup (anchorRight) and DROP the effort
    # props so the in-popup effort row disappears (it is standalone in the composer).
    # YOLO mode: force "bypassPermissions" into availableModes (native only lists it
    # when the allowDangerouslySkip setting is on; we always show it, sorted right
    # after "auto"). Its onSelect drives Orbit's REAL auto-approve — the per-chat
    # ccPatchAutoApprove map (the webview handleToolPermissionRequest short-circuit),
    # with a one-time confirm — instead of the broken native enforcement. State is
    # keyed by the STABLE ccPatchYoloKey(session) (internalId), NOT claudeChannelId
    # (ephemeral per turn). The displayed `mode` is forced to bypassPermissions
    # whenever our auto-approve is on (ccPatchAutoApproveOn), so the menu shows "YOLO mode"
    # selected for the whole chat (the native mode signal downgrades bypassPermissions).
    # Picking any other mode turns auto-approve OFF and sets that native mode normally.
    # Wrapped in .ccPatchModeWrap so CSS can left-align + round its popup.
    mode_new = (
        f'{RE}.default.createElement("div",{{className:"ccPatchModeWrap"}},'
        f'{RE}.default.createElement({GJE},{{mode:(ccPatchAutoApproveOn(ccPatchYoloKey({XE}))?"bypassPermissions":{XT}),'
        f'availableModes:(function(ccA){{return ccA&&ccA.indexOf("bypassPermissions")<0?ccA.concat(["bypassPermissions"]):ccA}})({XI}),'
        f'onSelect:(ccPE)=>{{if(ccPE==="bypassPermissions"){{ccPatchYoloConfirm(function(){{ccPatchAutoApproveSet(ccPatchYoloKey({XE}),!0);{XE}.setPermissionMode("bypassPermissions",!0)}})}}else{{ccPatchAutoApproveSet(ccPatchYoloKey({XE}),!1);{XE}.setPermissionMode(ccPE,!0)}}}},anchorRight:!0}}))'
    )
    ...
    # supports effort; IK self-sizes via its own track class. Lives in the composer
    # bottom bar (Jacob: keep it here); the Default panel's effort is separate.
    effort_new = (
    ...
    # Second footer ring: PLAN USAGE, sitting right next to the native context ring
    # (g_usage / usageButtonV2). Same global helpers as the sidebar usage button —
    # ccPatchUsageBtnIcon() draws a color-coded ring of the highest plan window
    # (5-hour / weekly), ccPatchShowUsageMenu opens the rings popover. Jacob: he wants
    # two circle buttons side by side — one context, one plan usage. Gated on the same
    # usage-meter flag; trailing comma so it drops cleanly to "" when the flag is off.
    plan_usage_btn = ""
    if feature_on("usage-meter"):
        plan_usage_btn = (
            f'{RE}.default.createElement("button",{{type:"button",'
            f'className:"ccPatchUsageBtn ccPatchUsageBtnFooter",title:"Plan usage",'
            f'onClick:function(ccE){{try{{ccPatchShowUsageMenu(ccE)}}catch(ccErr){{}}}},'
            f'dangerouslySetInnerHTML:{{__html:ccPatchUsageBtnIcon()}}}}),'
        )
    new_return = (
        ...
        f"{g_usage},{plan_usage_btn}{g_cmd},{g_send})"
    )
    ...
    # ──────────────────────────────────────────────────────────────────────
    # CC 2.1.186 jsx-runtime: `let p=null;if(e.busy.value&&!o)p=b(WG,{className:Id.stopIcon});else p=b(QYe,{className:Id.sendIcon});`
    sendicon_re = re.compile(
        rf'let\s+(?P<W>{JS_ID})=null;'
        rf'if\((?P<S>{JS_ID})\.busy\.value&&!(?P<X>{JS_ID})\)'
        rf'(?P=W)={JS_ID}\((?P<stop>{JS_ID}),\{{className:(?P<cls>{JS_ID})\.stopIcon\}}\);'
        rf'else (?P=W)={JS_ID}\((?P<send>{JS_ID}),\{{className:(?P=cls)\.sendIcon\}}\);'
    )
    ...
    # CC 2.1.186 jsx-runtime: `b("button",{type:"submit",disabled:!e.busy.value&&!o,className:Id.sendButton,"data-permission-mode":t,onClick:(M)=>{...},children:p})`
    sendbtn_re = re.compile(
        rf'{JS_ID}\("button",\{{type:"submit",'
    ...
    #      compact" pie fires onCompact (=> submit "/compact") the instant it is
    #      touched — no prompt, no undo, the older turns are gone. Wrap its
    #      onClick so a tap opens an "Are you sure?" dialog first and compact
    #      only runs on confirm. The handler identifier is minified, so capture
    #      it off the stable title string (verified unique in the bundle).
    #      NON-FATAL by design: a destructive native control is better left
    #      untouched than to abort the whole patch if the anchor ever drifts —
    #      and the wrapper itself falls back to running compact directly if the
    #      injected helper is somehow absent.
    # ──────────────────────────────────────────────────────────────────────
    compact_anchor = re.compile(
        r"(click to compact`,onClick:)(?P<h>" + JS_ID + r")(,onMouseEnter:)"
    )

    def _wrap_compact(m: "re.Match[str]") -> str:
        h = m.group("h")
        return (
            m.group(1)
            + f'(ccCmpE)=>{{(typeof ccPatchConfirmCompact==="function")'
            + f"?ccPatchConfirmCompact({h}):{h}()}}"
            + m.group(3)
        )

    text, _n_compact = compact_anchor.subn(_wrap_compact, text, count=1)
    if _n_compact == 1:
        log("Compact confirmation wired")
    else:
        log("Compact-confirm anchor not matched — native compact left as-is (non-fatal)")
    ...
    # ──────────────────────────────────────────────────────────────────────
    # CC 2.1.186 jsx-runtime: `function QXe({session:e,context:t,onCreateNewSession:i,onTeleportCheckout:n}){zn();let o=Ie(null),...`
    he1_re = re.compile(
        rf'function (?P<fn>{JS_ID})\(\{{session:(?P<S>{JS_ID}),context:(?P<Z>{JS_ID}),'
        rf'onCreateNewSession:{JS_ID},onTeleportCheckout:{JS_ID}\}}\)\{{'
        rf'(?P<y4>{JS_ID})\(\);let {JS_ID}={re.escape(UR)}\(null\)'
    )
    ...
    he1_hook = (
        f"try{{ccPatchSyncWsTheme({he1_S},{he1_Z})}}catch(ccE){{}}"
    ...
    # (b) capture the chat CSS class-map (G2) via the messagesContainer (jsx).
    mc_re = re.compile(
        rf'{JS_ID}\("div",\{{ref:{JS_ID},'
    ...
    # Injection point: the bottom spacer `,<h5>,b("div",{ref:o,style:{height:`${c}px`...`.
    ...
    inj_re = re.compile(
        rf',(?P<h5>{JS_ID}),(?P<spc>{JS_ID}\("div",\{{ref:{JS_ID},'
...
def patch_webview_css(webview_css: Path) -> bool:
    ...
    for mo in re.finditer(r"\.(overlay_[A-Za-z0-9_]+)\{(?P<body>[^}]*)\}", text):
        body = mo.group("body")
        # Lift real modal scrims, but leave transparent click-catchers alone.
        if "background:var(--app-modal-background)" in body or "background-color:var(--app-modal-background)" in body or "background-color:#00000080" in body:
            modal_overlay_classes.append("." + mo.group(1))
    modal_overlay_rule = ""
    if modal_overlay_classes:
        modal_overlay_rule = ",".join(sorted(set(modal_overlay_classes))) + "{z-index:10000!important}"
    # Native dialog overlays need to clear the split-pane sidebar, but transparent
    # click-catchers used by menus must stay at their native z-index or they eat
    # menu clicks. Boost only modal-looking overlay rules discovered above.
    ...
    new = old + modal_overlay_rule + (
        ...
        # Footer variant: the plan-usage ring sitting next to the native context ring.
        ".ccPatchUsageBtnFooter{flex:0 0 auto;align-self:center}"
        ...
        ".ccPatchUsageMenu{position:fixed;z-index:100000;padding:13px 16px 15px;width:min(300px,calc(100vw - 16px));box-sizing:border-box;"
        ...
        ".ccPatchUsageRings{display:flex;gap:10px;align-items:flex-start;justify-content:space-around}"
        ...
        ".ccPatchUsageReset{margin-top:5px;font-size:12px;font-weight:600;opacity:.96;white-space:nowrap;font-variant-numeric:tabular-nums}"
        ".ccPatchUsageSub{margin-top:2px;font-size:10px;opacity:.4;letter-spacing:.02em;white-space:nowrap}"
        ".ccPatchUsageMsg{font-size:12px;opacity:.7;padding:10px 4px;text-align:center;min-width:160px}"
        # Background task-notifications (native Claude Code) are injected as a
        # USER-role message, so Claude wraps them in the human bubble
        # (.userMessage_ = bordered/rounded/input-bg). Non-human content must NEVER
        # wear the human bubble — STRIP it and show a quiet, dim "Background task"
        # line instead (no container).
        '[data-cc-tasknote] [class*="userMessage_"]{border:none!important;border-radius:0!important;background:none!important;background-color:transparent!important;padding:0!important}'
        '[data-cc-tasknote]{opacity:.6;font-size:11px;line-height:1.4;color:var(--app-secondary-foreground,#8a8f98)}'
        '[data-cc-tasknote]::before{content:"\\2699  " attr(data-cc-tasknote-label);display:block;font-size:11px;font-weight:600;color:var(--app-secondary-foreground,#9aa0a6);margin-bottom:2px;white-space:normal;opacity:.9}'
        ...
        # Hover lift. The themes set --vscode-list-hoverBackground (Glow, Black,
        # Bright, …) but the old rule keyed off --app-list-hover-background, which
        # the themes never touch and which falls back to a near-invisible 6% white
        # — so non-colored rows on the default + every non-Glow theme read as
        # having NO hover at all. Prefer the token the themes actually drive, then
        # a visibly-lifted neutral fallback that works on dark AND light.
        "button.ccPatchSessionItem:hover{background:var(--vscode-list-hoverBackground,"
        "var(--app-list-hover-background,rgba(128,128,128,.16)))!important}"
        ...
        ".ccPatchRowActions{display:flex;align-items:center;gap:1px;"
        ...
        "padding:2px!important;border-radius:3px!important;"
        "color:var(--app-secondary-foreground)!important;"
        "width:17px!important;height:22px!important}"
        ...
        # ── Per-chat color (Google-Calendar style; tints the row container) ──
        ".ccPatchSessionItem[data-cc-color=red]{background:rgba(229,72,77,.16)!important}"
        "button.ccPatchSessionItem[data-cc-color=red]:hover{background:rgba(229,72,77,.26)!important}"
        ".ccPatchSessionItem[data-cc-color=red].ccPatchActive{background:rgba(229,72,77,.34)!important}"
        ".ccPatchSessionItem[data-cc-color=orange]{background:rgba(247,107,21,.16)!important}"
        "button.ccPatchSessionItem[data-cc-color=orange]:hover{background:rgba(247,107,21,.26)!important}"
        ".ccPatchSessionItem[data-cc-color=orange].ccPatchActive{background:rgba(247,107,21,.34)!important}"
        ".ccPatchSessionItem[data-cc-color=yellow]{background:rgba(245,197,24,.18)!important}"
        "button.ccPatchSessionItem[data-cc-color=yellow]:hover{background:rgba(245,197,24,.28)!important}"
        ".ccPatchSessionItem[data-cc-color=yellow].ccPatchActive{background:rgba(245,197,24,.36)!important}"
        ".ccPatchSessionItem[data-cc-color=green]{background:rgba(48,164,108,.16)!important}"
        "button.ccPatchSessionItem[data-cc-color=green]:hover{background:rgba(48,164,108,.26)!important}"
        ".ccPatchSessionItem[data-cc-color=green].ccPatchActive{background:rgba(48,164,108,.34)!important}"
        ".ccPatchSessionItem[data-cc-color=blue]{background:rgba(0,145,255,.16)!important}"
        "button.ccPatchSessionItem[data-cc-color=blue]:hover{background:rgba(0,145,255,.26)!important}"
        ".ccPatchSessionItem[data-cc-color=blue].ccPatchActive{background:rgba(0,145,255,.34)!important}"
        ".ccPatchSessionItem[data-cc-color=purple]{background:rgba(142,78,198,.16)!important}"
        "button.ccPatchSessionItem[data-cc-color=purple]:hover{background:rgba(142,78,198,.26)!important}"
        ".ccPatchSessionItem[data-cc-color=purple].ccPatchActive{background:rgba(142,78,198,.34)!important}"
        # ── Selected/open row — theme- AND color-INDEPENDENT indicator ───────
        # The 6 color tints already paint EVERY row, and ONLY the Glow themes ship
        # their own active-outline; the default + all other themes had just a weak
        # blue bar/ring (the old focusBorder) that vanished on a blue chat and read
        # as nothing on default. Fix lives in the BASE css so EVERY theme gets it:
        # a solid THEME-FOREGROUND left NOTCH (white on dark / black on light) as
        # the hero — it can never collide with the 6 tints, exactly like Discord's
        # active-server bar / VS Code's open file — plus a supporting static ring
        # and bold name. Drawn as overlays (::before notch + ::after ring) so they
        # sit ON TOP of any tint and the Glow themes' animated ring layers cleanly
        # over the same row without fighting the notch.
        ".ccPatchSessionItem.ccPatchActive::before{content:\"\";position:absolute;"
        "left:0;top:3px;bottom:3px;width:4px;border-radius:0 4px 4px 0;z-index:4;"
        "background:var(--app-primary-foreground,#fff);"
        "box-shadow:0 0 0 1px var(--app-primary-background,rgba(0,0,0,.65))}"
        ".ccPatchSessionItem.ccPatchActive::after{content:\"\";position:absolute;"
        "inset:0;pointer-events:none;border-radius:5px;z-index:3;"
        "box-shadow:inset 0 0 0 1.5px var(--app-primary-foreground,#fff);opacity:.45}"
        ".ccPatchSessionItem.ccPatchActive .ccPatchRowName{font-weight:700!important}"
        ".ccPatchColorBtn{color:var(--app-secondary-foreground)}"
        # Color swatch popup
        ".ccPatchColorMenu{position:fixed;z-index:100001;display:flex;gap:7px;"
        "background:var(--app-primary-background);border:1px solid var(--app-primary-border-color);"
        "border-radius:9px;padding:8px;box-shadow:0 6px 22px rgba(0,0,0,.45)}"
        ".ccPatchColorSwatch{width:20px;height:20px;border-radius:50%;border:2px solid transparent;"
        "cursor:pointer;padding:0;background:transparent;position:relative;transition:transform .1s}"
        ".ccPatchColorSwatch:hover{transform:scale(1.18)}"
        ".ccPatchColorSwatchOn{border-color:var(--app-primary-foreground)}"
        ".ccPatchColorSwatchNone{border:1.5px dashed var(--app-secondary-foreground)}"
        ".ccPatchColorSwatchNone:after{content:'';position:absolute;left:50%;top:1px;bottom:1px;"
        "width:2px;margin-left:-1px;background:#e5484d;transform:rotate(45deg);border-radius:1px}"
        # Search row
        ...
        ".claudePatchStatusError{background:var(--vscode-charts-red,#e5484d)!important;"
        "animation:claudePatchErrorPulse 1.2s ease-in-out infinite}"
        "@keyframes claudePatchSpin{to{transform:rotate(360deg)}}"
        "@keyframes claudePatchWaitPulse{0%,100%{opacity:1}50%{opacity:.4}}"
        "@keyframes claudePatchErrorPulse{0%,100%{opacity:1}50%{opacity:.45}}"
        # Context menu
        ...
        # "Default" popup (new-chat defaults: model / mode / effort) — centered modal
        ".ccPatchDefaultsOverlay{position:fixed;inset:0;z-index:9000;display:flex;"
        "align-items:center;justify-content:center;background:rgba(0,0,0,.45)}"
        ".ccPatchDefaultsCard{min-width:360px;max-width:440px;background:var(--app-background,#1e1e1e);"
        "border:1px solid var(--app-border-color,rgba(255,255,255,.12));border-radius:14px;"
        "padding:18px 18px 16px;box-shadow:0 16px 48px rgba(0,0,0,.5)}"
        ".ccPatchDefHead{font-weight:600;font-size:15px;margin-bottom:2px}"
        ".ccPatchDefSub{font-size:11.5px;opacity:.6;line-height:1.35;margin-bottom:12px}"
        ".ccPatchDefRow{display:flex;align-items:center;gap:12px;padding:7px 0}"
        ".ccPatchDefRowLabel{font-size:13px;flex:0 0 auto;width:54px;opacity:.9}"
        ".ccPatchDefEffWrap{margin-left:auto;display:flex;align-items:center;gap:10px;min-width:172px}"
        ".ccPatchDefEffVal{font-size:12px;opacity:.75;width:62px;text-align:right}"
        ".ccPatchSel{position:relative;margin-left:auto;min-width:172px}"
        ".ccPatchSelBtn{display:flex;align-items:center;justify-content:space-between;gap:8px;width:100%;"
        "font-size:12.5px;background:var(--app-secondary-background,#2a2a2a);color:var(--app-foreground,#e6e6e6);"
        "border:1px solid var(--app-border-color,rgba(255,255,255,.16));border-radius:8px;padding:6px 10px;cursor:pointer;outline:none}"
        ".ccPatchSelBtn:hover{border-color:rgba(255,255,255,.32)}"
        ".ccPatchSelLabel{overflow:hidden;text-overflow:ellipsis;white-space:nowrap}"
        ".ccPatchSelChev{display:flex;opacity:.6;transition:transform .15s}"
        ".ccPatchSelOpen .ccPatchSelChev{transform:rotate(180deg)}"
        ".ccPatchSelMenu{position:absolute;top:calc(100% + 4px);left:0;right:0;"
        "background:var(--app-secondary-background,#262626);border:1px solid var(--app-border-color,rgba(255,255,255,.16));"
        "border-radius:8px;padding:4px;z-index:5;box-shadow:0 10px 28px rgba(0,0,0,.45);max-height:260px;overflow:auto}"
        ".ccPatchSelItem{font-size:12.5px;padding:6px 9px;border-radius:6px;cursor:pointer}"
        ".ccPatchSelItem:hover{background:var(--app-list-hover-background,rgba(255,255,255,.08))}"
        ".ccPatchSelItemOn{background:var(--app-list-active-selection-background,rgba(110,120,255,.28))}"
        ".ccPatchDefSaveRow{display:flex;justify-content:flex-end;padding-top:10px}"
        ".ccPatchSaveBtn{font-size:12.5px;font-weight:600;padding:6px 16px;border-radius:8px;"
        "border:1px solid var(--app-border-color,rgba(255,255,255,.16));background:transparent;"
        "color:var(--app-foreground,#e6e6e6);opacity:.4;cursor:default}"
        ".ccPatchSaveBtn.ccPatchSaveOn{opacity:1;cursor:pointer;background:#2563eb;border-color:#2563eb;color:#fff}"
        ".ccPatchSaveBtn.ccPatchSaveOn:hover{background:#1d4ed8}"
        ".ccPatchDefDivider{height:1px;background:var(--app-border-color,rgba(255,255,255,.1));margin:12px -4px 4px}"
        ".ccPatchDefActionRow{display:flex;align-items:center;justify-content:space-between;font-size:13px;"
        "padding:8px 4px;border-radius:8px;cursor:pointer}"
        ".ccPatchDefActionRow:hover{background:var(--app-list-hover-background,rgba(255,255,255,.06))}"
        ".ccPatchDefActionHint{font-size:11px;opacity:.55}"
        ".ccPatchDefToggleRow{display:flex;align-items:center;padding:6px 4px;border-radius:6px;cursor:pointer}"
        ".ccPatchDefToggleRow:hover{background:var(--app-list-hover-background,rgba(255,255,255,.06))}"
        ".ccPatchDefToggleLabel{font-size:12.5px}"
        ".ccPatchDefToggleParent{margin-top:8px}"
        ".ccPatchDefToggleParent .ccPatchDefToggleLabel{font-weight:600}"
        ".ccPatchDefToggleChild{padding-left:20px}"
        ".ccPatchDefToggleChild .ccPatchDefToggleLabel{font-size:12px;opacity:.8}"
        # Settings-row right-hand value (e.g. Auto-continue "On")
        ...
        "margin-left:auto;min-width:122px}"
        ".ccPatchEffTrack{position:relative;flex:1;height:16px;display:flex;"
        "align-items:center;cursor:pointer;touch-action:none}"
        ".ccPatchEffTrack::before{content:'';position:absolute;left:0;right:0;top:50%;"
        "transform:translateY(-50%);height:5px;border-radius:3px;background:rgba(255,255,255,.18)}"
        ".ccPatchEffFill{position:absolute;left:0;top:50%;transform:translateY(-50%);"
        "height:5px;border-radius:3px;background:var(--app-primary-foreground);pointer-events:none;transition:width .13s cubic-bezier(.4,0,.2,1)}"
        ".ccPatchEffUltra .ccPatchEffFill{background:#f97316}"
        ".ccPatchEffNotch{position:relative;z-index:1;width:5px;height:5px;border-radius:50%;"
        "background:rgba(255,255,255,.35);margin:0 auto;pointer-events:none}"
        ".ccPatchEffNotchUltra{background:#f97316}"
        ".ccPatchEffThumb{position:absolute;top:50%;width:11px;height:11px;border-radius:50%;"
        "background:var(--app-primary-foreground);transform:translate(-50%,-50%);"
        "box-shadow:0 1px 3px #00000066;pointer-events:none;transition:left .13s cubic-bezier(.4,0,.2,1)}"
        ".ccPatchEffUltra .ccPatchEffThumb{background:#f97316}"
        # While actively dragging, kill the glide so the thumb tracks the pointer
        # 1:1; the transition above then makes clicks + the release-snap smooth.
        ".ccPatchEffDragging .ccPatchEffThumb,.ccPatchEffDragging .ccPatchEffFill{transition:none}"
        ".ccPatchEffVal{flex:0 0 auto;font-size:11px;opacity:.65;min-width:46px;text-align:right;"
        "white-space:nowrap}"
        ...
        "display:flex;flex-direction:column;min-width:190px;max-width:300px;"
        "font-size:13px;color:var(--app-primary-foreground)}"
        ".ccPatchSettingsItem{display:flex;align-items:center;gap:8px;"
        "box-sizing:border-box;padding:6px 10px;border-radius:5px;cursor:pointer;"
        ...
        # Effort-row label is FIXED width so dragging the slider (which rewrites the
        # label Auto->Low->...->Ultracode live) can't change the row's width and
        # resize the whole menu mid-drag. 76px clears the longest name ("Extra high").
        ".ccPatchEffItem .ccPatchSettingsItemLabel{flex:0 0 auto;width:76px;"
        ...
        # Multi-account switcher dropdown — a wider floating panel (Google-style)
        # anchored to the sidebar's right edge; each account card shows avatar,
        # name/plan/check, email, and two thin 5h/weekly usage bars.
        ".ccPatchAcctMenu{position:fixed;z-index:10001;width:360px;max-width:94vw;"
        "max-height:70vh;overflow-y:auto;padding:10px;box-sizing:border-box;"
        "background:var(--app-primary-background);border:1px solid var(--app-primary-border-color);"
        "border-radius:14px;box-shadow:0 16px 46px #00000075;animation:ccPatchSlideUp .16s ease}"
        ".ccPatchAcctHead{font-size:14px;font-weight:600;padding:4px 6px 2px;color:var(--app-primary-foreground)}"
        ".ccPatchAcctSub{font-size:11.5px;line-height:1.35;padding:0 6px 8px;opacity:.6}"
        ".ccPatchAcctList{display:flex;flex-direction:column;gap:6px}"
        ".ccPatchAcctEmptyMsg{padding:14px 8px;text-align:center;font-size:12.5px;opacity:.6}"
        ".ccPatchAcctRow{position:relative;display:flex;gap:11px;align-items:flex-start;"
        "padding:11px 12px;border-radius:11px;border:1px solid transparent;cursor:pointer;"
        "transition:background .12s ease,border-color .12s ease}"
        ".ccPatchAcctRow:not(.ccPatchAcctRowActive):hover{background:rgba(127,127,127,.10)}"
        ".ccPatchAcctRowActive{cursor:default;background:rgba(127,127,127,.07);"
        "border-color:var(--app-primary-border-color)}"
        ".ccPatchAcctRowBusy{opacity:.55;pointer-events:none}"
        ".ccPatchAcctAvatar{flex:0 0 auto;width:34px;height:34px;border-radius:50%;"
        "display:flex;align-items:center;justify-content:center;font-size:12.5px;font-weight:600;"
        "background:linear-gradient(135deg,rgba(59,130,246,.9),rgba(147,51,234,.9));color:#fff;letter-spacing:.3px}"
        ".ccPatchAcctBody{flex:1 1 auto;min-width:0}"
        ".ccPatchAcctTop{display:flex;align-items:center;gap:7px}"
        ".ccPatchAcctName{font-size:13px;font-weight:600;color:var(--app-primary-foreground);"
        "white-space:nowrap;overflow:hidden;text-overflow:ellipsis}"
        ".ccPatchAcctPlan{flex:0 0 auto;font-size:9.5px;font-weight:600;text-transform:uppercase;"
        "letter-spacing:.4px;padding:1.5px 6px;border-radius:999px;background:rgba(127,127,127,.16);opacity:.8}"
        ".ccPatchAcctCheck{flex:0 0 auto;margin-left:auto;color:var(--vscode-charts-green,#3fb950);display:flex}"
        ".ccPatchAcctEmail{font-size:11.5px;opacity:.62;margin:1px 0 8px;white-space:nowrap;"
        "overflow:hidden;text-overflow:ellipsis}"
        ".ccPatchAcctBars{display:flex;flex-direction:column;gap:5px}"
        ".ccPatchAcctBarRow{display:flex;align-items:center;gap:8px}"
        ".ccPatchAcctBarLab{flex:0 0 20px;font-size:10px;font-weight:600;opacity:.55}"
        ".ccPatchAcctBarTrack{flex:1 1 auto;height:7px;border-radius:999px;overflow:hidden;"
        "background:rgba(127,127,127,.20)}"
        ".ccPatchAcctBarFill{height:100%;border-radius:999px;transition:width .3s ease}"
        ".ccPatchAcctBarEmpty .ccPatchAcctBarFill{background:transparent}"
        ".ccPatchAcctBarVal{flex:0 0 34px;text-align:right;font-size:10.5px;font-weight:600;"
        "font-variant-numeric:tabular-nums;opacity:.75}"
        ".ccPatchAcctRemove{position:absolute;top:8px;right:8px;width:20px;height:20px;padding:0;"
        "display:flex;align-items:center;justify-content:center;border:none;border-radius:6px;"
        "background:rgba(127,127,127,.14);color:var(--app-primary-foreground);opacity:.5;cursor:pointer;transition:opacity .12s ease,background .12s ease}"
        ".ccPatchAcctRow:hover .ccPatchAcctRemove{opacity:.8}"
        ".ccPatchAcctRemove:hover{opacity:1;background:rgba(229,72,77,.2);color:#e5484d}"
        ".ccPatchAcctFoot{margin-top:8px;padding-top:8px;border-top:1px solid var(--app-primary-border-color)}"
        ".ccPatchAcctAdd{display:flex;align-items:center;gap:8px;width:100%;padding:9px 12px;"
        "border:none;border-radius:10px;background:transparent;color:var(--app-primary-foreground);"
        "font-size:12.5px;font-weight:500;cursor:pointer}"
        ".ccPatchAcctAdd:hover{background:rgba(127,127,127,.12)}"
        # Switch-account confirmation modal — small, centered, blue accent
        ".ccPatchConfirmOverlay{position:fixed;inset:0;z-index:10002;"
        "background:rgba(0,0,0,.55);display:flex;align-items:center;justify-content:center;"
        "animation:ccPatchFadeIn .18s ease}"
        # Confirm dialog — Material-style surface to match the beta-program popup:
        # large rounded surface, soft tonal hero icon, light headline, roomy
        # supporting text, and a right-aligned row of pill buttons (low-emphasis
        # text Cancel + filled primary).
        ".ccPatchConfirmBox{width:min(360px,92vw);padding:26px 24px 18px;"
        "background:var(--app-primary-background);border:1px solid var(--app-primary-border-color);"
        "border-radius:26px;box-shadow:0 18px 50px #00000070;"
        "display:flex;flex-direction:column;align-items:center;text-align:center;"
        "animation:ccPatchSlideUp .22s ease}"
        ".ccPatchConfirmIcon{display:flex;align-items:center;justify-content:center;"
        "width:54px;height:54px;border-radius:50%;margin:2px 0 16px;"
        "background:rgba(59,130,246,.16);color:var(--vscode-charts-blue,#3b82f6)}"
        ".ccPatchConfirmIcon svg{width:24px;height:24px}"
        ".ccPatchConfirmTitle{font-size:19px;font-weight:500;margin:0 0 11px;letter-spacing:0;line-height:1.25}"
        ".ccPatchConfirmDesc{font-size:13px;line-height:1.5;margin:0 2px 11px;max-width:312px;"
        "color:var(--vscode-descriptionForeground,var(--app-primary-foreground))}"
        ".ccPatchConfirmNote{font-size:11.5px;line-height:1.5;margin:0 2px 16px;max-width:312px;opacity:.72;"
        "color:var(--vscode-descriptionForeground,var(--app-primary-foreground))}"
        ".ccPatchConfirmStatus{font-size:11.5px;opacity:.6;margin:0 0 12px;min-height:0}"
        ".ccPatchConfirmActions{display:flex;gap:8px;width:100%;justify-content:flex-end}"
        ".ccPatchConfirmCancelBtn,.ccPatchConfirmConfirmBtn{flex:0 0 auto;padding:9px 18px;border-radius:999px;"
        "border:0;cursor:pointer;font-size:13px;font-weight:600;transition:background .12s,opacity .12s}"
        ".ccPatchConfirmCancelBtn{background:transparent;"
        "color:var(--vscode-descriptionForeground,var(--app-primary-foreground))}"
        ".ccPatchConfirmCancelBtn:hover{background:rgba(127,127,127,.16)}"
        ".ccPatchConfirmConfirmBtn{background:var(--vscode-button-background,#2563eb);"
        "color:var(--vscode-button-foreground,#fff)}"
        ".ccPatchConfirmConfirmBtn:hover{background:var(--vscode-button-hoverBackground,#1d4ed8)}"
...
# Host-side API error capture (ring buffer). Wraps globalThis.fetch in the
# EXTENSION HOST so Orbit can finally record what the Anthropic API actually
# returned — the HTTP status (429/529/5xx), the error type
# (overloaded_error / rate_limit_error), request-id, retry-after, and the
# anthropic-ratelimit-*-reset headers — for the last ~40 calls. The webview's
# "Copy diagnostics" button reads this over the orbit_diagnostics RPC, so a
# server error is identifiable by its real status/type instead of a bare
# "API Error" string (which is all the renderer ever sees).
#
# PRIVACY (matches Jacob's "never read a secret" rail): it NEVER touches request
# headers (so the api key / OAuth bearer is never read), NEVER reads SUCCESS
# bodies (no prompt or response content), and only keeps a small JSON error body
# (status >= 400, which is a non-secret server error like {"type":"overloaded_error"})
# plus a fixed whitelist of safe response headers. Non-Anthropic requests are a
# pure pass-through. Wraps the global once (guarded by __ccOrbitApiLog) and is
# prepended at the very top of extension.js (like the zombie reaper) so it
# installs before the bundled SDK captures its fetch reference.
CLAUDE_HOST_APILOG_JS = r'''
;(function ccOrbitApiLogInit(){
try{
if(globalThis.__ccOrbitApiLog)return;
globalThis.__ccOrbitApiLog=1;
var BUF=globalThis.ccOrbitApiLog=[];
var MAX=40;
var KEEP=["request-id","x-request-id","retry-after","x-should-retry","anthropic-ratelimit-unified-reset","anthropic-ratelimit-unified-status","anthropic-ratelimit-requests-reset","anthropic-ratelimit-requests-remaining","anthropic-ratelimit-tokens-reset","anthropic-ratelimit-input-tokens-reset","anthropic-ratelimit-output-tokens-reset"];
function safeHeaders(h){var o={};try{if(h&&typeof h.get==="function"){for(var i=0;i<KEEP.length;i++){var v=h.get(KEEP[i]);if(v!=null&&v!=="")o[KEEP[i]]=String(v)}}}catch(e){}return o}
function rec(e){try{BUF.push(e);while(BUF.length>MAX)BUF.shift()}catch(_){}}
function isAnthropic(u){try{return /(^|[\/.])anthropic\.com([:\/]|$)/i.test(String(u||""))}catch(e){return false}}
function pathOf(u){try{var p=String(u||"").replace(/[?#].*$/,"").replace(/^[a-z]+:\/\/[^\/]+/i,"");return p||String(u||"")}catch(e){return ""}}
...
if(!isAnthropic(url))return of.apply(this,arguments);
var method="GET";try{method=(init&&init.method)||(input&&input.method)||"GET"}catch(e){}
var path=pathOf(url);var started=Date.now();
var p;try{p=of.apply(this,arguments)}catch(e){rec({t:started,method:method,path:path,status:null,ok:false,kind:"throw",err:String((e&&e.message)||e).slice(0,200)});throw e}
if(!p||typeof p.then!=="function")return p;
return p.then(function(resp){
try{
var st=resp?resp.status:null;
var e={t:started,ms:Date.now()-started,method:method,path:path,status:st,ok:!!(resp&&resp.ok),statusText:(resp&&resp.statusText)||"",headers:safeHeaders(resp&&resp.headers)};
if(typeof st==="number"&&st>=400){
try{resp.clone().text().then(function(txt){try{var j=JSON.parse(String(txt||""));if(j&&j.error){e.errType=j.error.type||null;e.errMsg=(j.error.message!=null?String(j.error.message).slice(0,400):null)}else if(j&&j.type){e.errType=String(j.type)}}catch(_){}}).catch(function(){})}catch(_){}
}
rec(e);
}catch(_){}
return resp;
},function(err){try{rec({t:started,ms:Date.now()-started,method:method,path:path,status:null,ok:false,kind:"reject",err:String((err&&err.message)||err).slice(0,200)})}catch(_){}throw err});
};
}catch(e){}
})();
'''
...
# Multi-account switcher — host side. All credential-file I/O + the per-account
# profile/usage fetches run HERE, in the extension host, so OAuth tokens NEVER
# enter the webview (they only ever move between files on disk). The webview's
# orbit_accounts RPC dispatches on `action`. Storage: ~/.claude/orbit-accounts/
# holds <id>.cred.json (the full credential blob, mirror of ~/.claude/.credentials.json)
# + <id>.meta.json (non-secret metadata: label/email/name/plan/cached-usage). id =
# the account's organizationUuid. Switching copies a saved .cred.json over
# ~/.claude/.credentials.json — but ONLY after ccFresh() guarantees a live access
# token: a slot that sat idle past its expiresAt is refreshed host-side first
# (ccRefresh — the exact native doRefreshOAuthToken request) and the rotated
# tokens are persisted back into the slot. Swapping in a stale blob was the
# 1.2.141 "switched, then logged out 20s later" bug: Claude's next auth check hit
# the hours-dead access token and killed the session. If the refresh itself fails
# (revoked/expired refresh token) the switch FAILS LOUDLY with needsLogin and the
# current login is left untouched. getOAuthTokens() reads the file fresh per
# request, so the next message/new chat is the new account with no reload.
# ccEnsure() auto-saves the CURRENT login before any switch so it's never lost
# (skipping token-less mid-logout files that would poison a slot). Placeholders
# __MSG__/__FS__/__PATH__ are replaced with the captured host request/fs/path
# identifiers.
ORBIT_ACCOUNTS_CASE = r'''case"orbit_accounts":{
let ccOs=require("os"),ccFs=__FS__,ccPa=__PATH__,ccReq=__MSG__.request,ccAct=ccReq.action;
let ccHome=ccOs.homedir(),ccDir=ccPa.join(ccHome,".claude","orbit-accounts"),ccCredP=ccPa.join(ccHome,".claude",".credentials.json"),ccBase="https://api.anthropic.com",ccBeta="oauth-2025-04-20";
let ccRJ=function(p){try{if(ccFs.existsSync(p))return JSON.parse(ccFs.readFileSync(p,"utf8"))}catch(e){}return null};
let ccWJ=function(p,o){try{ccFs.mkdirSync(ccPa.dirname(p),{recursive:!0});ccFs.writeFileSync(p,JSON.stringify(o),"utf8")}catch(e){}};
let ccSan=function(s){return String(s||"").toLowerCase().replace(/[^a-z0-9]/g,"_")};
// STABLE account id. organizationUuid is constant per account; NEVER key off the
// accessToken (it rotates on every refresh -> a new id -> a duplicate slot for the
// same account, the "it duplicated itself" bug). No orgUuid in the file yet -> null
// (ccEnsure then fetches it from /profile before deciding the id).
let ccIdOf=function(c){try{return(c&&c.organizationUuid)||null}catch(e){return null}};
let ccMP=function(id){return ccPa.join(ccDir,id+".meta.json")};
let ccCP=function(id){return ccPa.join(ccDir,id+".cred.json")};
let ccTok=function(c){try{return c&&c.claudeAiOauth&&c.claudeAiOauth.accessToken}catch(e){return null}};
let ccOa=function(c){return(c&&c.claudeAiOauth)||null};
// A saved blob is STALE when its access token is missing or expires within 5
// minutes. Access tokens only live hours: any account that sat saved while
// another was active WILL be stale, and swapping a stale blob into the live
// creds file gets the user logged out by Claude's next auth check (~20s).
let ccStale=function(c){let o=ccOa(c);if(!o||!o.accessToken)return!0;return((o.expiresAt||0)-Date.now())<3e5};
// Refresh a saved blob's OAuth tokens EXACTLY like the native doRefreshOAuthToken
// (same endpoint, client_id, JSON body and response mapping, lifted from the
// installed bundle) so a switched-in account always lands with a LIVE token.
let ccRefresh=async function(blob){let o=ccOa(blob);if(!o||!o.refreshToken)return{ok:!1,reason:"no refresh token"};try{let sc=(o.scopes&&o.scopes.length)?o.scopes:["user:inference","user:profile"];let r=await fetch("https://platform.claude.com/v1/oauth/token",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({grant_type:"refresh_token",refresh_token:o.refreshToken,client_id:"9d1c250a-e61b-44d9-88ed-5944d1962f5e",scope:sc.join(" ")})});if(!r||!r.ok)return{ok:!1,reason:"HTTP "+(r?r.status:"no response")};let d=await r.json();if(!d||!d.access_token)return{ok:!1,reason:"no access_token in response"};let nb=Object.assign({},blob);nb.claudeAiOauth=Object.assign({},o,{accessToken:d.access_token,refreshToken:d.refresh_token||o.refreshToken,expiresAt:Date.now()+(d.expires_in||3600)*1000,scopes:d.scope?d.scope.split(" "):sc});return{ok:!0,blob:nb}}catch(e){return{ok:!1,reason:String(e)}}};
// Freshness gate for slot `id`: refresh when stale and persist the rotated
// tokens back into the slot IMMEDIATELY (refresh tokens rotate — losing the new
// one kills the slot). De-duped per id on globalThis so the dropdown's usage
// fetch and a switch racing on the same stale slot share ONE refresh instead of
// burning the one-shot rotating token twice.
let ccFresh=function(id,blob){let g=globalThis.ccOrbitAcctFresh=globalThis.ccOrbitAcctFresh||{};if(g[id])return g[id];if(!ccStale(blob))return Promise.resolve({ok:!0,blob:blob});let p=(async function(){let r;try{r=await ccRefresh(blob);if(r.ok)ccWJ(ccCP(id),r.blob)}catch(e){r={ok:!1,reason:String(e)}}delete g[id];return r})();g[id]=p;return p};
let ccMetas=function(){let out=[];try{if(ccFs.existsSync(ccDir)){let fl=ccFs.readdirSync(ccDir);for(let f of fl){if(f.slice(-10)===".meta.json"){let m=ccRJ(ccPa.join(ccDir,f));if(m&&m.id)out.push(m)}}}}catch(e){}return out};
let ccHdr=function(tok){return{Authorization:"Bearer "+tok,"anthropic-beta":ccBeta,"Content-Type":"application/json"}};
let ccProfile=async function(tok){try{let r=await fetch(ccBase+"/api/oauth/profile",{headers:ccHdr(tok)});if(!r||!r.ok)return null;let d=await r.json();let a=(d&&d.account)||{},o=(d&&d.organization)||{};return{email:a.email_address||a.email||"",name:a.full_name||a.display_name||a.name||o.name||"",orgUuid:o.uuid||""}}catch(e){return null}};
let ccUsage=async function(tok){try{let r=await fetch(ccBase+"/api/oauth/usage",{headers:ccHdr(tok)});if(!r||!r.ok)return null;return await r.json()}catch(e){return null}};
let ccActive=function(){let c=ccRJ(ccCredP);return c?ccIdOf(c):null};
// Snapshot the CURRENT active login under a STABLE id. If the creds file has no
// organizationUuid yet, fetch it from /profile FIRST so the id is stable from the
// very first save (never the rotating token). Falls back to an email-derived id
// only if org uuid is truly unavailable. Returns the saved meta (its id is the
// authoritative active id). A token-LESS creds file (mid-logout, e.g. during the
// "Add account" sign-out window) is never snapshotted — it would poison a good
// slot with an unusable blob.
let ccEnsure=async function(){let c=ccRJ(ccCredP);if(!c)return null;let tok=ccTok(c);if(!tok)return null;var orgUuid=c.organizationUuid||"",email="",name="";if(tok&&!orgUuid){var p=await ccProfile(tok);if(p){orgUuid=p.orgUuid||"";email=p.email||"";name=p.name||""}}var id=orgUuid||(email?("email_"+ccSan(email)):null);if(!id&&tok){var p2=await ccProfile(tok);if(p2){orgUuid=p2.orgUuid||orgUuid;email=p2.email||email;name=p2.name||name;id=orgUuid||(email?("email_"+ccSan(email)):null)}}if(!id)return null;let ex=ccRJ(ccMP(id))||{};if(!email)email=ex.email||"";if(!name)name=ex.name||"";ccWJ(ccCP(id),c);let meta={id:id,orgUuid:orgUuid||ex.orgUuid||"",label:ex.label||email||id,email:email,name:name,subscriptionType:(c.claudeAiOauth&&c.claudeAiOauth.subscriptionType)||ex.subscriptionType||"",usage:ex.usage||null,usageAt:ex.usageAt||0,savedAt:Date.now()};ccWJ(ccMP(id),meta);return meta};
// Self-heal: collapse any duplicate slots for the SAME account (same email, or
// same id) that older unstable-id builds created. Keep the active one (else the
// newest), delete the rest's meta + cred files.
let ccDedupe=function(activeId){try{let ms=ccMetas(),by={};ms.forEach(function(m){let k=m.email?("e:"+String(m.email).toLowerCase()):("i:"+m.id);(by[k]=by[k]||[]).push(m)});Object.keys(by).forEach(function(k){let g=by[k];if(g.length<2)return;let keep=null;for(let i=0;i<g.length;i++){if(g[i].id===activeId){keep=g[i];break}}if(!keep)keep=g.slice().sort(function(a,b){return(b.savedAt||0)-(a.savedAt||0)})[0];g.forEach(function(m){if(m.id!==keep.id){try{ccFs.unlinkSync(ccMP(m.id))}catch(e){}try{ccFs.unlinkSync(ccCP(m.id))}catch(e){}}})})}catch(e){}};
try{
if(ccAct==="list"){let am=await ccEnsure();let act=am?am.id:ccActive();ccDedupe(act);let ms=ccMetas();ms.forEach(function(m){m.active=(m.id===act)});return{type:"orbit_accounts_response",ok:!0,accounts:ms,activeId:act};}
if(ccAct==="save"){let m=await ccEnsure();return{type:"orbit_accounts_response",ok:!!m,account:m};}
if(ccAct==="switch"){let id=ccReq.id;await ccEnsure();let blob=ccRJ(ccCP(id));if(!blob)return{type:"orbit_accounts_response",ok:!1,error:"account not saved"};let fr=await ccFresh(id,blob);if(!fr.ok)return{type:"orbit_accounts_response",ok:!1,needsLogin:!0,error:"this account's sign-in has expired ("+fr.reason+") - remove it, then use Add account to sign in again"};try{ccFs.writeFileSync(ccCredP,JSON.stringify(fr.blob),"utf8")}catch(e){return{type:"orbit_accounts_response",ok:!1,error:String(e)}}return{type:"orbit_accounts_response",ok:!0,activeId:id};}
if(ccAct==="remove"){let id=ccReq.id;try{ccFs.unlinkSync(ccMP(id))}catch(e){}try{ccFs.unlinkSync(ccCP(id))}catch(e){}return{type:"orbit_accounts_response",ok:!0};}
if(ccAct==="rename"){let id=ccReq.id,m=ccRJ(ccMP(id));if(m){m.label=String(ccReq.label||m.label||id);ccWJ(ccMP(id),m)}return{type:"orbit_accounts_response",ok:!!m,account:m};}
if(ccAct==="usage"){let id=ccReq.id,blob=ccRJ(ccCP(id));if(!blob)return{type:"orbit_accounts_response",ok:!1};let fr=await ccFresh(id,blob),tok=ccTok(fr.ok?fr.blob:blob);if(!tok)return{type:"orbit_accounts_response",ok:!1};let u=await ccUsage(tok),p=await ccProfile(tok),m=ccRJ(ccMP(id))||{id:id};if(u){m.usage=u;m.usageAt=Date.now()}if(p){if(p.email)m.email=p.email;if(p.name)m.name=p.name}ccWJ(ccMP(id),m);return{type:"orbit_accounts_response",ok:!0,usage:u||m.usage||null,email:m.email||"",name:m.name||""};}
if(ccAct==="addlogin"){try{await require("vscode").commands.executeCommand("claude-vscode.logout")}catch(e){}return{type:"orbit_accounts_response",ok:!0};}
return{type:"orbit_accounts_response",ok:!1,error:"unknown action"};
}catch(ccE){try{this.output?.error?.("orbit_accounts failed: "+ccE)}catch(_){}return{type:"orbit_accounts_response",ok:!1,error:String(ccE)};}
}'''
...
def patch_extension_host_js(path: Path) -> bool:
    ...
    if "ccOrbitZombieReaper" not in text:
        ...
        changed = True
    if "ccOrbitApiLog" not in text:
        text = CLAUDE_HOST_APILOG_JS + "\n" + text
        changed = True
    # (YOLO mode auto-approve is handled PER-CHAT in the webview's
    # handleToolPermissionRequest — see patch_webview_js. The host stays native:
    # no global flag, so YOLO can never leak across chats.)
    ...
    insert = (
        ...
        # orbit_diagnostics: hand the webview's "Copy diagnostics" the host-side
        # API error ring buffer (ccOrbitApiLog) + the host's own versions, so the
        # report can name an API error by status/type/headers. Read-only; never
        # touches secrets.
        f'case"orbit_diagnostics":{{let ccLog=[];try{{ccLog=globalThis.ccOrbitApiLog||[]}}catch(ccE){{}}'
        f'let ccVer="";try{{let ccX=require("vscode").extensions.getExtension("anthropic.claude-code");ccVer=(ccX&&ccX.packageJSON&&ccX.packageJSON.version)||""}}catch(ccE){{}}'
        f'let ccNode="",ccEl="",ccPl="";try{{ccNode=(process&&process.versions&&process.versions.node)||"";ccEl=(process&&process.versions&&process.versions.electron)||"";ccPl=(process&&process.platform)||""}}catch(ccE){{}}'
        f'return{{type:"orbit_diagnostics_response",apiLog:ccLog,captured:!!globalThis.__ccOrbitApiLog,claudeVersion:ccVer,node:ccNode,electron:ccEl,platform:ccPl}}}}'
    )
    # Multi-account switcher case — appended as a plain string (no f-string brace
    # doubling) with the captured host identifiers substituted.
    insert = insert + ORBIT_ACCOUNTS_CASE.replace("__MSG__", msg).replace("__FS__", fs_id).replace("__PATH__", path_id)
...
def verify_extension_dir(extension_dir: Path) -> None:
    ...
    checks = {
        ...
        # Live run-state from the host broadcast so a background/streaming chat is
        # never mislabeled "dead": the indicator + activity text consult the
        # connection's sessionStates (state!=="idle" === running) via ccPatchRunState,
        # not just the attached-only local `busy` signal.
        "live run state": ("function ccPatchRunState(" in js
                           and "function ccPatchLiveState(" in js
                           and "sessionStates" in js
                           and "ccPatchRunState(e).running" in js),
        "error dot": "ccPatchErrorReason" in js and ".claudePatchStatusError" in css
                     and 'R1==="error"' in js,
        # The helper IIFE is strict; a bare name in render scope only resolves if
        # exported to globalThis. ccPatchErrorReason is called bare in the row
        # title (R1==="error" branch), so it MUST be in the Object.assign export
        # or the whole webview throws "ccPatchErrorReason is not defined" the
        # moment any session hits a rate-limit/error state. The `,` form appears
        # ONLY in the export list (every call site uses `(`), so it asserts the
        # export. (Latent since 1.2.66; fixed 1.2.70.)
        "error reason exported (render-scope safe)": "ccPatchErrorReason," in js,
        # The activity label patch calls ccPatchRecency from a render expression
        # outside the helper IIFE. Keep it exported or the webview crashes with
        # "ccPatchRecency is not defined" as soon as the session row renders.
        "recency exported (render-scope safe)": "ccPatchRecency," in js,
        ...
        # Session-list de-dup: one conversation must render as exactly one row.
        # ccPatchFilterSort must run the input through ccPatchDedupeSessions, and
        # the row key must fall back to the stable internalId (not the positional
        # idx) so re-sorts can't rebind a row to the wrong session.
        "session dedup": ("function ccPatchDedupeSessions(" in js
                          and "function ccPatchDedupeKey(" in js
                          and "function ccPatchDupeBetter(" in js
                          and "e=ccPatchDedupeSessions(e)" in js
                          and ".sessionId.value??" in js
                          and ".internalId??" in js),
        ...
        "modal overlay zboost": "{z-index:10000!important}" in css and ".overlay_Wc_2Bg{z-index:10000!important}" not in css,
        "collapsed pane z-index": ".ccPatchPaneHidden .claudePatchInlineSessions" in css and "z-index:40!important" in css,
        # ("switch model item" removed — the gear's "Switch model" was replaced by
        # the "Default" popup; see "defaults popup". The composer keeps its own model
        # picker for the current chat.)
        ...
        "fork action row": ('"ccPatchForkRow"' in js
                            ...
                            and 'type:"button",className:"ccPatchForkBtn"' in js
        ...
        # Background task-notifications reframed as a tidy card (sweep + CSS).
        "task note format": ("ccPatchTaskNoteSweep" in js and "data-cc-tasknote" in js
                             and "[data-cc-tasknote]" in css),
        # Refresh button in the sessions header + gentle auto-refresh poll.
        "refresh sessions": ("ccPatchRefreshBtn" in js and "ccPatchAutoRefreshTimer" in js),
        ...
        # "Default" gear popup: new-chat defaults (model / effort / YOLO). Replaces
        # the old "Switch model" gear item; applies via ccPatchNewChatDefaultsTick.
        "defaults popup": ("ccPatchShowDefaultsMenu" in js and "ccPatchDefaultModelGet" in js
                           and "ccPatchDefaultModeGet" in js and "ccPatchNewChatDefaultsTick" in js
                           and "ccPatchAvailableModels" in js and "ccPatchBuildSelect" in js
                           and "ccPatchModeStops" in js and "ccPatchSaveBtn" in js
                           and ".ccPatchDefaultsCard" in css and ".ccPatchSelBtn" in css
                           and ".ccPatchSaveBtn" in css),
        # YOLO auto-approve is PER-CHAT: it lives in the webview's
        # handleToolPermissionRequest, which only gets the ephemeral channelId, so the
        # short-circuit goes through ccPatchAutoApproveOnChannel (channelId -> live
        # session -> STABLE ccPatchYoloKey). Keying the store on channelId directly was
        # the regression (it is regenerated every turn) — assert the resolver + the
        # stable-key helper are both present. No host global, so it can't leak across
        # chats. (Always applied, independent of COMPOSER_REWORK.)
        "yolo per-chat handler": ("handleToolPermissionRequest" in js
                                  and "globalThis.ccPatchAutoApproveOnChannel(" in js
                                  and "function ccPatchAutoApproveOnChannel(" in js
                                  and "function ccPatchYoloKey(" in js
                                  and "ccPatchAutoApproveOnChannel," in js
                                  and 'result:{behavior:"allow",updatedInput:' in js
                                  # Source-recorded channelId->key map (O(1), scan-proof)
                                  and "function ccPatchYoloChanRecord(" in js
                                  and "ccPatchYoloChanRecord," in js
                                  and "globalThis.ccPatchYoloChanRecord(" in js
                                  # YOLO approves actions, not questions: AskUserQuestion /
                                  # ExitPlanMode are skipped so the user is still consulted.
                                  and "function ccPatchYoloIsAsk(" in js
                                  and "ccPatchYoloAskTools" in js
                                  and "AskUserQuestion" in js
                                  and "globalThis.ccPatchYoloIsAsk(" in js),
        "yolo mode menu": ('label:"YOLO mode"' in js and 'label:"Bypass permissions"' not in js
                           and "availableModes:(function(ccA)" in js
                           and "ccPatchAutoApproveSet(ccPatchYoloKey(" in js
                           and "ccPatchAutoApproveOn(ccPatchYoloKey(" in js
                           and "ccPatchYoloConfirm" in js),
        # Goal enforcement: gear toggle that installs/removes a generic always-on
        # prompt-based Stop hook in ~/.claude/settings.json (reuses the generic
        # read_claude_md/write_claude_md file bridge), marked ORBIT-GOAL-ENFORCEMENT
        # so only our own hook entry is ever added/removed.
        "goal enforcement toggle": ("ccPatchGoalApply" in js and "ccPatchGoalOn" in js
                                    and "ccPatchGoalHookPrompt" in js
                                    and "ORBIT-GOAL-ENFORCEMENT" in js
                                    and "Goal enforcement" in js),
        # Default effort: the inline gear-row slider builder + its exported getter
        # (the `,` form appears ONLY in the Object.assign export, so it asserts the
        # export the field initializer depends on) + the construction-time
        # effortLevel field patch.
        ...
        "default ultracode": ("ccPatchDefaultUltracode," in js
                              and "`xhigh`,`Extra high`" in js and "`max`,`Max`" in js
                              and "`ultracode`,`Ultracode`" in js
        ...
        # Multi-account switcher: wider dropdown (name/email/usage bars) + host RPC
        # that swaps ~/.claude/.credentials.json server-side (tokens never enter webview).
        "multi account switcher": ("function ccPatchShowAccountsMenu(" in js
                                   and "ccPatchShowAccountsMenu(ctx)" in js
                                   and "function ccPatchAcctRpc(" in js
                                   and "ccPatchAcctBar" in js
                                   and ".ccPatchAcctMenu" in css
                                   and ".ccPatchAcctBarFill" in css
                                   and 'case"orbit_accounts"' in host
                                   and "orbit-accounts" in host
                                   and "/api/oauth/usage" in host),
        # Switch-time OAuth refresh (1.2.145): stale saved accounts are refreshed
        # host-side before the swap, so switching never hands Claude a dead token.
        "account switch refresh": ("ccOrbitAcctFresh" in host
                                   and "platform.claude.com/v1/oauth/token" in host
                                   and 'grant_type:"refresh_token"' in host
                                   and "needsLogin" in host),
        ...
        "build channel marker": 'var ccPatchChannel="' in js,
        ...
        # Auto-continue error trigger: server-streamed API/rate-limit errors land
        # in the native session's reactive .error signal (NOT the message array nor
        # the rateLimitWarning banner), so ccPatchContErrorStop must read it; and the
        # tick must let a positively-detected error beat a SOFT rate-limit banner
        # while a REAL maxed usage window (>=95%) still waits for its reset. The
        # reactive signal does NOT survive a refresh, so ccPatchContErrorStop must ALSO
        # scrape the rendered errorBanner_ DOM, and ccPatchContTransientErr must force
        # the prompt-retry branch on a server "(not your usage limit)" rate-limit so it
        # is never parked in the multi-hour usage-reset wait.
        "auto-continue error detection": ("ccPatchContErrorStop" in js
                                          and "s.error&&s.error.value" in js
                                          # Live-meter is ground truth: a stale/cross-account
                                          # "you've hit your limit" banner (hard) with the live
                                          # window NOT maxed must NOT fire "keep going".
                                          and "var staleHard=" in js
                                          and "else if(staleHard){}" in js
                                          and 'errorBanner_' in js
                                          and "ccPatchContTransientErr" in js
                                          and "errStop||transient" in js
                                          # Reload-survival: errored chats are persisted to
                                          # localStorage (reliable signals only) and fed back
                                          # in as errRec after a reload.
                                          and "ccPatchContWasErrored" in js
                                          and "errRec&&maxed" in js),
        # Copy diagnostics: the gear-menu button + the webview report builder, plus
        # the HOST-side fetch ring buffer (ccOrbitApiLog) exposed over the
        # orbit_diagnostics RPC. The host capture is what finally identifies an API
        # error by status/type/headers instead of a bare string, so assert BOTH ends.
        "copy diagnostics": ("ccPatchCopyDiagnostics" in js and "ccPatchBuildDiag" in js
                             and "Copy diagnostics" in js and 'type:"orbit_diagnostics"' in js
                             and "ccOrbitApiLog" in host and "orbit_diagnostics_response" in host),
        ...
        "color theme": ("ccPatchThemeModal" in js and "ccPatchApplyTheme" in js
                        and "var ccPatchThemes=" in js and "ccPatchSpin" in js
                        and ".ccPatchThemeGrid" in css and ".ccPatchThemeCard" in css),
        "per-chat color": ("ccPatchShowColorMenu" in js and "ccPatchGetColor" in js
                           and '"data-cc-color":ccPatchGetColor(' in js
                           and "ccPatchColorBtn" in js
                           and "[data-cc-color=red]" in css and ".ccPatchColorMenu" in css
                           and ".ccPatchColorSwatch" in css),
        "right-click color": ("function ccPatchShowMenu(e,t,pLabel,n,sLabel,r,a,sess,rn)" in js
                              and "ccPatchCtxColors" in js
                              and ".ccPatchCtxColors" in css),
        "right-click rename": "if(rn)s(`Rename`,rn)" in js,
        # New Session buttons (collapsed "+" and full "New Session") must use the
        # in-pane create+activate handler, never the dead `startNewConversationTab`
        # editor-tab path that no-ops in Orbit's single-pane webview. Two markers:
        # the post-create re-assert, AND the ?session= URL clear that stops
        # doListSessions from bouncing the user back to the prior chat on refresh.
        "new session in-pane": ("createSession()).then(function(ccNs)" in js
                                and 'ccU.searchParams.delete("session")' in js),
    ...
    for _fid, _names in {
        ...
        if not feature_on(_fid):
            for _nm in _names:
                checks.pop(_nm, None)
    # CC 2.1.186 composer/fork rework deferred (see COMPOSER_REWORK_2186): these
    # markers come from blocks 17/18b/19/20/21, which are skipped until re-ported.
    if not COMPOSER_REWORK_2186:
        for _nm in (
            "yolo mode menu",
            "bottom bar reorder",
            "model popup outside-close",
            "effort slider tooltip",
            "bottom bar css",
            "queue display",
            "send menu",
            "fork action row",
    ...
    # Duplicate-declaration guard. A repeated `function ccPatchX(` is a fatal
    # "Identifier X has already been declared" SyntaxError in the webview's strict
    # module scope — it takes the WHOLE patched script down (dead sidebar). But
    # `node --check` parses index.js in a mode that tolerates the redeclaration, so
    # it silently false-greens (exactly how 1.2.143 shipped a dead sidebar: a new
    # helper reused the existing ccPatchSessionKey name). Catch it here, loudly.
    _decls = re.findall(r"function\s+(ccPatch\w+)\s*\(", js)
    _dupes = sorted({n for n in _decls if _decls.count(n) > 1})
    if _dupes:
        raise RuntimeError(
            "Verification failed: duplicate helper declaration(s) in webview "
            "index.js (fatal in the webview's strict module scope, though "
            "node --check tolerates it): " + ", ".join(_dupes)
        )
    node = shutil.which("node")
    if node:
        log("Running JS syntax check")
        subprocess.check_call([node, "--check", str(extension_dir / "webview" / "index.js")])
        # Also syntax-check the HOST bundle — we inject into it now (the
        # ccOrbitApiLog fetch wrapper + the orbit_diagnostics RPC case), and a
        # broken host file takes the whole extension down, so never ship it
        # unchecked.
        host_js = extension_dir / "extension.js"
        if host_js.exists():
            subprocess.check_call([node, "--check", str(host_js)])
    log(f"Verification passed ({len(checks)} checks)")


def read_source_modes(vsix: Path) -> dict[str, int]:
    """Capture each entry's full Unix st_mode (file-type + permission bits) from
    the stock VSIX so the re-zip can re-apply them verbatim.

    Job: keep Anthropic's executable bit on the bundled native binary.
    Why this way vs the rejected alternative (let zip_dir infer modes from disk):
    Python's zipfile.extractall() silently discards the Unix mode stored in each
    entry's external_attr high word, and ZipInfo.from_file() then re-reads the
    stripped on-disk mode. On macOS/Linux the binary is named `claude` (no
    extension), so it re-zips as 0644 — VS Code installs it non-executable,
    spawn() fails with EACCES, and the extension reports the native binary
    "exists but failed to launch" (Anthropic tags that spawn error as a
    ReferenceError). Windows escapes the bug only because its stat() fakes an
    exec bit for the `.exe` name. The single source of truth for these modes is
    the stock VSIX vsce produced; copying that word forward preserves it exactly.
    Primitive reused: the zip external_attr high word VS Code already honors.
    """
    modes: dict[str, int] = {}
...
def zip_dir(src: Path, dest: Path, source_modes: dict[str, int] | None = None) -> None:
    ...
    with zipfile.ZipFile(dest, "w", zipfile.ZIP_DEFLATED) as zf:
        for path in src.rglob("*"):
            ...
            mode = source_modes.get(arc)
            if not mode:
                # No recorded Unix mode (e.g. patching a local dir, not a VSIX):
                # fall back to zipfile's default so behavior is unchanged.
                zf.write(path, arc)
                continue
            info = zipfile.ZipInfo.from_file(path, arc)
            info.compress_type = zipfile.ZIP_DEFLATED
            # Re-apply the stock Unix mode (high word); keep from_file's MS-DOS
            # low word so the archive bit / directory flag stay correct.
            info.external_attr = (mode << 16) | (info.external_attr & 0xFFFF)
            with path.open("rb") as fh, zf.open(info, "w") as out:
                shutil.copyfileobj(fh, out)


def verify_exec_bits(out_vsix: Path, source_modes: dict[str, int]) -> None:
    """The blade: refuse to ship a VSIX that dropped an executable bit the stock
    package carried.

    Job: make "native binary exists but failed to launch" structurally
    impossible to ship — not merely fixed once. extractall() silently strips Unix
    modes, so a future refactor of zip_dir, or a new bundled executable Anthropic
    adds, could reintroduce the brick without anyone noticing on Windows (whose
    stat() fakes an exec bit for `.exe`). This re-reads the finished VSIX and
    asserts every file that was executable in the stock package is still
    executable, aborting the patch instead of installing a non-launchable Claude
    on macOS/Linux. Catches the whole class, forever, on the user's own machine.
    """
    out_modes = read_source_modes(out_vsix)
    stripped = sorted(
        name for name, mode in source_modes.items()
        if not name.endswith("/")               # skip directory entries
        and (mode & 0o111)                       # stock entry was executable
        and not (out_modes.get(name, 0) & 0o111)  # output lost it (or went missing)
    )
    if stripped:
        raise RuntimeError(
            "Refusing to ship: patched VSIX lost the executable bit on "
            + ", ".join(stripped)
            + ". On macOS/Linux this surfaces as 'native binary exists but "
            "failed to launch'. The re-zip must preserve external_attr."
        )
...
def main() -> int:
    ...
    parser.add_argument("--download-dir", default=str(DEFAULT_ARTIFACTS_DIR))
    ...
    parser.add_argument(
        "--channel",
        default=None,
        help="Release channel embedded in the patched webview as ccPatchChannel "
             "('experimental' | 'stable'). Defaults to the patcher's own ORBIT_CHANNEL "
             "constant — the tag ships INSIDE the patcher, so the wrapper reads it back "
             "from the webview rather than inferring it.",
    )
    parser.add_argument(
        "--disable",
        default="",
        help="Comma-separated feature ids to SKIP (e.g. usage-meter,yolo-mode). "
             "Core scaffold and the sidebar cluster always apply.",
    )
    parser.add_argument(
        "--enable",
        default="",
        help="Comma-separated gateable feature ids to apply (narrows from all). "
    ...
    # Channel embedded into the webview. CLI --channel can override, but the
    # default is the patcher's own ORBIT_CHANNEL constant (the tag that ships with
    # this file). Validate to the only two real channels; anything else => the
    # standing default (experimental).
    global PATCHER_CHANNEL
    _ch = (args.channel or ORBIT_CHANNEL or "experimental").strip().lower()
    PATCHER_CHANNEL = _ch if _ch in ("experimental", "stable") else "experimental"

    global ENABLED_FEATURES
    _disable = {s.strip() for s in args.disable.split(",") if s.strip()}
    _enable = {s.strip() for s in args.enable.split(",") if s.strip()}
    if _disable and _enable:
        raise SystemExit("Use only one of --disable / --enable, not both.")
    _unknown = (_disable | _enable) - GATEABLE_FEATURES
    if _unknown:
        # Lenient on purpose: the wrapper and the patcher can be different
        # versions, so an unknown id just means "this patcher can't gate that" —
        # ignore it and apply the rest rather than failing the whole patch.
        print(f"[patcher] ignoring unknown/non-gateable feature id(s): "
              f"{', '.join(sorted(_unknown))} (gateable: {', '.join(sorted(GATEABLE_FEATURES))})",
              flush=True)
        _disable &= GATEABLE_FEATURES
        _enable &= GATEABLE_FEATURES
    if _disable:
        ENABLED_FEATURES = GATEABLE_FEATURES - _disable
    elif _enable:
        ENABLED_FEATURES = set(_enable)
    else:
    ...
    builds_dir = DEFAULT_ARTIFACTS_DIR
    ...
    with tempfile.TemporaryDirectory(prefix="claude-vsix-patch-") as temp:
        root = Path(temp) / "vsix"
        log("Extracting VSIX")
        # Capture the stock file modes before extractall() throws them away, so
        # the bundled native `claude` binary keeps its executable bit on
        # macOS/Linux (see read_source_modes).
        source_modes = read_source_modes(target)
        with zipfile.ZipFile(target) as zf:
            zf.extractall(root)
        changed = patch_extension_dir(root / "extension")
        log("Writing patched VSIX")
        zip_dir(root, out, source_modes)
        verify_exec_bits(out, source_modes)
        bin_mode = next(
            (m for a, m in source_modes.items() if "native-binary/claude" in a),
            None,
        )
        if bin_mode is not None:
            log(f"Preserved native binary mode: {oct(bin_mode & 0o7777)} (exec bits verified)")
