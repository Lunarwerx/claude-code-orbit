...
function ccPatchGoalHookPrompt(){return `ORBIT-GOAL-ENFORCEMENT: Decide whether Claude may stop now. Return {"ok":true} (allow stop) if ANY of these hold: (1) Claude has done all the work IT can do this turn without the user, and verified it; (2) the only things left are for the USER to do - test, review, run it, decide, approve, or provide a value - which is a handoff, NOT unfinished work for Claude; (3) Claude is waiting on a decision, value, approval, or confirmation of a destructive/irreversible step that only the user can give, and has said so; (4) the turn is a status update, a question to the user, or a conversational answer to what was asked. Return {"ok":false,"reason":"<specific reversible work Claude itself can do RIGHT NOW with no user input>"} ONLY when such concrete work actually exists. Never keep Claude working to make the user perform a future action, to re-verify something already proven, or because it merely feels like more could be done.`;}
