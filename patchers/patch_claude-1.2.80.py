...
def patch_webview_js(webview_js: Path) -> bool:
    ...
    RET = m_ret.group("ret")
    m_ik = re.search(
        rf"function (?P<ik>{JS_ID})\(\{{level:{JS_ID},levels:{JS_ID},showUltracode:{JS_ID},"
        rf"ultracodeSelected:{JS_ID},onSelect:{JS_ID},onSelectUltracode:{JS_ID}\}}\)",
        text,
    )
    if m_ik is None:
...
def patch_webview_css(webview_css: Path) -> bool:
    ...
    new = old + modal_overlay_rule + (
        ...
        "background:0 0;border:none;border-radius:2px;display:inline-flex;align-items:center;"
        "gap:4px;min-width:0;padding:2px 4px;font-family:inherit;font-size:.85em}"
        ...
        # left:0;right:0;bottom:100% so it already opens upward).
        ".ccPatchModelWrap>div{right:auto;min-width:240px;width:max-content;max-width:340px}"
...
def verify_extension_dir(extension_dir: Path) -> None:
    ...
    checks = {
        ...
        "bottom bar css": (".ccPatchModelWrap" in css and ".ccPatchModelBtn" in css
                           and ".ccPatchEffortSlider" in css),
