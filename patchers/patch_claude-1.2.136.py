...
// ephemeral per-turn channelId. We find the live session whose claudeChannelId
// matches it (claudeChannelId===channelId holds for the whole duration of the turn,
// which is when tool-permission requests fire) and key off its stable ccPatchYoloKey.
// No matching session (rare timing) => false, i.e. safely fall back to asking.
function ccPatchAutoApproveOnChannel(ch){if(!ch)return false;try{var ss=ccPatchContSessions();for(var i=0;i<ss.length;i++){var s=ss[i];if(s&&s.claudeChannelId===ch)return ccPatchAutoApproveOn(ccPatchYoloKey(s))}}catch(e){}return false}
