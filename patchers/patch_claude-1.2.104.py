...
function ccPatchGoalHookPrompt(){return `ORBIT-GOAL-ENFORCEMENT: Evaluate the conversation so far and decide if Claude may stop. Return {"ok":true} ONLY if the latest turn has genuinely (a) finished everything that can be done without the user AND verified it (proven, not just claimed), or (b) hit a real blocker - a decision, value, or approval only the user can give, or a destructive/irreversible step needing confirmation - and clearly stated that blocker, or (c) the exchange is purely conversational and the user's question is answered. Otherwise return {"ok":false,"reason":"<the specific reversible work that still remains>"} so Claude keeps working instead of handing unfinished work back. Do not let Claude stop merely because it feels like enough has been done.`;}
