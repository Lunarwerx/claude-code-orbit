...
function ccPatchEffortStops(){return[[``,`Auto`],[`low`,`Low`],[`medium`,`Medium`],[`high`,`High`],[`xhigh`,`Ultra`],[`max`,`Max`]]}
function ccPatchDefaultEffort(){try{return localStorage.getItem(`ccPatchDefaultEffort`)||``}catch(e){return``}}
...
function ccPatchBuildEffortSlider(){var stops=ccPatchEffortStops(),n=stops.length;var wrap=document.createElement(`div`);wrap.className=`ccPatchEffWrap`;var track=document.createElement(`div`);track.className=`ccPatchEffTrack`;var fill=document.createElement(`div`);fill.className=`ccPatchEffFill`;var thumb=document.createElement(`div`);thumb.className=`ccPatchEffThumb`;track.appendChild(fill);for(var i=0;i<n;i++){var nt=document.createElement(`div`);nt.className=`ccPatchEffNotch`;if(stops[i][0]===`xhigh`)nt.classList.add(`ccPatchEffNotchUltra`);track.appendChild(nt)}track.appendChild(thumb);var val=document.createElement(`span`);val.className=`ccPatchEffVal`;wrap.appendChild(track);wrap.appendChild(val);function render(idx){var pct=n>1?idx/(n-1)*100:0;thumb.style.left=pct+`%`;fill.style.width=pct+`%`;val.textContent=stops[idx][1];wrap.classList.toggle(`ccPatchEffUltra`,stops[idx][0]===`xhigh`)}function setIdx(idx,persist){idx=Math.max(0,Math.min(n-1,idx));render(idx);if(persist)ccPatchSetDefaultEffort(stops[idx][0])}function idxFromX(cx){var r=track.getBoundingClientRect();var f=r.width>0?(cx-r.left)/r.width:0;return Math.round(Math.max(0,Math.min(1,f))*(n-1))}setIdx(ccPatchEffortIndex(ccPatchDefaultEffort()),false);track.onmousedown=function(e){e.preventDefault();e.stopPropagation();setIdx(idxFromX(e.clientX),true);var mv=function(g){setIdx(idxFromX(g.clientX),true)};var up=function(){document.removeEventListener(`pointermove`,mv);document.removeEventListener(`pointerup`,up)};document.addEventListener(`pointermove`,mv);document.addEventListener(`pointerup`,up)};return wrap}
...
def patch_webview_css(webview_css: Path) -> bool:
    ...
    new = old + modal_overlay_rule + (
        ...
        "margin-left:auto;min-width:118px}"
        ...
        "transform:translateY(-50%);height:3px;border-radius:2px;background:rgba(255,255,255,.18)}"
        ".ccPatchEffFill{position:absolute;left:0;top:50%;transform:translateY(-50%);"
        "height:3px;border-radius:2px;background:var(--app-primary-foreground);pointer-events:none}"
        ".ccPatchEffUltra .ccPatchEffFill{background:#f97316}"
        ".ccPatchEffNotch{position:relative;z-index:1;width:3px;height:3px;border-radius:50%;"
