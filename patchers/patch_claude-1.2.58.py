...
def patch_webview_js(webview_js: Path) -> bool:
    ...
    # ──────────────────────────────────────────────────────────────────────
    # 23. Fork fix — the native $1() handler calls $.forkConversation(N,X,I)
    #     which can fail silently (RPC error, API change, etc.). When it
    #     fails, nothing happens — no notification, no fallback. Replace the
    #     entire $1() body with a single call to Q(X) (onCreateNewSession),
    #     which always works. Trade-off: no message history is preserved in
    #     the fork, but the user gets a working new session with their prompt
    #     text every time.
    # ──────────────────────────────────────────────────────────────────────
    fork_func_re = re.compile(
        ...
        rf'\$\.forkConversation\((?P=N),(?P=X),(?P<I>{JS_ID})\)\.catch'
    )
    ...
    if m_ff is None:
        ...
        if end_idx < 0:
            ...
            new_func = f"function {fn}(){{{Qn}({Xn})}}"
            text = text[:func_start] + new_func + text[end_idx:]
            log("Fork $1() replaced — always calls onCreateNewSession, never fails silently")
...
def verify_extension_dir(extension_dir: Path) -> None:
    ...
    checks = {
        ...
        # Block 23 — fork $1() replaced: $.forkConversation() should no longer
        # appear in the webview JS (it was only called from $1()).
        "fork simplified": '$.forkConversation(' not in js,
    }
