...
**Fork-from-here "branch now, visit later" (patcher v1.2.77) — kills the silent mid-stream fork no-op:** clicking "Fork" while a turn was streaming did nothing ~all of the time — you had to wait for the chat to fully stop, or it just "didn't care to work." **Files:** `Claude Code/patch_claude_vsix_v147.py` — new webview block 17b rewrites Claude's native fork handler `ee()`; added verify check `fork mid-stream fix` (gated under `fork-row`); corrected the now-stale "Fork is intentionally left stock" note. **Job:** make "Fork conversation from here" behave like the beloved GitHub-Copilot fork — one click snapshots everything ABOVE the message into a separate chat instantly, tells you it exists, and lets you open it at your leisure — and make it work WHILE a turn streams. **Why THIS way / what was broken:** native `ee()` calls the context service's `forkConversation`, which both (a) forks off the on-disk transcript and (b) immediately auto-navigates via `viewSession`. Both halves race a live mid-stream turn: while busy the session-list/disk snapshot lags the live conversation, so `viewSession → activateSessionFromServer`'s `listSessions().find(newId)` misses and returns a bare `false`, and the service's un-awaited call + `return""` swallows it. Net: click Fork mid-stream → the branch is usually CREATED but nothing visibly happens (no toast, no navigation). That SILENT navigation no-op — not a throw (a throw would toast) — is the "fork never works" report. Rewind works mid-stream because it routes through the LIVE channel (`withChannel → query.rewindFiles`) carrying the channelId; fork carries none and falls back to the stale disk store. Fix is a SUBTRACTION of the fragile auto-navigation dependency: call the raw connection RPC (`comms.connection.value.forkConversation(sessionId,resumeAt)`) to create the branch and get its id with NO navigation; always confirm with a toast ("Forked from here ✓ — open your new branch anytime", [Open] button); navigate ONLY on [Open], mirroring the service's own `openNewInTab`/`viewSession` choice (so it lands identically, but on demand, after the race is gone). Genuine failures now surface a real error toast instead of vanishing. **Rejected:** (1) interrupt-then-fork — deterministic but STOPS the running turn, which the user explicitly does not want (they fork to spawn parallel branches while the original keeps going); (2) a new host-side "live fork" RPC sourcing from the live channel like rewind — the gold standard for forking with zero disk dependency, but a far larger/riskier surface (transcript shape must match `getTranscript` incl. `file-history-snapshot` rows) — deferred as future work. **Most-loved shape:** every beloved fork UX (Copilot edit-branch, ChatGPT message-edit branches, Cursor checkpoints) is "snapshot-above + instant + non-disruptive + discoverable"; the toast is the discoverability the stock silent path lacked, on-demand [Open] is the non-disruptive navigation. **Primitive/debt:** zero new machinery — reuses the existing connection `forkConversation` RPC, the `showNotification(msg,severity,buttons)` primitive (now with its `["Open"]` + returned button value, previously only 2-arg for errors), and the service's own `viewSession`/`startNewConversationTab`. The first-message `if(A){r(o)}` branch and the "Unknown session id" guard are preserved byte-for-byte; a defensive fallback to the stock service call covers a future Claude build moving the `comms`/`connection` internals. Identifiers captured via named regex groups + backrefs (not hard-coded), so minifier renames across versions survive. **Blade:** verify check `fork mid-stream fix` = `"ccForkConn" in js and "ccForkNewId" in js`, gated under `fork-row`. Anchor is the unique "Unknown session id" fork string; the rewind-modal `g(b)` twin shares that string but cannot match this shape (its guards are `!b.resumeAtMessageId` / `n(b.promptText)`) — confirmed by a single-match assertion in testing. **Verdict:** KEEP. Verified end-to-end against stock Claude **2.1.163**: full patcher run clean (87 checks, JS syntax check passed, native exec bits preserved); the `ee()` regex matches exactly once and is idempotent (no re-match post-patch). Pending: live in-app confirmation + OTA ship (patcher_version bump + archive + push) on go-ahead.
